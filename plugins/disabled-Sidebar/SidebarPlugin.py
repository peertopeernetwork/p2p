...
class UiRequestPlugin(object):
    def actionUiMedia(self, path):
        if path == "/uimedia/all.js" or path == "/uimedia/all.css":
            ...
            if ext == "js":
                yield _.translateData(open(plugin_media_file).read()).encode(
                    "utf_8"
                )
            else:
                for part in self.actionFile(
                    plugin_media_file, send_header=False
                ):
                    yield part
    ...
    def actionZip(self):
        ...
        title = site.content_manager.contents.get("content.json", {}).get(
            "title", ""
        )
        filename = "%s-backup-%s.zip" % (
            title,
...
class UiWebsocketPlugin(object):
    def sidebarRenderPeerStats(self, body, site):
        ...
        type_defs = {
            "local-ipv4": {"order": -21, "color": "yellow"},
            "local-ipv6": {"order": -20, "color": "orange"},
            "ipv4": {"order": -11, "color": "blue"},
            "ipv6": {"order": -10, "color": "darkblue"},
            "unknown": {"order": 10, "color": "red"},
        }
        ...
        peer_ips = [
            peer.key
            for peer in site.getConnectablePeers(20, allow_private=False)
        ]
        peer_ips.sort(key=lambda peer_ip: ".onion:" in peer_ip)
        copy_link = "http://127.0.0.1:55555/%s/?p2p_peers=%s" % (
        ...
        body.append(
            ...
            <li>
             ...
             <ul class='graph graph-stacked'>
              ...
              <li style='width: {percent_connectable:.0%}' class='connectable back-blue' title='{_[Peers that were recently connected successfully]}'></li>
             ...
             <ul class='graph-legend'>
              ...
              <li class='color-blue' title='{_[Peers that were recently connected successfully]}'><span>{_[Connectable]}:</span><b>{peers_connectable}</b></li>
        ...
        if peers_total:
            for t in peers_by_type:
                ...
                """ % (
                    float(t["count"]) * 100.0 / peers_total,
                    t["type"],
                    t["color"],
                    t["readable_type"],
                )
    ...
    def sidebarRenderFileStats(self, body, site):
            ...
            <li>
             <label>
              ...
              <small class="label-right">
               <a href='/Peer-to-Peer-Network-Internal/Zip?address={site.address}' id='link-zip' class='link-right' download='site.zip'>{_[Save as .zip]}</a>
              </small>
             </label>
             <ul class='graph graph-stacked'>
        """
        ...
        for inner_path in contents:
            ...
            for file_name, file_details in list(content["files"].items()):
                ...
                size_filetypes[ext] = (
                    size_filetypes.get(ext, 0) + file_details["size"]
                )
        ...
        if "json" in size_filetypes:
            size_filetypes["json"] += max(
                0, site.settings["size"] - size_total
            )
        ...
        for extension, color in extensions:
            ...
            if extension == "Other":
                ...
                size = (
                    size_filetypes.get("jpg", 0)
                    + size_filetypes.get("png", 0)
                    + size_filetypes.get("gif", 0)
                )
                size_other -= size
            else:
    ...
    def sidebarRenderOptionalFileSettings(self, body, site):
        ...
        if hasattr(config, "autodownload_bigfile_size_limit"):
            autodownload_bigfile_size_limit = int(
                site.settings.get(
    ...
    def sidebarRenderBadFiles(self, body, site):
        ...
        for bad_file, tries in site.bad_files.items():
            ...
            body.append(
                _(
                    """<li class='color-red' title="{bad_file_path} ({tries})">{bad_filename}</li>""",
                    {
        ...
        if len(site.bad_files) > 30:
            ...
            body.append(
                _(
                    """<li class='color-red'>{_[+ {num_bad_files} more]}</li>""",
                    nested=True,
                )
            )
        body.append(
    ...
    def sidebarRenderIdentity(self, body, site):
        auth_address = self.user.getAuthAddress(
            self.site.address, create=False
        )
        rules = self.site.content_manager.getRules(
            "data/users/%s/content.json" % auth_address
        )
        if rules and rules.get("max_size"):
            quota = rules["max_size"] / 1024
            try:
                content = site.content_manager.contents[
                    "data/users/%s/content.json" % auth_address
                ]
                used = len(json.dumps(content)) + sum(
                    [file["size"] for file in list(content["files"].values())]
                )
            except:
                used = 0
            used = used / 1024
    ...
    def sidebarRenderControls(self, body, site):
        ...
        donate_key = site.content_manager.contents.get("content.json", {}).get(
            "donate", True
        )
        ...
        if donate_key is False or donate_key == "":
            pass
        elif (type(donate_key) == str or type(donate_key) == str) and len(
            donate_key
        ) > 0:
            body.append(
                _(
    ...
    def sidebarRenderOwnSettings(self, body, site):
        ...
        description = site.content_manager.contents.get(
            "content.json", {}
        ).get("description", "")
        body.append(
            _(
    ...
    def sidebarRenderContents(self, body, site):
        has_privatekey = bool(
            self.user.getSiteData(site.address, create=False).get("privatekey")
        )
        if has_privatekey:
            tag_privatekey = _(
                "{_[Private key saved.]} <a href='#Forget+private+key'"
                " id='privatekey-forget' class='link-right'>{_[Forget]}</a>"
            )
        else:
            tag_privatekey = _(
                "<a href='#Add+private+key' id='privatekey-add'"
                " class='link-right'>{_[Add saved private key]}</a>"
            )
        body.append(
            _(
                """
        ...
        for content in contents:
            body.append(
                _(
                    "<a href='{content}'"
                    " class='contents-content'>{content}</a> "
                )
            )
        body.append("</div>")
        body.append("</li>")
    ...
    def downloadGeoLiteDb(self, db_path):
        ...
        self.cmd(
            ...
            [
                "geolite-info",
                _[
                    "Downloading GeoLite2 City database (one time only,"
                    " ~20MB)…"
                ],
                0,
            ],
        ...
        for db_url in db_urls:
            ...
            try:
                    ...
                    if data_size:
                        ...
                        self.cmd(
                            ...
                            [
                                "geolite-info",
                                _[
                                    "Downloading GeoLite2 City database (one"
                                    " time only, ~20MB)…"
                                ],
                                progress,
                            ],
                ...
                self.cmd(
                    ...
                    [
                        "geolite-info",
                        _["GeoLite2 City database downloaded!"],
                        100,
                    ],
                )
                time.sleep(2)
                self.log.info(
                    "GeoLite2 City database is ready at: %s" % db_path
                )
                return True
        ...
        self.cmd(
            ...
            [
                "geolite-info",
                _[
                    "GeoLite2 City database download error: {}!<br>Please"
                    " download manually and unpack to data dir:<br>{}"
                ].format(download_err, db_urls[0]),
                -100,
            ],
        )
    ...
    def getGeoipDb(self):
        ...
        for path in db_paths:
            if os.path.isfile(path) and os.path.getsize(path) > 0:
                return path
        self.log.info(
            "GeoIP database not found at [%s]. Downloading to: %s",
            " ".join(db_paths),
            data_dir_db_path,
        )
    ...
    def getPeerLocations(self, peers):
        ...
        for peer in peers:
            ...
            latlon = "%s,%s" % (lat, lon)
            if latlon in placed and peer.getIpType() == "ipv4":
        ...
        try:
            ...
            for peer_location in peer_locations:
                if peer_location["ping"] == 0:
                    ...
                    height = min(
                        0.20,
                        math.log(1 + peer_location["ping"] / ping_avg, 300),
                    )
                else:
                    height = -0.03
                globe_data += [
            ...
            self.log.debug(
                "sidebarGetPeers error: %s" % Debug.formatException(err)
            )
    ...
    def actionSiteSetOwned(self, to, owned):
        if self.site.address == config.updatesite:
            return {
                "error": "You can't change the ownership of the updater site"
            }
    ...
    def actionSiteRecoverPrivatekey(self, to):
        ...
        if site_data.get("privatekey"):
            return {"error": "This site already has saved private key"}
        address_index = self.site.content_manager.contents.get(
            "content.json", {}
        ).get("address_index")
        if not address_index:
            return {"error": "No address_index in content.json"}
        privatekey = CryptBitcoin.hdPrivatekey(
            self.user.master_seed, address_index
        )
        ...
        if privatekey_address == self.site.address:
            ...
            return {
                "error": (
                    "Unable to deliver private key for this site from current"
                    " user's master_seed"
                )
            }
