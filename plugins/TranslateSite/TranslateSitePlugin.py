...
class UiRequestPlugin(object):
    def actionSiteMedia(self, path, **kwargs):
        ...
        if should_translate:
            ...
            file_generator = super(UiRequestPlugin, self).actionSiteMedia(
                path, **kwargs
            )
            if "__next__" in dir(file_generator):
                site = self.server.sites.get(path_parts["address"])
                if not site or not site.content_manager.contents.get(
                    "content.json"
                ):
                    return file_generator
                return self.actionPatchFile(
                    site, path_parts["inner_path"], file_generator
                )
            else:
                return file_generator
        else:
    ...
    def actionUiMedia(self, path):
        ...
        if translate.lang != "en" and path.endswith(".js"):
            ...
            data = translate.translateData(data.decode("utf_8"))
            self.log.debug(
                "Patched %s (%s bytes) in %.3fs"
                % (path, len(data), time.time() - s)
            )
            return iter([data.encode("utf_8")])
        else:
            return file_generator
    ...
    def actionPatchFile(self, site, inner_path, file_generator):
        ...
        if site.settings.get("own"):
            ...
            if lang_file in content_json.get("files", {}):
                lang_file_exist = True
        if not lang_file_exist or inner_path not in content_json.get(
            "translate", []
        ):
            for part in file_generator:
                if inner_path.endswith(".html"):
                    yield part.replace(
                        b"lang={lang}",
                        b"lang=" + translate.lang.encode("utf_8"),
                    )
                else:
                    yield part
        else:
            s = time.time()
            data = b"".join(list(file_generator)).decode("utf_8")
            site.needFile(lang_file, priority=10)
            try:
                if inner_path.endswith("js"):
                    data = translate.translateData(
                        data, site.storage.loadJson(lang_file), "js"
                    )
                else:
                    data = translate.translateData(
                        data, site.storage.loadJson(lang_file), "html"
                    )
            except Exception as err:
                site.log.error(
                    "Error loading translation file %s: %s" % (lang_file, err)
                )
            self.log.debug(
                "Patched %s (%s bytes) in %.3fs"
                % (inner_path, len(data), time.time() - s)
            )
            yield data.encode("utf_8")
