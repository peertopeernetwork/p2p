...
try:
    local_master_addresses = set(
        json.load(open("%s/users.json" % config.data_dir)).keys()
    )
...
class UiRequestPlugin(object):
    ...
    def actionWrapper(self, path, extra_headers=None):
        ...
        if config.multiuser_no_new_sites:
            ...
            if not self.server.site_manager.get(match.group("address")) and (
                not user or user.master_address not in local_master_addresses
            ):
                self.sendHeader(404)
        ...
        if user_created:
            ...
            extra_headers["Set-Cookie"] = (
                "master_address=%s;path=/;max-age=2592000;"
                % user.master_address
            )
        ...
        if not back_generator:
            ...
            inject_html = """
                ...
                <script nonce="{script_nonce}">
                 setTimeout(function() {
                    p2pframe.cmd("wrapperNotification", ["done", "{message}<br><small>You have been logged in successfully</small>", 5000])
                 }, 1000)
                </script>
                </body>
                </html>
            ...
            return iter(
                [re.sub(b"</body>\s*</html>\s*$", inject_html.encode(), back)]
            )
        else:
...
class UiWebsocketPlugin(object):
    ...
    def formatServerInfo(self):
        ...
        if "ADMIN" in self.site.settings["permissions"]:
            ...
            is_multiuser_admin = (
                ...
                or self.user.master_address in local_master_addresses
            )
    ...
    def actionUserShowMasterSeed(self, to):
        message = (
            "<b style='padding-top: 5px; display: inline-block'>Your unique"
            " private key:</b>"
        )
        message += (
            "<div style='font-size: 84%%; background-color: #FFF0AD; padding:"
            " 5px 8px; margin: 9px 0px'>%s</div>"
            % self.user.master_seed
        )
        message += (
            "<small>(Save it, you can access your account using this"
            " information)</small>"
        )
    ...
    def actionUserLogout(self, to):
        message = (
            ...
            " id='button_notification'>Login to another account</a>"
        )
        self.cmd("notification", ["done", message, 1000000])
        script = (
            "document.cookie = 'master_address=; expires=Thu, 01 Jan 1970"
            " 00:00:00 UTC; path=/';"
        )
        script += (
            "$('#button_notification').on('click', function() {"
            ' p2pframe.cmd("userLoginForm", []); });'
        )
    ...
    def actionUserSet(self, to, master_address):
        ...
        script += "p2pframe.cmd('wrapperReload', ['login=done']);"
    ...
    def actionUserSelectForm(self, to):
        ...
        for master_address, user in user_manager.list().items():
            ...
            if user.certs:
                ...
                title = "%s@%s" % (
                    user.certs[first_cert]["auth_user_name"],
                    first_cert,
                )
            else:
            ...
            if is_active:
                ...
                css_class = "noclass"
            body += (
                "<a href='#Select+user' class='select select-close user %s'"
                " title='%s'>%s</a>" % (css_class, user.master_address, title)
            )
        script = (
            """
        ...
        self.cmd(
            "notification",
            ["ask", body],
            lambda master_address: self.actionUserSet(to, master_address),
        )
        self.cmd("injectScript", script)
    ...
    def actionUserLoginForm(self, to):
        self.cmd(
            "prompt",
            ["<b>Login</b><br>Your private key:", "password", "Login"],
            self.responseUserLogin,
        )
    ...
    def responseUserLogin(self, master_seed):
        ...
        if user.master_address:
            ...
            script += "p2pframe.cmd('wrapperReload', ['login=done']);"
            self.cmd(
                "notification", ["done", "Successful login, reloading page…"]
            )
            self.cmd("injectScript", script)
        else:
    ...
    def hasCmdPermission(self, cmd):
        ...
        is_public_proxy_user = (
            not config.multiuser_local
            and self.user.master_address not in local_master_addresses
        )
        if is_public_proxy_user and "no_multiuser" in flags:
            self.cmd(
                ...
                [
                    "info",
                    _("This function ({cmd}) is disabled on this proxy!"),
                ],
            )
            return False
        else:
    ...
    def actionCertAdd(self, *args, **kwargs):
        ...
        message = """
            ...
            <b>Hello, welcome to Peer-to-Peer Network Proxy!</b><div style='margin-top: 8px'>A new, unique account created for you:</div>
            ...
            <div style='text-align: center; font-size: 85%; margin-bottom: 10px;'>
             ...
             class='masterseed_download' download='p2p_private_key.backup'>Download backup as text file</a>
            </div>
            <div>
            ...
            <small>This site allows you to browse Peer-to-Peer Network content, but if you want to secure your account <br>
            and help to keep the network alive, then please run your own <a href='https://peer-to-peer.network' target='_blank'>Peer-to-Peer Network client</a>.</small>
        """
        self.cmd("notification", ["info", message])
        script = """
            ...
            $("#button_notification_download").on("mousedown", function() {
                this.href = window.URL.createObjectURL(new Blob(["Peer-to-Peer Network user master seed:\\r\\n{master_seed}"]))
            })
        """.replace(
    ...
    def actionPermissionAdd(self, to, permission):
        ...
        if permission == "NOSANDBOX" and is_public_proxy_user:
            ...
            return super(UiWebsocketPlugin, self).actionPermissionAdd(
                to, permission
            )
...
class ConfigPlugin(object):
    def createArguments(self):
        ...
        group.add_argument(
            "--multiuser_local",
            help="Enable unsafe Ui functions and write users to disk",
            action="store_true",
        )
        group.add_argument(
            "--multiuser_no_new_sites",
            help="Denies adding new sites by normal users",
            action="store_true",
        )
