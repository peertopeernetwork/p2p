...
class UiWebsocketPlugin(object):
    ...
    def addBigfileInfo(self, row):
        ...
        if piecefield:
            ...
            row["downloaded_percent"] = (
                100 * row["pieces_downloaded"] / row["pieces"]
            )
            if row["pieces_downloaded"]:
                if row["pieces"] == row["pieces_downloaded"]:
                    ...
                    row["bytes_downloaded"] = row[
                        "pieces_downloaded"
                    ] * file_info.get("piece_size", 0)
            else:
        ...
        for peer in site.peers.values():
            if (
                not peer.time_piecefields_updated
                or sha512 not in peer.piecefields
            ):
                continue
    ...
    def actionOptionalFileList(
        ...
        if wheres_raw:
            ...
            query_wheres_raw = ""
        query = (
            "SELECT * FROM file_optional %s WHERE ? %s ORDER BY %s LIMIT %s"
            % (
                join,
                query_wheres_raw,
                orderby,
    ...
    def actionOptionalFileInfo(self, to, inner_path):
        ...
        if (
            ...
            gevent.spawn(self.updatePeerNumbers)
        query = (
            "SELECT * FROM file_optional WHERE site_id = :site_id AND"
            " inner_path = :inner_path LIMIT 1"
        )
        res = content_db.execute(
            query, {"site_id": site_id, "inner_path": inner_path}
        )
        row = next(res, None)
        if row:
            row = dict(row)
    ...
    def actionOptionalFilePin(self, to, inner_path, address=None):
        ...
        if back == "ok":
            if num_file == 1:
                ...
                self.cmd(
                    "notification",
                    ["done", _["Pinned %s files"] % num_file, 5000],
                )
        self.response(to, back)
    ...
    def actionOptionalLimitStats(self, to):
        ...
        back[
            "used"
        ] = self.site.content_manager.contents.db.getOptionalUsedBytes()
    ...
    def actionOptionalLimitSet(self, to, limit):
        config.optional_limit = re.sub(r"\.0+$", "", limit)
    ...
    def actionOptionalHelp(self, to, directory, title, address=None):
        ...
        stats = content_db.execute(
            (
                ...
                " WHERE site_id = :site_id AND inner_path LIKE :inner_path"
            ),
        ...
        self.cmd(
            ...
            [
                ...
                _[
                    ...
                    " <b>%s</b>.<br><small>Directory: %s</small>"
                ]
        ...
        if value:
            if "ADMIN" in self.site.settings["permissions"]:
                ...
                self.cmd(
                    ...
                    [
                        _[
                            "Help distribute all new optional files on site"
                            " <b>%s</b>"
                        ]
