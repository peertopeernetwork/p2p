    ...
    if access_log:
        ...
        for site_id in access_log_prev:
            content_db.execute(
                ...
                {
                    "site_id": site_id,
                    "inner_path": list(access_log_prev[site_id].keys()),
                },
            )
            num += len(access_log_prev[site_id])
        content_db.log.debug(
            "Inserted %s web request stat in %.3fs" % (num, time.time() - s)
        )
...
def processRequestLog():
    ...
    if request_log:
        ...
        request_log = collections.defaultdict(
        ...
        for site_id in request_log_prev:
            for inner_path, uploaded in request_log_prev[site_id].items():
                ...
                num += 1
        content_db.log.debug(
            "Inserted %s file request stat in %.3fs" % (num, time.time() - s)
        )
...
class ContentManagerPlugin(object):
    ...
    def optionalDownloaded(self, inner_path, hash_id, size=None, own=False):
        ...
        self.contents.db.executeDelayed(
            (
                "UPDATE file_optional SET time_downloaded = :now,"
                " is_downloaded = 1, peer = peer + 1 WHERE site_id = :site_id"
                " AND inner_path = :inner_path AND is_downloaded = 0"
            ),
            {
                "now": int(time.time()),
    ...
    def optionalRemoved(self, inner_path, hash_id, size=None):
        res = self.contents.db.execute(
            (
                "UPDATE file_optional SET is_downloaded = 0, is_pinned = 0,"
                " peer = peer - 1 WHERE site_id = :site_id AND inner_path ="
                " :inner_path AND is_downloaded = 1"
            ),
            {
                "site_id": self.contents.db.site_ids[self.site.address],
    ...
    def optionalRenamed(self, inner_path_old, inner_path_new):
        ...
        self.contents.db.execute(
            (
                "UPDATE file_optional SET inner_path = :inner_path_new WHERE"
                " site_id = :site_id AND inner_path = :inner_path_old"
            ),
            {
                "site_id": self.contents.db.site_ids[self.site.address],
    ...
    def isDownloaded(
        self, inner_path=None, hash_id=None, force_check_db=False
    ):
        ...
        if inner_path:
            res = self.contents.db.execute(
                (
                    "SELECT is_downloaded FROM file_optional WHERE site_id ="
                    " :site_id AND inner_path = :inner_path LIMIT 1"
                ),
                {
                    "site_id": self.contents.db.site_ids[self.site.address],
                    "inner_path": inner_path,
            ...
            res = self.contents.db.execute(
                (
                    "SELECT is_downloaded FROM file_optional WHERE site_id ="
                    " :site_id AND hash_id = :hash_id AND is_downloaded = 1"
                    " LIMIT 1"
                ),
                {
                    "site_id": self.contents.db.site_ids[self.site.address],
    ...
    def isPinned(self, inner_path):
        ...
        res = self.contents.db.execute(
            (
                "SELECT is_pinned FROM file_optional WHERE site_id = :site_id"
                " AND inner_path = :inner_path LIMIT 1"
            ),
            {
                "site_id": self.contents.db.site_ids[self.site.address],
                "inner_path": inner_path,
        ...
        self.site.log.debug(
            "Cache set is pinned: %s %s" % (inner_path, is_pinned)
        )
        return is_pinned
    ...
    def setPin(self, inner_path, is_pinned):
        ...
        content_db.execute(
            "UPDATE file_optional SET is_pinned = %d WHERE ?" % is_pinned,
            {"site_id": site_id, "inner_path": inner_path},
        )
        self.cache_is_pinned = {}
...
class UiRequestPlugin(object):
    def parsePath(self, path):
        ...
        if path_parts:
            ...
            if site_id:
                if ContentDbPlugin.content_db.isOptionalFile(
                    site_id, path_parts["inner_path"]
                ):
...
class FileRequestPlugin(object):
    ...
    def recordFileRequest(self, site_address, inner_path, stats):
        ...
        if site_id and ContentDbPlugin.content_db.isOptionalFile(
            site_id, inner_path
        ):
...
class SitePlugin(object):
    ...
    def fileForgot(self, inner_path):
        if "|" in inner_path and self.content_manager.isPinned(
            re.sub(r"\|.*", "", inner_path)
        ):
        ...
        if "|" in inner_path and self.bad_files.get(inner_path, 0) > 5:
            ...
            for key, val in self.bad_files.items():
                if key.startswith(inner_path_file) and val > 1:
                    ...
                    num_changed += 1
            self.log.debug(
                "Idle optional file piece done, changed retry number of %s"
                " pieces." % num_changed
            )
            if num_changed:
...
class ConfigPlugin(object):
    def createArguments(self):
        ...
        group.add_argument(
            "--optional_limit",
            help="Limit total size of optional files",
            default="10%",
            metavar="GB or free space %",
        )
        group.add_argument(
            "--optional_limit_exclude_minsize",
            help=(
                "Exclude files larger than this limit from optional size limit"
                " calculation"
            ),
            default=20,
            metavar="MB",
