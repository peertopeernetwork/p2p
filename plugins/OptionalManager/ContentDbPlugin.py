...
class ContentDbPlugin(object):
    def __init__(self, *args, **kwargs):
        ...
        self.timer_check_optional = helper.timer(
            60 * 5, self.checkOptionalLimit
        )
    ...
    def getSchema(self):
        ...
        schema["tables"]["file_optional"] = {
            ...
            "indexes": [
                (
                    "CREATE UNIQUE INDEX file_optional_key ON file_optional"
                    " (site_id, inner_path)"
                ),
    ...
    def loadFilesOptional(self):
        ...
        res = content_db.execute(
            "SELECT site_id, inner_path, size, is_downloaded FROM"
            " file_optional"
        )
        ...
        for row in res:
            ...
            if row["is_downloaded"]:
                site_sizes[row["site_id"]]["optional_downloaded"] += row[
                    "size"
                ]
        ...
        self.log.info(
            ...
            % (
                ...
                float(total_downloaded) / 1024 / 1024,
                time.time() - s,
            )
        )
        if (
            self.need_filling
            and 0 <= self.getOptionalLimitBytes() < total_downloaded
        ):
            limit_bytes = self.getOptionalLimitBytes()
            limit_new = round(
                (float(total_downloaded) / 1024 / 1024 / 1024) * 1.1, 2
            )
            self.log.info(
                "First startup after update and limit is smaller than"
                " downloaded files size (%.2fGB), increasing it from %.2fGB to"
                " %.2fGB"
                % (
                    float(total_downloaded) / 1024 / 1024 / 1024,
                    float(limit_bytes) / 1024 / 1024 / 1024,
                    limit_new,
                )
            )
    ...
    def fillTableFileOptional(self, site):
        ...
        cur = self.getCursor()
        res = cur.execute(
            "SELECT * FROM content WHERE size_files_optional > 0 AND site_id"
            " = %s" % site_id
        )
        num = 0
        for row in res.fetchall():
            ...
            try:
                num += self.setContentFilesOptional(
                    site, row["inner_path"], content, cur=cur
                )
            except Exception as err:
                self.log.error(
                    "Error loading %s into file_optional: %s"
                    % (row["inner_path"], err)
                )
        cur.close()
        ...
        res = self.execute(
            (
                "UPDATE file_optional SET is_pinned = 1 WHERE site_id ="
                " :site_id AND inner_path LIKE :inner_path"
            ),
            {"site_id": site_id, "inner_path": "%%/%s/%%" % auth_address},
        )
        self.log.debug(
            "Filled file_optional table for %s in %.3fs (loaded: %s,"
            " is_pinned: %s)"
            % (site.address, time.time() - s, num, res.rowcount)
        )
        self.filled[site.address] = True

    def setContentFilesOptional(
        self, site, content_inner_path, content, cur=None
    ):
        if not cur:
            cur = self
        num = 0
        ...
        for relative_inner_path, file in content.get(
            "files_optional", {}
        ).items():
            ...
            if hash_id in site.content_manager.hashfield:
                ...
                is_downloaded = 0
            if (
                site.address + "/" + content_inner_dir
                in self.my_optional_files
            ):
                is_pinned = 1
            else:
                is_pinned = 0
            cur.insertOrUpdate(
                "file_optional",
                {"hash_id": hash_id, "size": int(file["size"])},
                {"site_id": site_id, "inner_path": file_inner_path},
                oninsert={
                    "time_added": int(time.time()),
                    "time_downloaded": int(time.time())
                    if is_downloaded
                    else 0,
    ...
    def setContent(self, site, inner_path, content, size=0):
        ...
        if (not self.need_filling or self.filled.get(site.address)) and (
            "files_optional" in content or "files_optional" in old_content
        ):
            ...
            if old_content:
                ...
                if deleted:
                    site_id = self.site_ids[site.address]
                    self.execute(
                        "DELETE FROM file_optional WHERE ?",
    ...
    def deleteContent(self, site, inner_path):
        ...
        if content and "files_optional" in content:
            ...
            optional_inner_paths = [
                ...
                for relative_inner_path in content.get(
                    "files_optional", {}
                ).keys()
            ]
            self.execute(
    ...
    def updatePeerNumbers(self):
        ...
        for site in list(self.sites.values()):
            ...
            has_updated_hashfield = next(
                (
                    ...
                    and peer.hashfield.time_changed
                    > self.time_peer_numbers_updated
                ),
                None,
            )
            if (
                not has_updated_hashfield
                and site.content_manager.hashfield.time_changed
                < self.time_peer_numbers_updated
            ):
                continue
            ...
            peer_nums = collections.Counter(
                itertools.chain(
                    hashfield_peers, site.content_manager.hashfield
                )
            )
            site_id = self.site_ids[site.address]
            if not site_id:
                continue
            res = self.execute(
                "SELECT file_id, hash_id, peer FROM file_optional WHERE ?",
                {"site_id": site_id},
            )
            updates = {}
            for row in res:
            ...
            for file_id, peer_num in updates.items():
                self.execute(
                    "UPDATE file_optional SET peer = ? WHERE file_id = ?",
                    (peer_num, file_id),
                )
        ...
        self.log.debug(
            "%s/%s peer number for %s site updated in %.3fs"
    ...
    def getOptionalLimitBytes(self):
        if config.optional_limit.endswith("%"):
            ...
            limit_bytes = (
                float(re.sub("[^0-9.]", "", config.optional_limit))
                * 1024
                * 1024
                * 1024
            )
    ...
    def getOptionalUsedBytes(self):
        size = self.execute(
            "SELECT SUM(size) FROM file_optional WHERE %s"
            % self.getOptionalUsedWhere()
        ).fetchone()[0]
        if not size:
            size = 0
    ...
    def checkOptionalLimit(self, limit=None):
        ...
        for row in self.queryDeletableFiles():
            ...
            try:
                ...
                site.log.error(
                    "Error deleting %s: %s" % (row["inner_path"], err)
                )
            if need_delete <= 0:
                break
        cur = self.getCursor()
        for file_id in deleted_file_ids:
            cur.execute(
                (
                    "UPDATE file_optional SET is_downloaded = 0, is_pinned ="
                    " 0, peer = peer - 1 WHERE ?"
                ),
                {"file_id": file_id},
            )
...
class SiteManagerPlugin(object):
    def load(self, *args, **kwargs):
        ...
        if (
            self.sites
            and not content_db.optional_files_loaded
            and content_db.conn
        ):
