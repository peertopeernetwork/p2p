...
class UiWebsocketPlugin(object):
    def formatSiteInfo(self, site, create_user=True):
        ...
        feed_following = self.user.sites.get(site.address, {}).get(
            "follow", None
        )
        if feed_following is None:
        ...
        for address, site_data in list(self.user.sites.items()):
            ...
            for name, query_set in feeds.items():
                ...
                for row in res:
                    row = dict(row)
                    if not isinstance(
                        row["date_added"], (int, float, complex)
                    ):
                        self.log.debug(
                    ...
                    if (
                        "date_added" not in row
                        or row["date_added"] > time.time() + 120
                    ):
                        self.log.debug(
