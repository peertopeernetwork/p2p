...
class ChartCollector(object):
    ...
    def setInitialLastValues(self, sites):
        for site in sites:
            self.last_values["site:" + site.address][
                "site_bytes_recv"
            ] = site.settings.get("bytes_recv", 0)
            self.last_values["site:" + site.address][
                "site_bytes_sent"
            ] = site.settings.get("bytes_sent", 0)
    ...
    def getCollectors(self):
        ...
        collectors["connection_in"] = lambda: len(
            [
                1
                for connection in file_server.connections
                if connection.type == "in"
            ]
        )
        collectors["connection_onion"] = lambda: len(
            [
                1
                for connection in file_server.connections
        ...
        collectors["connection_ping_avg"] = lambda: round(
            1000
            * helper.avg(
                [
                    connection.last_ping_delay
                    for connection in file_server.connections
                    if connection.last_ping_delay
                ]
            )
        )
        collectors["connection_ping_min"] = lambda: round(
            1000
            * min(
                [
                    connection.last_ping_delay
        ...
        collectors["connection_rev_avg"] = lambda: helper.avg(
            [
                connection.handshake["rev"]
                for connection in file_server.connections
        ...
        collectors[
            "optional_limit"
        ] = lambda: content_db.getOptionalLimitBytes()
        collectors["optional_used"] = lambda: content_db.getOptionalUsedBytes()
        collectors["optional_downloaded"] = lambda: sum(
            [
                site.settings.get("optional_downloaded", 0)
                for site in sites.values()
            ]
        )
        collectors["peer"] = lambda peers: len(peers)
        collectors["peer_onion"] = lambda peers: len(
            [True for peer in peers if ".onion" in peer]
        )
        collectors["size"] = lambda: sum(
            [site.settings.get("size", 0) for site in sites.values()]
        )
        collectors["size_optional"] = lambda: sum(
            [site.settings.get("size_optional", 0) for site in sites.values()]
        )
        collectors["content"] = lambda: sum(
            [len(site.content_manager.contents) for site in sites.values()]
        )
        return collectors

    def getSiteCollectors(self):
        site_collectors = {}
        site_collectors["site_size"] = lambda site: site.settings.get(
            "size", 0
        )
        site_collectors["site_size_optional"] = lambda site: site.settings.get(
            "size_optional", 0
        )
        site_collectors[
            "site_optional_downloaded"
        ] = lambda site: site.settings.get("optional_downloaded", 0)
        site_collectors["site_content"] = lambda site: len(
            site.content_manager.contents
        )
        site_collectors[
            "site_bytes_recv|change"
        ] = lambda site: site.settings.get("bytes_recv", 0)
        site_collectors[
            "site_bytes_sent|change"
        ] = lambda site: site.settings.get("bytes_sent", 0)
        ...
        site_collectors["site_peer_connected"] = lambda site: len(
            [True for peer in site.peers.values() if peer.connection]
        )
    ...
    def collectGlobal(self, collectors, last_values):
        ...
        cur.executemany(
            "INSERT INTO data (type_id, value, date_added) VALUES (?, ?, ?)",
            values,
        )
        self.log.debug(
            "Global collectors inserted in %.3fs" % (time.time() - s)
        )
    ...
    def collectSites(self, sites, collectors, last_values):
        ...
        for address, site in list(sites.items()):
            site_datas = self.collectDatas(
                collectors, last_values["site:%s" % address], site
            )
            for key, value in site_datas.items():
                values.append(
                    (
                        self.db.getTypeId(key),
                        self.db.getSiteId(address),
                        value,
                        now,
                    )
                )
