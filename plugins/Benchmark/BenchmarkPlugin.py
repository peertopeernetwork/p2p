...
class UiRequestPlugin(object):
    ...
    def actionBenchmarkResult(self):
        ...
        s = time.time()
        for part in main.actions.testBenchmark(
            filter=self.get.get("filter", "")
        ):
            yield part
...
class ActionsPlugin:
    ...
    def getBenchmarkTests(self, online=False):
        ...
        tests.extend(
            [
                {
                    "func": self.testHdPrivatekey,
                    "num": 50,
                    "time_standard": 0.57,
                },
                {"func": self.testSign, "num": 20, "time_standard": 0.46},
                {
                    "func": self.testVerify,
                    "kwargs": {"lib_verify": "sslcrypto_fallback"},
                    "num": 20,
                    "time_standard": 0.38,
                },
                {
                    "func": self.testVerify,
                    "kwargs": {"lib_verify": "sslcrypto"},
                    "num": 200,
                    "time_standard": 0.30,
                },
                {
                    "func": self.testVerify,
                    "kwargs": {"lib_verify": "libsecp256k1"},
                    "num": 200,
                    "time_standard": 0.10,
                },
                {
                    "func": self.testPackMsgpack,
                    "num": 100,
                    "time_standard": 0.35,
                },
                {
                    "func": self.testUnpackMsgpackStreaming,
                ...
                {
                    "func": self.testPackArchive,
                    "kwargs": {"archive_type": "gz"},
                    "num": 5,
                    "time_standard": 0.08,
                },
                {
                    "func": self.testPackArchive,
                    "kwargs": {"archive_type": "bz2"},
                    "num": 5,
                    "time_standard": 0.68,
                },
                {
                    "func": self.testPackArchive,
                    "kwargs": {"archive_type": "xz"},
                    "num": 5,
                    "time_standard": 0.47,
                },
                {"func": self.testUnpackZip, "num": 20, "time_standard": 0.25},
                {
                    "func": self.testUnpackArchive,
                    "kwargs": {"archive_type": "gz"},
                    "num": 20,
                    "time_standard": 0.28,
                },
                ...
                {
                    "func": self.testCryptHash,
                    "kwargs": {"hash_type": "sha256"},
                    "num": 10,
                    "time_standard": 0.50,
                },
                {
                    "func": self.testCryptHash,
                    "kwargs": {"hash_type": "sha512"},
                    "num": 10,
                    "time_standard": 0.33,
                },
                {
                    "func": self.testCryptHashlib,
                    "kwargs": {"hash_type": "sha3_256"},
                    "num": 10,
                    "time_standard": 0.33,
                },
                {
                    "func": self.testCryptHashlib,
                    "kwargs": {"hash_type": "sha3_512"},
                    "num": 10,
                    "time_standard": 0.65,
                },
    ...
    def testBenchmark(
        self, num_multipler=1, online=False, num_run=None, filter=None
    ):
        """
        ...
        if filter:
            tests = [
                test
                for test in tests[:]
                if filter.lower() in test["func"].__name__.lower()
            ]
        yield "\n"
        res = {}
        res_time_taken = {}
        ...
        for test in tests:
            ...
            try:
                for progress in func(num_run_test, **kwargs):
                    ...
                    if num_run_test > 10:
                        should_print = (
                            i % (num_run_test / 10) == 0 or progress != "."
                        )
                    else:
                        should_print = True
                    if should_print:
        ...
        if "testVerify {'lib_verify': 'sslcrypto'}" in res_time_taken:
            ...
            for lib_verify in speed_order:
                time_taken[lib_verify] = res_time_taken[
                    "testVerify {'lib_verify': '%s'}" % lib_verify
                ]
            time_taken["sslcrypto_fallback"] *= 10
            speedup_sslcrypto = (
                time_taken["sslcrypto_fallback"] / time_taken["sslcrypto"]
            )
            speedup_libsecp256k1 = (
                time_taken["sslcrypto_fallback"] / time_taken["libsecp256k1"]
            )
            yield "\n* Verification speedup:\n"
            yield " - OpenSSL: %.1fx (reference: 7.0x)\n" % speedup_sslcrypto
            yield " - libsecp256k1: %.1fx (reference: 23.8x)\n" % speedup_libsecp256k1
            if speedup_sslcrypto < 2:
                res["Verification speed"] = (
                    "error: OpenSSL speedup low: %.1fx" % speedup_sslcrypto
                )
            if speedup_libsecp256k1 < speedup_sslcrypto:
                res["Verification speed"] = (
                    "error: libsecp256k1 speedup low: %.1fx"
                    % speedup_libsecp256k1
                )
        if not res:
    ...
    def testCryptHash(self, num_run=1, hash_type="sha256"):
        ...
        hash_types = {
            "sha256": {
                "func": CryptHash.sha256sum,
                "hash_valid": "8cd629d9d6aff6590da8b80782a5046d2673d5917b99d5603c3dcb4005c45ffa",
            },
            "sha512": {
                "func": CryptHash.sha512sum,
                "hash_valid": "9ca7e855d430964d5b55b114e95c6bbb114a6d478f6485df93044d87b108904d",
            },
        }
    ...
    def testCryptHashlib(self, num_run=1, hash_type="sha3_256"):
        ...
        hash_types = {
            "sha3_256": {
                "func": hashlib.sha3_256,
                "hash_valid": "c8aeb3ef9fe5d6404871c0d2a4410a4d4e23268e06735648c9596f436c495f7e",
            },
            "sha3_512": {
                "func": hashlib.sha3_512,
                "hash_valid": "b75dba9472d8af3cc945ce49073f3f8214d7ac12086c0453fb08944823dee1ae83b3ffbc87a53a57cc454521d6a26fe73ff0f3be38dddf3f7de5d7692ebc7f95",
            },
        }
        hash_func = hash_types[hash_type]["func"]
        hash_valid = hash_types[hash_type]["hash_valid"]
    ...
    def testVerify(self, num_run=1, lib_verify="sslcrypto"):
        ...
        for i in range(num_run):
            ok = CryptBitcoin.verify(
                data, address, sign, lib_verify=lib_verify
            )
            yield "."
    ...
    def testPortCheckers(self):
        ...
        for (
            ip_type,
            func_names,
        ) in PeerPortchecker.PeerPortchecker.checker_functions.items():
            yield "\n- %s:" % ip_type
    ...
    def testAll(self):
        """
        Run all tests to check system compatibility with functions of the Peer-to-Peer Network
        """
        for progress in self.testBenchmark(
            online=not config.offline, num_run=1
        ):
            yield progress
    ...
    def createArguments(self):
        ...
        if self.getCmdlineValue("test") == "benchmark":
            self.test_parser.add_argument(
                "--num_multipler",
                help="Benchmark run time multipler",
                default=1.0,
                type=float,
                metavar="num",
            )
            ...
            self.test_parser.add_argument(
                "--func_name",
                help="Name of open port checker function",
                default=None,
                metavar="func_name",
            )
        return back
