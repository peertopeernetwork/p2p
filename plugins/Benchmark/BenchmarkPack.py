...
class ActionsPlugin:
    def createZipFile(self, path):
        ...
        file_name = b"\xc3\x81rv\xc3\xadzt\xc5\xb1r\xc5\x91%s.txt".decode(
            "utf_8"
        )
        with zipfile.ZipFile(path, "w") as archive:
            for y in range(100):
                zip_info = zipfile.ZipInfo(
                    file_name % y, (1980, 1, 1, 0, 0, 0)
                )
    ...
    def testPackZip(self, num_run=1):
        ...
        valid = (
            "cb32fb43783a1c06a2170a6bc5bb228a032b67ff7a1fd7a5efb9b467b400f553"
        )
    ...
    def testUnpackZip(self, num_run=1):
        ...
        zip_path = "%s/test.zip" % config.data_dir
        test_data = b"Test" * 1024
        file_name = b"\xc3\x81rv\xc3\xadzt\xc5\xb1r\xc5\x91".decode("utf_8")
        self.createZipFile(zip_path)
        for i in range(num_run):
            with zipfile.ZipFile(zip_path) as archive:
                for f in archive.filelist:
                    assert f.filename.startswith(
                        file_name
                    ), "Invalid filename: %s != %s" % (f.filename, file_name)
                    data = archive.open(f.filename).read()
                    assert archive.open(f.filename).read() == test_data, (
                        "Invalid data: %s…" % data[0:30]
                    )
            yield "."
        os.unlink(zip_path)
    ...
    def testUnpackArchive(self, num_run=1, archive_type="gz"):
        ...
        for i in range(num_run):
            with tarfile.open(archive_path, "r:%s" % archive_type) as archive:
                for y in range(100):
                    assert (
                        archive.extractfile(file_name % y).read() == test_data
                    )
            yield "."
        if os.path.isfile(archive_path):
            os.unlink(archive_path)
    ...
    def testPackMsgpack(self, num_run=1):
        ...
        data_packed_valid = (
            ...
            b" \xbbv\xa5float\xcb@\xc8\x1c\xd6\xe61\xf8\xa1\xa3int\xce@\x00\x00\x00\xa4text\xda\x14\x00"
        )
        ...
        for y in range(num_run):
            ...
            yield "."
        assert data_packed == data_packed_valid, "%s<br>!=<br>%s" % (
            repr(data_packed),
            repr(data_packed_valid),
        )
