    ...
    def getBenchmarkTests(self, online=False):
        ...
        tests.extend(
            [
                ...
                {
                    "func": self.testDbInsertMultiuser,
                    "num": 1,
                    "time_standard": 0.57,
                },
                {
                    "func": self.testDbQueryIndexed,
                    "num": 1000,
                    "time_standard": 0.84,
                },
                {
        ...
        schema = {
            ...
            "tables": {
                "test": {
                    ...
                    "indexes": [
                        "CREATE UNIQUE INDEX test_key ON test(test_id,"
                        " json_id)"
                    ],
            ...
            with self.getTestDb() as db:
                ...
                for i in range(1000):
                    data["test"].append(
                        {
                            "test_id": i,
                            "title": "Testdata for %s message %s" % (u, i),
                        }
                    )
                json.dump(
                    data, open("%s/test_%s.json" % (config.data_dir, u), "w")
                )
                db.updateJson("%s/test_%s.json" % (config.data_dir, u))
                os.unlink("%s/test_%s.json" % (config.data_dir, u))
                assert (
                    db.execute("SELECT COUNT(*) FROM test").fetchone()[0]
                    == 1000
                )
            yield "."
        ...
        for u in range(num_run):
            with self.getTestDb() as db:
                for progress in self.fillTestDb(db):
                    yield progress
                num_rows = db.execute("SELECT COUNT(*) FROM test").fetchone()[
                    0
                ]
