...
class UiRequestPlugin(object):
    ...
    def renderHead(self):
        ...
        try:
            ...
            yield (
                "Calc size <a href='?size=1'>on</a> <a href='?size=0'>off</a>"
            )
        except Exception:
            pass
        yield "<br>"
    ...
    def renderConnectionsTable(self):
        ...
        yield "<b>Connections</b> (%s, total made: %s, in: %s, out: %s):<br>" % (
            ...
            main.file_server.last_connection_id,
            main.file_server.num_incoming,
            main.file_server.num_outgoing,
        )
        yield (
            "<table class='connections'><tr> <th>id</th> <th>type</th>"
            " <th>ip</th> <th>open</th> <th>crypt</th> <th>ping</th>"
        )
        yield (
            "<th>buff</th> <th>bad</th> <th>idle</th> <th>open</th>"
            " <th>delay</th> <th>cpu</th> <th>out</th> <th>in</th> <th>last"
            " sent</th>"
        )
        yield (
            "<th>wait</th> <th>version</th> <th>time</th> <th>sites</th> </tr>"
        )
            ...
            yield self.formatTableRow(
                [
                    ...
                    (
                        "%s r%s",
                        (
                            connection.handshake.get("version"),
    ...
    def renderTrackers(self):
        ...
        yield (
            "<table class='trackers'><tr> <th>address</th> <th>request</th>"
            " <th>successive errors</th> <th>last_request</th></tr>"
        )
        from Site import (
            SiteAnnouncer,
        )
        ...
        for tracker_address, tracker_stat in sorted(
            SiteAnnouncer.global_stats.items()
        ):
            yield self.formatTableRow(
                [
                    ("%s", tracker_address),
                    ...
                    (
                        ...
                        min(
                            999,
                            (time.time() - tracker_stat["time_request"]) / 60,
                        ),
                    ),
                ]
            )
        ...
        if "AnnounceShare" in PluginManager.plugin_manager.plugin_names:
            ...
            yield (
                "<table class='trackers'><tr> <th>address</th> <th>added</th>"
                " <th>found</th> <th>latency</th> <th>successive errors</th>"
                " <th>last_success</th></tr>"
            )
            from AnnounceShare import AnnounceSharePlugin

            for tracker_address, tracker_stat in sorted(
                AnnounceSharePlugin.tracker_storage.getTrackers().items()
            ):
                yield self.formatTableRow(
                    [
                        ("%s", tracker_address),
                        (
                            ...
                            min(
                                999,
                                (time.time() - tracker_stat["time_added"])
                                / 60,
                            ),
                        ),
                        (
                        ...
                        (
                            ...
                            min(
                                999,
                                (time.time() - tracker_stat["time_success"])
                                / 60,
                            ),
                        ),
                    ]
    ...
    def renderTor(self):
        ...
        for site_address, onion in list(
            main.file_server.tor_manager.site_onions.items()
        ):
    ...
    def renderSites(self):
        ...
        yield "<table>"
        yield (
            "<tr><th>address</th> <th>connected</th> <th"
            " title='connected/good/total'>peers</th> <th>content.json</th>"
            " <th>out</th> <th>in</th>  </tr>"
        )
        for site in list(self.server.sites.values()):
            ...
            yield (
                "<tr><td id='peers_%s' style='display: none; white-space: pre'"
                " colspan=6>"
                % site.address
            )
            for key, peer in list(site.peers.items()):
                ...
                yield (
                    "(#%4s, rep: %2s, err: %s, found: %.1fs min, add: %.1f"
                    " day) %30s -<br>"
                    % (
                        connection_id,
                        peer.reputation,
    ...
    def renderRequests(self):
        ...
        for stat_key, stat in sorted(
            ...
            key=lambda i: i[1]["bytes"],
            reverse=True,
        ):
            yield (
                "<tr><td>%s</td><td style='white-space: nowrap'>x %s"
                " =</td><td>%.0fkB</td></tr>"
                % (
                    stat_key,
                    stat["num"],
    ...
    def renderMemory(self):
        ...
        yield (
            "<br><br><b>Objects in memory (types: %s, total: %s,"
            " %.2fkb):</b><br>"
            % (
                len(obj_count),
        ...
        for obj, stat in sorted(
            list(obj_count.items()), key=lambda x: x[1][0], reverse=True
        ):
        ...
        yield (
            "<br><br><b>Classes in memory (types: %s, total: %s,"
            " %.2fkb):</b><br>"
            % (
                len(class_count),
        ...
        for obj, stat in sorted(
            list(class_count.items()), key=lambda x: x[1][0], reverse=True
        ):
        ...
        objs = [
            (key, val) for key, val in sys.modules.items() if val is not None
        ]
        objs.sort()
        yield "<br>Modules (%s):<br>" % len(objs)
        for module_name, module in objs:
            yield " - %.3fkb: %s %s<br>" % (
                self.getObjSize(module, hpy),
                module_name,
                html.escape(repr(module)),
            )
    ...
    def actionStats(self):
        ...
        self.sendHeader()
        if (
            "Multiuser" in PluginManager.plugin_manager.plugin_names
            and not config.multiuser_local
        ):
    ...
    def actionDumpobj(self):
        ...
        for obj in objs:
            ...
            if (
                ...
                continue
            yield "%.1fkb %s… " % (
                float(sys.getsizeof(obj)) / 1024,
                html.escape(str(obj)),
            )
            for attr in dir(obj):
                yield "- %s: %s<br>" % (
                    attr,
                    html.escape(str(getattr(obj, attr))),
                )
            yield "<br>"
        gc.collect()
    ...
    def actionListobj(self):
        ...
        """
        yield "Listing all %s objects in memory…<br>" % html.escape(
            type_filter
        )
        ref_count = {}
        objs = gc.get_objects()
        for obj in objs:
            ...
            for ref in refs:
                ...
                if "object at" in str(ref) or len(str(ref)) > 100:
                    ...
                    yield str(ref.__class__.__name__) + ":" + html.escape(
                        str(ref)
                    )
                yield "] "
        ...
        yield "<br>Object referrer (total: %s, %.2fkb):<br>" % (
            len(ref_count),
            sum([stat[1] for stat in list(ref_count.values())]),
        )
        for obj, stat in sorted(
            list(ref_count.items()), key=lambda x: x[1][0], reverse=True
        )[0:30]:
            yield " - %.1fkb = %s x %s<br>" % (
                stat[1],
                stat[0],
...
class ActionsPlugin:
    ...
    def testEnv(self, format="text"):
        ...
        yield from self.formatTable(
            [
                "Peer-to-Peer Network version:",
                "%s rev%s" % (config.version, config.rev),
            ],
            ...
            [
                "Libsecp256k1:",
                "%s" % type(coincurve._libsecp256k1.lib).__name__,
            ],
            [
                "SQLite:",
                "%s, API: %s" % (sqlite3.sqlite_version, sqlite3.version),
            ],
            format=format,
        )
        ...
        for lib_name in [
            ...
            try:
                ...
                rows.append(
                    ["- %s:" % lib_name, version, "at " + module.__file__]
                )
