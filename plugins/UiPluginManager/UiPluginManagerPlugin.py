...
class UiRequestPlugin(object):
        ...
        return iter(
            [
                super(UiRequestPlugin, self).renderWrapper(
                    site,
                    path,
                    "uimedia/plugins/plugin_manager/plugin_manager.html",
                    "Plugin Manager",
                    extra_headers,
    ...
    def actionUiMedia(self, path, *args, **kwargs):
        if path.startswith("/uimedia/plugins/plugin_manager/"):
            ...
            if config.debug and (
                file_path.endswith("all.js") or file_path.endswith("all.css")
            ):
            ...
            if file_path.endswith("js"):
                data = _.translateData(
                    open(file_path).read(), mode="js"
                ).encode("utf_8")
            elif file_path.endswith("html"):
                data = _.translateData(
                    open(file_path).read(), mode="html"
                ).encode("utf_8")
            else:
                data = open(file_path, "rb").read()
            return self.actionFile(
                file_path, file_obj=io.BytesIO(data), file_size=len(data)
            )
        else:
...
class UiWebsocketPlugin(object):
    ...
    def actionPluginList(self, to):
        plugins = []
        for plugin in PluginManager.plugin_manager.listPlugins(
            list_disabled=True
        ):
            ...
            if plugin["source"] != "builtin":
                ...
                if plugin_site:
                    try:
                        ...
                        plugin[
                            "site_title"
                        ] = plugin_site.content_manager.contents[
                            "content.json"
                        ].get(
                            "title"
                        )
                        ...
                        plugin["updated"] = (
                            plugin_key
                            in PluginManager.plugin_manager.plugins_updated
                        )
                    except Exception:
                        pass
    ...
    def actionPluginConfigSet(self, to, source, inner_path, key, value):
        ...
        plugin = None
        for item in plugins:
            if item["source"] == source and item["inner_path"] in (
                inner_path,
                "disabled-" + inner_path,
            ):
                plugin = item
    ...
    def doPluginAdd(self, to, inner_path, res):
        ...
        self.cmd(
            ...
            [
                (
                    "Plugin installed!<br>You have to restart the client to"
                    " load the plugin"
                ),
                "Restart",
            ],
    ...
    def actionPluginAddRequest(self, to, inner_path):
        ...
        warning = (
            "<b>Warning!<br/>Plugins has the same permissions as the"
            " Peer-to-Peer Network client.<br/>"
        )
    ...
    def actionPluginUpdate(self, to, address, inner_path):
        ...
        PluginManager.plugin_manager.saveConfig()
        PluginManager.plugin_manager.plugins_updated[
            "%s/%s" % (address, inner_path)
        ] = True
        return "ok"
