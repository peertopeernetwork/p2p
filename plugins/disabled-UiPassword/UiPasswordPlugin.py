...
def showPasswordAdvice(password):
    ...
    if not password or not isinstance(password, str):
        error_msgs.append(
            "You have enabled <b>UiPassword</b> plugin, but you forgot to set"
            " a password!"
        )
    elif len(password) < 8:
...
class UiRequestPlugin(object):
        ...
        if path.endswith("favicon.ico"):
            ...
            if config.ui_password:
                ...
                if (
                    ...
                    and self.getClientId() not in self.whitelisted_client_ids
                ):
    ...
    def actionLogin(self):
        ...
        if posted:
            if self.sessionCheckPassword(posted.get("password")):
                ...
                cookie_header = (
                    "Set-Cookie",
                    "session_id=%s;path=/;max-age=2592000;" % session_id,
                )
                self.start_response(
    ...
    def sessionDelete(self, session_id):
        ...
        for client_id in list(self.whitelisted_client_ids):
            if (
                self.whitelisted_client_ids[client_id]["session_id"]
                == session_id
            ):
    ...
    def sessionCleanup(self):
        ...
        for session_id, session in list(self.sessions.items()):
            if (
                session["keep"]
                and time.time() - session["added"] > 60 * 60 * 24 * 60
            ):
                self.sessionDelete(session_id)
            elif (
                not session["keep"]
                and time.time() - session["added"] > 60 * 60 * 24
            ):
                self.sessionDelete(session_id)
    ...
    def actionLogout(self):
        session_id = self.getCookies().get("session_id")
        if not self.env.get("HTTP_REFERER") or session_id == self.get.get(
            "session_id"
        ):
            ...
            self.start_response(
                ...
                [
                    ...
                    (
                        "Set-Cookie",
                        (
                            "session_id=deleted; path=/; expires=Thu, 01 Jan"
                            " 1970 00:00:00 GMT"
                        ),
                    ),
                ],
...
class ConfigPlugin(object):
    def createArguments(self):
        ...
        group.add_argument(
            "--ui_password",
            help="Password to access UiServer",
            default=None,
            metavar="password",
        )
