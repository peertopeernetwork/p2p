...
class TrackerP2PDb(Db.Db):
    def __init__(self):
        self.version = 7
        self.hash_ids = {}
        super(TrackerP2PDb, self).__init__(
            {"db_name": "TrackerP2P"}, "%s/tracker-p2p.db" % config.data_dir
        )
    ...
    def peerAnnounce(
        ...
        res = self.execute(
            "SELECT peer_id FROM peer WHERE ? LIMIT 1",
        ...
        if user_row:
            ...
            self.execute(
                "UPDATE peer SET date_announced = ? WHERE peer_id = ?",
                (now, peer_id),
            )
        else:
        ...
        if hash_ids_db != hashes_ids_announced:
            ...
            if ip_type != "onion" or onion_signed:
                for hash_id in hash_ids_added:
                    self.execute(
                        "INSERT INTO peer_to_hash ?",
                        {"peer_id": peer_id, "hash_id": hash_id},
                    )
                if hash_ids_removed and delete_missing_hashes:
                    self.execute(
                        "DELETE FROM peer_to_hash WHERE ?",
                        {
                            "peer_id": peer_id,
                            "hash_id": list(hash_ids_removed),
                        },
                    )
