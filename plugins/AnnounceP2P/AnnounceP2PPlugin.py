...
class SiteAnnouncerPlugin(object):
    def getTrackerHandler(self, protocol):
        if protocol == "p2p":
            return self.announceTrackerP2P
        else:
            return super(SiteAnnouncerPlugin, self).getTrackerHandler(protocol)

    def announceTrackerP2P(self, tracker_address, mode="start", num_want=10):
        ...
        if mode == "start" or mode == "more":
            ...
            full_announce = True
            if (
                time.time() - time_full_announced.get(tracker_address, 0)
                < 60 * 15
            ):
                return None
            ...
            sites = [
                site
                for site in SiteManager.site_manager.sites.values()
                if site.isServing()
            ]
        ...
        if "onion_sign_this" in res:
            ...
            for site in sites:
                ...
                publickey = (
                    self.site.connection_server.tor_manager.getPublickey(onion)
                )
                if publickey not in request["onion_signs"]:
                    sign = CryptTor.sign(
                        res["onion_sign_this"].encode("utf_8"),
        ...
        if full_announce:
            tracker_peer.remove()
        self.site.log.debug(
            "Tracker announce result: p2p://%s (sites: %s, new peers: %s, add:"
            " %s, mode: %s) in %.3fs"
            % (
                tracker_address,
                site_index,
