    ...
    def do_rescan(self):
        ...
        try:
            if re.match("^http(s)?://", url):
                ...
                response = req.read().decode("utf_8")
                req.close()
                req = None
            else:
                response = open(url, "r").read().decode("utf_8")
        ...
        if "tracker_storage" not in locals():
            try:
                if "TrackerShare" in PluginManager.plugin_manager.plugin_names:
                    ...
                    "AnnounceShare"
                    in PluginManager.plugin_manager.plugin_names
                ):
                    from AnnounceShare.AnnounceSharePlugin import (
                        tracker_storage,
                    )
...
class SiteAnnouncerPlugin(object):
    def announceTracker(self, tracker, *args, **kwargs):
        tracker_list.reload()
        return super(SiteAnnouncerPlugin, self).announceTracker(
            tracker, *args, **kwargs
        )
...
class ConfigPlugin(object):
    def createArguments(self):
        ...
        group.add_argument(
            "--tracker_list_url",
            help=(
                "URL of local file path, where the list of additional trackers"
                " is located"
            ),
            default="https://raw.githubusercontent.com/ngosang/trackerslist/master/trackers_all_ip.txt",
            metavar="url",
        )
        group.add_argument(
            "--tracker_list_rescan_interval",
            help=(
                "Interval in seconds between rescans of the list of additional"
                " trackers"
            ),
            default=60 * 60,
            type=int,
