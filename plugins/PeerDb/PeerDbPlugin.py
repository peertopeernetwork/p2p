    ...
    def iteratePeers(self, site):
        ...
        for key, peer in list(site.peers.items()):
            ...
            yield (
                ...
                int(peer.time_added),
                int(peer.time_found),
                int(peer.time_response),
                int(peer.connection_error),
            )

    def savePeers(self, site, spawn=False):
        if spawn:
            site.greenlet_manager.spawnLater(
                60 * 60 + random.randint(0, 60),
                self.savePeers,
                site,
                spawn=True,
            )
        ...
        try:
            cur.execute(
                "DELETE FROM peer WHERE site_id = :site_id",
                {"site_id": site_id},
            )
            cur.executemany(
                (
                    "INSERT INTO peer (site_id, address, port, hashfield,"
                    " reputation, time_added, time_found, time_response,"
                    " connection_error) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
                ),
