...
# Modified for Peer-to-Peer Network
...
def GenerateDummyWindow(callback, uid):
    ...
    hwnd = ctypes.windll.user32.CreateWindowExW(
        0, newclass.lpszClassName, None, WS_POPUP, 0, 0, 0, 0, 0, 0, 0, 0
    )
    return hwnd
...
TIMERCALLBACK = ctypes.WINFUNCTYPE(
    None,
    ctypes.wintypes.HWND,
    ctypes.wintypes.UINT,
...
class NotificationIcon:
    def __init__(self, iconfilename, tooltip=None):
        assert os.path.isfile(str(iconfilename)), "{} doesn't exist".format(
            iconfilename
        )
    ...
    def _run(self):
        self.WM_TASKBARCREATED = ctypes.windll.user32.RegisterWindowMessageW(
            "TaskbarCreated"
        )
        ...
        iconinfo.uFlags = (
            ...
            | NIF_SHOWTIP
            | NIF_MESSAGE
            | (NIF_TIP if self._tooltip else 0)
        )
        ...
        while not self._die:
            ...
            time.sleep(0.125)
        print(
            "Icon thread stopped, removing icon (hicon: %s, hwnd: %s)…"
            % (self._hicon, self._hwnd)
        )

        Shell_NotifyIcon(
            NIM_DELETE,
            ctypes.cast(
                ctypes.pointer(iconinfo), ctypes.POINTER(NOTIFYICONDATA)
            ),
        )
    ...
    def _callback(self, hWnd, msg, wParam, lParam):
        ...
        if msg == WM_TIMER:
            if not any(
                thread.getName() == "MainThread" and thread.isAlive()
