...
class SiteManagerPlugin(object):
    ...
    def isAddressBlocked(self, address):
        ...
        if filter_storage.isSiteblocked(
            address
        ) or filter_storage.isSiteblocked(address_hashed):
            return True
        return super(SiteManagerPlugin, self).isAddressBlocked(address)

    def add(self, address, *args, **kwargs):
        should_ignore_block = kwargs.get("ignore_block") or kwargs.get(
            "settings"
        )
        if should_ignore_block:
            ...
            if filter_storage.isSiteblocked(address_hashed):
                block_details = filter_storage.getSiteblockDetails(
                    address_hashed
                )
            else:
                block_details = None
        if block_details:
            raise Exception(
                "Site blocked: %s"
                % html.escape(block_details.get("reason", "unknown reason"))
            )
        else:
...
class UiWebsocketPlugin(object):
    ...
    def actionMuteAdd(self, to, auth_address, cert_user_id, reason):
        if "ADMIN" in self.getPermissions(to):
            ...
            self.cmd(
                "confirm",
                [
                    _["Hide all content from <b>%s</b>?"]
                    % html.escape(cert_user_id),
                    _["Mute"],
                ],
                lambda res: self.cbMuteAdd(
                    to, auth_address, cert_user_id, reason
                ),
            )
    ...
    def actionSiteblockIgnoreAddSite(self, to, site_address):
        if site_address in filter_storage.site_manager.sites:
            ...
            if filter_storage.site_manager.need(
                site_address, ignore_block=True
            ):
                return "ok"
            else:
    ...
    def actionSiteblockAdd(self, to, site_address, reason=None):
        filter_storage.file_content["siteblocks"][site_address] = {
            "date_added": time.time(),
            "reason": reason,
        }
        filter_storage.save()
    ...
    def actionSiteblockGet(self, to, site_address):
        if filter_storage.isSiteblocked(site_address):
            ...
            site_address_hashed = filter_storage.getSiteAddressHashed(
                site_address
            )
    ...
    @flag.no_multiuser
    def actionFilterIncludeAdd(
        self, to, inner_path, description=None, address=None
    ):
        if address:
            if "ADMIN" not in self.getPermissions(to):
                return self.response(
                    ...
                    {
                        "error": (
                            "Forbidden: Only ADMIN sites can manage different"
                            " site include"
                        )
                    },
                )
        ...
        if "ADMIN" in self.getPermissions(to):
            ...
            title = _[
                "New shared global content filter: <b>%s</b> (%s sites, %s"
                " users)"
            ] % (
                html.escape(inner_path),
                len(content.get("siteblocks", {})),
                len(content.get("mutes", {})),
            )
            self.cmd(
                "confirm",
                [title, "Add"],
                lambda res: self.cbFilterIncludeAdd(
                    to, res, address, inner_path, description
                ),
            )
    ...
    def actionFilterIncludeList(self, to, all_sites=False, filters=False):
        if all_sites and "ADMIN" not in self.getPermissions(to):
            return self.response(
                ...
                {
                    "error": (
                        "Forbidden: Only ADMIN sites can list all sites"
                        " includes"
                    )
                },
            )
        ...
        for include in includes:
            ...
            if filters:
                ...
                include_site = filter_storage.site_manager.get(
                    include["address"]
                )
                if not include_site:
                    continue
        ...
        if file is not False:
            ...
            for auth_address in matches:
                if filter_storage.isMuted(auth_address):
                    self.log.debug(
                        "Mute match: %s, ignoring %s"
                        % (auth_address, inner_path)
                    )
                    return False
        return super(SiteStoragePlugin, self).updateDbFile(
            inner_path, file=file, cur=cur
        )
...
class UiRequestPlugin(object):
    def actionWrapper(self, path, extra_headers=None):
        ...
        if self.server.site_manager.get(address):
            return super(UiRequestPlugin, self).actionWrapper(
                path, extra_headers
            )
        ...
        if filter_storage.isSiteblocked(
            ...
            return iter(
                [
                    super(UiRequestPlugin, self).renderWrapper(
                        site,
                        path,
                        "uimedia/plugins/contentfilter/blocklisted.html?address="
                        + address,
                        "Blacklisted site",
                        extra_headers,
    ...
    def actionUiMedia(self, path, *args, **kwargs):
        if path.startswith("/uimedia/plugins/contentfilter/"):
            file_path = path.replace(
                "/uimedia/plugins/contentfilter/", plugin_dir + "/media/"
            )
