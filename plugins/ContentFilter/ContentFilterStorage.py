...
class ContentFilterStorage(object):
    def __init__(self, site_manager):
        ...
        if "site_blacklist" in self.file_content:
            self.file_content["siteblocks"] = self.file_content[
                "site_blacklist"
            ]
    ...
    def includeUpdateAll(self, update_site_dbs=True):
        ...
        self.log.debug(
            ...
            % (
                num_mutes,
                num_siteblocks,
                len(self.file_content["includes"]),
                time.time() - s,
            )
        )
    ...
    def isMuted(self, auth_address):
        if (
            auth_address in self.file_content["mutes"]
            or auth_address in self.include_filters["mutes"]
        ):
            return True
        else:
            return False
    ...
    def isSiteblocked(self, address):
        if (
            address in self.file_content["siteblocks"]
            or address in self.include_filters["siteblocks"]
        ):
            return True
        return False
    ...
    def changeDbs(self, auth_address, action):
        ...
        res = list(self.site_manager.list().values())[
            0
        ].content_manager.contents.db.execute(
            (
                "SELECT * FROM content LEFT JOIN site USING (site_id) WHERE"
                " inner_path LIKE :inner_path"
            ),
