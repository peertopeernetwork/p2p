        ...
        if ".zip/" in path or ".tar.gz/" in path:
            ...
            try:
                ...
                self.sendHeader(
                    ...
                    noscript=kwargs.get("header_noscript", False),
                    allow_ajax=header_allow_ajax,
                )
                return self.streamFile(file)
            except Exception as err:
                self.log.debug(
                    "Error opening archive file: %s"
...
class SiteStoragePlugin(object):
    ...
    def openArchive(self, inner_path):
        ...
        try:
            ...
            raise Exception(
                "Unable to download file: %s" % Debug.formatException(err)
            )
        return archive
    ...
    def walk(self, inner_path, *args, **kwags):
        if ".zip" in inner_path or ".tar.gz" in inner_path:
            ...
            return super(SiteStoragePlugin, self).walk(
                inner_path, *args, **kwags
            )
    ...
    def list(self, inner_path, *args, **kwags):
        if ".zip" in inner_path or ".tar.gz" in inner_path:
            ...
            return super(SiteStoragePlugin, self).list(
                inner_path, *args, **kwags
            )
    ...
    def read(self, inner_path, mode="rb", **kwargs):
        if ".zip/" in inner_path or ".tar.gz/" in inner_path:
            ...
            return super(SiteStoragePlugin, self).read(
                inner_path, mode, **kwargs
            )
