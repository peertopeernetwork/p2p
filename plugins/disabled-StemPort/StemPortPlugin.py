...
class TorManagerPlugin(object):
    def connectController(self):
        ...
        try:
            ...
            self.log.error(
                "Tor stem connect error: %s" % Debug.formatException(err)
            )
    ...
    def makeOnionAndKey(self):
        try:
            ...
            if service.private_key_type != "ED25519":
                raise Exception(
                    "Peer-to-Peer Network doesn't support crypto "
                    + service.private_key_type
                )
            self.log.debug(
                "Stem created %s.onion (async descriptor publication)"
                % service.service_id
            )
    ...
    def delOnion(self, address):
        try:
            ...
            self.log.error(
                "Stem failed to delete %s.onion: %s" % (address, err)
            )
            self.disconnect()
            return False
