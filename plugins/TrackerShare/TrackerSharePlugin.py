...
class TrackerStorage(object):
    ...
    def initTrackerLimitForProtocol(self):
        for s in re.split("[,;]", config.shared_trackers_limit_per_protocol):
            ...
            try:
                ...
                pass
        self.log.info(
            "Limits per protocol: %s" % self.shared_trackers_limit_per_protocol
        )
    ...
    def isTrackerAddressValid(self, tracker_address):
        ...
        if not address_parts:
            ...
            return False
        handler = self.site_announcer.getTrackerHandler(
            address_parts["protocol"]
        )
        if not handler:
            self.log.debug(
                "Invalid tracker address: Unknown protocol %s: %s"
                % (address_parts["protocol"], tracker_address)
            )
            return False
        return True
    ...
    def deleteUnusedTrackers(self, supported_trackers):
        ...
        for tracker_address, tracker in list(trackers.items()):
            t = max(
                trackers[tracker_address]["time_added"],
                trackers[tracker_address]["time_success"],
            )
            if (
                tracker_address not in supported_trackers
                and t < time.time() - self.tracker_down_time_interval
            ):
                self.log.info(
                    "Tracker %s seems to be disabled by the configuration,"
                    " removing." % tracker_address
                )
    ...
    def considerTrackerDeletion(self, tracker_address):
        ...
        if tracker["num_error"] > error_limit:
            if self.isTrackerDown(tracker_address):
                self.log.info(
                    "Tracker %s looks down, removing." % tracker_address
                )
                self.deleteTracker(tracker_address)
            elif self.areWayTooManyTrackers(tracker_address):
                self.log.info(
                    "Tracker %s has %d successive errors. Looks like we have"
                    " too many trackers, so removing."
                    % (tracker_address, tracker["num_error"])
                )
                self.deleteTracker(tracker_address)

    def areWayTooManyTrackers(self, tracker_address):
        protocol = self.getNormalizedTrackerProtocol(tracker_address) or ""
        nr_trackers_for_protocol = len(
            self.getTrackersPerProtocol().get(protocol, [])
        )
        nr_trackers = len(self.getTrackers())
        hard_limit_mult = 5
        hard_limit_for_protocol = (
            self.getTrackerLimitForProtocol(protocol) * hard_limit_mult
        )
        hard_limit = config.shared_trackers_limit * hard_limit_mult
        if (nr_trackers_for_protocol > hard_limit_for_protocol) and (
            nr_trackers > hard_limit
        ):
            return True
        return False
    ...
    def getSuccessiveErrorLimit(self, tracker_address):
        protocol = self.getNormalizedTrackerProtocol(tracker_address) or ""
        nr_working_trackers_for_protocol = len(
            self.getTrackersPerProtocol(working_only=True).get(protocol, [])
        )
        nr_working_trackers = len(self.getWorkingTrackers())
        error_limit = 30
        if (
            nr_working_trackers_for_protocol
            >= self.getTrackerLimitForProtocol(protocol)
        ):
            error_limit = 10
    ...
    def isTrackerWorking(self, tracker):
        ...
        if not tracker:
            return False
        if (
            tracker["time_success"]
            > time.time() - self.working_tracker_time_interval
        ):
            return True
        return False
    ...
    def getTrackersPerProtocol(self, working_only=False):
        ...
        for tracker_address in trackers:
            ...
            if not working_only or self.isTrackerWorking(tracker_address):
                trackers_per_protocol.setdefault(protocol, []).append(
                    tracker_address
                )
    ...
    def save(self):
        ...
        helper.atomicWrite(
            self.file_path,
            json.dumps(self.file_content, indent=2, sort_keys=True).encode(
                "utf_8"
            ),
        )
    ...
    def enoughWorkingTrackers(self):
        ...
        for protocol in supported_protocols:
            ...
            if len(trackers) < self.getTrackerLimitForProtocol(protocol):
                self.log.info(
                    "Not enough working trackers for protocol %s: %s < %s"
                    % (
                        protocol,
                        len(trackers),
                        self.getTrackerLimitForProtocol(protocol),
        ...
        if total_nr < config.shared_trackers_limit:
            self.log.info(
                "Not enough working trackers (total): %s < %s"
    ...
    def discoverTrackers(self, peers):
        ...
        for peer in peers:
            if (
                peer.connection
                and peer.connection.handshake.get("rev", 0) < 3560
            ):
                continue
            ...
            for tracker_address in res["trackers"]:
                if type(tracker_address) is bytes:
                    tracker_address = tracker_address.decode("utf_8")
        ...
        if num_success:
            self.save()
        self.log.info(
            "Discovered %s new trackers from %s/%s peers in %.3fs"
            % (
                num_trackers_discovered,
                num_success,
                len(peers),
                time.time() - s,
            )
...
class SiteAnnouncerPlugin(object):
    def getTrackers(self):
        ...
        tracker_storage.checkDiscoveringTrackers(
    ...
    def announceTracker(self, tracker, *args, **kwargs):
        tracker_storage.setSiteAnnouncer(self)
        res = super(SiteAnnouncerPlugin, self).announceTracker(
            tracker, *args, **kwargs
        )
        if res:
            latency = res
...
class ConfigPlugin(object):
    def createArguments(self):
        ...
        group.add_argument(
            "--shared_trackers_limit",
            help=(
                "Discover new shared trackers if this number of shared"
                " trackers isn't reached (total)"
            ),
            default=20,
            type=int,
            metavar="limit",
        ...
        group.add_argument(
            ...
            help=(
                "Discover new shared trackers if this number of shared"
                " trackers isn't reached per each supported protocol"
            ),
            default="p2p=10,other=5",
            metavar="limit",
        )
