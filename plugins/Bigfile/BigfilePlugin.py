...
class UiRequestPlugin(object):
    def isCorsAllowed(self, path):
        if path == "/Peer-to-Peer-Network-Internal/BigfileUpload":
            return True
        else:
    ...
    def actionBigfileUpload(self):
        ...
        result = self.handleBigfileUpload(
            upload_info, self.env["wsgi.input"].read
        )
    ...
    def handleBigfileUpload(self, upload_info, read):
        ...
        with site.storage.open(inner_path, "wb", create_dirs=True) as out_file:
            (
                ...
                piece_size,
                piecemap_info,
            ) = site.content_manager.hashBigfile(
    ...
    def actionFile(self, file_path, *args, **kwargs):
        ...
        return super(UiRequestPlugin, self).actionFile(
...
class UiWebsocketPlugin(object):
    def actionBigfileUploadInit(self, to, inner_path, size, protocol="xhr"):
        ...
        file_info = self.site.content_manager.getFileInfo(
            inner_path, new_file=True
        )
        content_inner_path_dir = helper.getDirname(
            file_info["content_inner_path"]
        ...
        if protocol == "xhr":
            return {
                "url": "/Peer-to-Peer-Network-Internal/BigfileUpload?upload_nonce="
                + nonce,
                "piece_size": piece_size,
                "inner_path": inner_path,
            ...
            return {
                "url": origin
                + "/Peer-to-Peer-Network-Internal/BigfileUploadWebsocket?upload_nonce="
                + nonce,
                "piece_size": piece_size,
                "inner_path": inner_path,
                "file_relative_path": file_relative_path,
            ...
            if file_info:
                ...
                if relative_path in content_json.get("files_optional", {}):
                    ...
                    try:
                        ...
                        self.log.error(
                            "File %s delete error: %s"
                            % (piecemap_inner_path, err)
                        )
...
class ContentManagerPlugin(object):
    ...
    def hashBigfile(
        self, read_func, size, piece_size=1024 * 1024, file_out=None
    ):
    ...
    def hashFile(self, dir_inner_path, file_relative_path, optional=False):
        ...
        if not hash or not piecemap_relative_path:
            ...
            if not piecemap_relative_path:
                ...
                piecemap_relative_path = (
                    file_relative_path + ".piecemap.msgpack"
                )
                piecemap_inner_path = inner_path + ".piecemap.msgpack"
    ...
    def optionalDownloaded(self, inner_path, hash_id, size=None, own=False):
        if "|" in inner_path:
            ...
            self.site.storage.piecefields[file_info["sha512"]][
                piece_i
            ] = b"\x01"
        ...
        if "piecefields" in self.site.settings.get("cache", {}):
            for sha512, piecefield_packed in (
                ...
                if piecefield_packed:
                    self.piecefields[sha512].unpack(
                        base64.b64decode(piecefield_packed)
                    )
        ...
        if os.path.isfile(file_path):
            if sha512 not in self.piecefields:
                if open(file_path, "rb").read(128) == b"\0" * 128:
                    ...
                    piece_data = b"\x01"
                self.log.debug(
                    "%s: File exists, but not in piecefield. Filling"
                    " piecefiled with %s * %s."
...
class BigFile(object):
    ...
    def read(self, buff=64 * 1024):
        with self.read_lock:
            ...
            if (
                self.read_bytes > 7 * 1024 * 1024
                and self.prebuffer < 5 * 1024 * 1024
            ):
                self.site.log.debug(
                    "%s: Increasing bigfile buffer size to 5MB…"
                    % self.inner_path
                )
...
class FileRequestPlugin(object):
        ...
        if file.read(10) == b"\0" * 10:
            ...
            if "piece_size" in file_info:
                ...
                if not site.storage.piecefields[file_info["sha512"]][piece_i]:
                    return False
        file.seek(pos)
        return super(FileRequestPlugin, self).isReadable(
            site, inner_path, file, pos
        )
    ...
    def actionGetPiecefields(self, params):
        ...
        if not site or not site.isServing():
            ...
            return False
        peer = site.addPeer(
            self.connection.ip, self.connection.port, return_peer=True
        )
...
class PeerPlugin(object):
    ...
    def updatePiecefields(self, force=False):
        ...
        try:
            ...
            self.log(
                "Invalid updatePiecefields response: %s"
    ...
    def getFile(self, site, inner_path, *args, **kwargs):
        ...
        return super(PeerPlugin, self).getFile(
            site, inner_path, *args, **kwargs
        )
...
class SitePlugin(object):
    def isFileDownloadAllowed(self, inner_path, file_info):
        if "piecemap" in file_info:
            ...
            if (
                config.bigfile_size_limit
                and file_size_mb > config.bigfile_size_limit
            ):
                self.log.debug(
        ...
        return super(SitePlugin, self).isFileDownloadAllowed(
            inner_path, file_info
        )
    ...
    def getSettingsCache(self):
        ...
        if self.storage.piecefields:
            back["piecefields"] = {
                sha512: base64.b64encode(piecefield.pack()).decode("utf_8")
    ...
    def needFile(self, inner_path, *args, **kwargs):
        if inner_path.endswith("|all"):
            ...
            if "piece_size" not in file_info:
                return super(SitePlugin, self).needFile(
                    inner_path, *args, **kwargs
                )
