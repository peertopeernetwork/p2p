...
class BroadcastServer(object):
    def __init__(self, service_name, listen_port=55565, listen_ip=""):
    ...
    def start(self):
        ...
        while self.running:
            ...
            try:
                ...
                self.log.error(
                    "HandleMessage error: %s" % Debug.formatException(err)
                )
    ...
    def broadcast(self, message, port=None):
        ...
        self.log.debug(
            "Broadcast using ips %s on port %s: %s"
            % (my_ips, port, message["cmd"])
        )
        for my_ip in my_ips:
            try:
                with closing(
                ...
                self.log.warning(
                    "Error sending broadcast using ip %s: %s" % (my_ip, err)
                )
    ...
    def handleMessage(self, addr, message):
        ...
        if (
            sender["service"] != "p2p"
            or sender["peer_id"] == self.sender_info["peer_id"]
        ):
            message = None
        elif func:
