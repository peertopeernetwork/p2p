...
class SiteAnnouncerPlugin(object):
    def announce(self, force=False, *args, **kwargs):
        ...
        thread = None
        if local_announcer and (
            force or time.time() - local_announcer.last_discover > 5 * 60
        ):
...
class LocalAnnouncer(BroadcastServer.BroadcastServer):
    def __init__(self, server, listen_port):
        super(LocalAnnouncer, self).__init__("p2p", listen_port=listen_port)
        self.server = server
    ...
    def discover(self, force=False):
        ...
        self.broadcast(
            {"cmd": "discoverRequest", "params": {}}, port=self.listen_port
        )
    ...
    def actionDiscoverRequest(self, sender, params):
        ...
        if sender["peer_id"] not in self.known_peers:
            ...
            self.log.debug(
                "Got discover request from unknown peer %s (%s), time to"
                " refresh known peers" % (sender["ip"], sender["peer_id"])
            )
    ...
    def actionDiscoverResponse(self, sender, params):
        ...
        if params["sites_changed"] != self.known_peers.get(
            sender["peer_id"], {}
        ).get("sites_changed"):
    ...
    def actionSiteListRequest(self, sender, params):
        ...
        for site_group in site_groups:
            res = {
                "sites_changed": self.server.site_manager.sites_changed,
                "sites": [site.address_hash for site in site_group],
            }
    ...
    def actionSiteListResponse(self, sender, params):
        ...
        self.known_peers[sender["peer_id"]]["sites_changed"] = params[
            "sites_changed"
        ]
        self.known_peers[sender["peer_id"]]["updated"] = time.time()
        self.known_peers[sender["peer_id"]]["found"] = time.time()
        self.log.debug(
            "Tracker result: Discover from %s response parsed in %.3fs, found:"
            " %s added: %s of %s"
            % (
                sender["ip"],
                time.time() - s,
...
class ConfigPlugin(object):
    def createArguments(self):
        ...
        group.add_argument(
            "--broadcast_port",
            help="UDP broadcasting port for local peer discovery",
            default=55565,
            type=int,
            metavar="port",
        )
