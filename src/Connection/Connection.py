...
class Connection(object):
    __slots__ = (
        ...
        "last_recv_time",
        "is_private_ip",
        "is_tracker_connection",
        "last_message_time",
        "last_send_time",
        "last_sent_time",
        "incomplete_buff_recv",
        "bytes_recv",
        "bytes_sent",
        "cpu_time",
        "send_lock",
    ...
    def createSocket(self):
        if self.server.getIpType(self.ip) == "ipv6" and not hasattr(
            socket, "socket_noproxy"
        ):
    ...
    def getValidSites(self):
        return [
            key
            for key, val in self.server.tor_manager.site_onions.items()
            if val == self.target_onion
        ]
    ...
    def _connect(self):
        ...
        if self.ip_type == "onion":
            self.sock = self.server.tor_manager.createSocket(
                self.ip, self.port
            )
        elif (
            config.trackers_proxy != "disable"
            and config.tor != "always"
            and self.is_tracker_connection
        ):
            if config.trackers_proxy == "tor":
                ...
                self.sock.set_proxy(
                    socks.PROXY_TYPE_SOCKS5, proxy_ip, int(proxy_port)
                )
        else:
        ...
        if self.shouldEncrypt():
            try:
                ...
                if self.mustEncrypt():
                    raise
                self.log(
                    "Crypt connection error, adding %s:%s as broken ssl. %s"
                    % (self.ip, self.port, Debug.formatException(err))
                )
        ...
        self.send(
            {
                "cmd": "handshake",
                "req_id": 0,
                "params": self.getHandshakeInfo(),
            }
        )
        self.server.outgoing_pool.spawn(self.messageLoop)
        connect_res = event_connected.get()
        if self.sock:
    ...
    def shouldEncrypt(self):
        ...
        return (
            ...
            and (self.ip not in self.server.broken_ssl_ips)
            and (self.ip not in config.ip_local)
            and ("tls-ed25519" in CryptConnection.manager.crypt_supported)
        )

    def wrapSocket(self, crypt="tls-ed25519", do_handshake=True):
        server = self.type == "in"
        sock = CryptConnection.manager.wrapSocket(
            self.sock, crypt, server=server, cert_pin=self.cert_pin
        )
        sock.do_handshake()
        self.crypt = crypt
            ...
            while not self.closed:
                ...
                while True:
                    ...
                    if "stream_bytes" in message:
                        ...
                        if config.debug_socket:
                            self.log(
                                "Start new unpacker with buff_left: %r"
                                % buff_left
                            )
                    else:
    ...
    def handleStream(self, message, buff):
        ...
        if unprocessed_bytes_num:
            unpacker_stream_bytes = min(
                unprocessed_bytes_num, stream_bytes_left
            )
            buff_stream_start = len(buff) - unprocessed_bytes_num
            file.write(
                buff[
                    buff_stream_start : buff_stream_start
        ...
        if config.debug_socket:
            self.log(
                "Starting stream %s: %s bytes (%s from unpacker, buff size:"
                " %s, unprocessed: %s)"
                % (
                    message["to"],
                    message["stream_bytes"],
                    unpacker_stream_bytes,
                    len(buff),
                    unprocessed_bytes_num,
                )
            )
        try:
            while 1:
        ...
        if config.debug_socket:
            self.log(
                "End stream %s, file pos: %s" % (message["to"], file.tell())
            )
        self.incomplete_buff_recv = 0
    ...
    def getHandshakeInfo(self):
        ...
        if (
            self.handshake
            and self.handshake.get("target_ip", "").endswith(".onion")
            and self.server.tor_manager.start_onions
        ):
            self.target_onion = self.handshake.get("target_ip").replace(
                ".onion", ""
            )
            if not self.server.tor_manager.site_onions.values():
                self.server.log.warning(
                    "Unknown target onion address: %s" % self.target_onion
                )
        handshake = {
    ...
    def setHandshake(self, handshake):
        ...
        if (
            handshake.get("peer_id") == self.server.peer_id
            and not handshake.get("tracker_connection")
            and not self.is_tracker_connection
        ):
            self.close("Same peer id, can't connect to myself")
            self.server.peer_blacklist.append(
                (handshake["target_ip"], handshake["fileserver_port"])
            )
            return False
        self.handshake = handshake
        if (
            handshake.get("port_opened", None) is False
            and "onion" not in handshake
            and not self.is_private_ip
        ):
            self.port = 0
        else:
            self.port = int(handshake["fileserver_port"])
        if handshake.get("use_bin_type") and self.unpacker:
            unprocessed_bytes_num = self.getUnpackerUnprocessedBytesNum()
            self.log(
                "Changing unpacker to bin type (unprocessed bytes: %s)"
                % unprocessed_bytes_num
            )
        ...
        if (
            handshake.get("crypt_supported")
            and self.ip not in self.server.broken_ssl_ips
        ):
            if type(handshake["crypt_supported"][0]) is bytes:
                handshake["crypt_supported"] = [
                    item.decode() for item in handshake["crypt_supported"]
                ]
            if self.ip_type == "onion" or self.ip in config.ip_local:
                crypt = None
                ...
                crypt = CryptConnection.manager.selectCrypt(
                    handshake["crypt_supported"]
                )
            if crypt:
                self.crypt = crypt
        if (
            self.type == "in"
            and handshake.get("onion")
            and not self.ip_type == "onion"
        ):
    ...
    def handleMessage(self, message):
        ...
        if cmd == "response":
            if message["to"] in self.waiting_requests:
                ...
                if config.debug_socket:
                    self.log(
                        "Handshake response: %s, ping: %s" % (message, ping)
                    )
                self.last_ping_delay = ping
                if message.get("crypt") and not self.sock_wrapped:
                    ...
                    server = self.type == "in"
                    self.log(
                        "Crypt out connection using: %s (server side: %s,"
                        " ping: %.3fs)…" % (crypt, server, ping)
                    )
                    self.wrapSocket(crypt)
                if not self.sock_wrapped and self.cert_pin:
                    self.close(
                        "Crypt connection error: Socket not encrypted, but"
                        " certificate pin present"
                    )
                    return
    ...
    def handleHandshake(self, message):
        ...
        if self.crypt and not self.sock_wrapped:
            server = self.type == "in"
            self.log(
                "Crypt in connection using: %s (server side: %s)…"
                % (self.crypt, server)
            )
            try:
                ...
                if not config.force_encryption:
                    self.log(
                        "Crypt connection error, adding %s:%s as broken"
                        " ssl. %s"
                        % (self.ip, self.port, Debug.formatException(err))
                    )
                    self.server.broken_ssl_ips[self.ip] = True
        ...
        if not self.sock_wrapped and self.cert_pin:
            self.close(
                "Crypt connection error: Socket not encrypted, but certificate"
                " pin present"
            )
    ...
    def send(self, message, streaming=False):
        ...
        if config.debug_socket:
            self.log(
                "Send: %s, to: %s, streaming: %s, site: %s, inner_path: %s,"
                " req_id: %s"
                % (
                    message.get("cmd"),
                    message.get("to"),
    ...
    def request(self, cmd, params=None, stream_to=None):
        if params is None:
            params = {}
        if (
            self.waiting_requests
            and self.protocol == "v2"
            and time.time() - max(self.last_req_time, self.last_recv_time) > 10
        ):
            self.close(
                "Request %s timeout: %.3fs"
                % (self.last_cmd_sent, time.time() - self.last_send_time)
            )
            return False
    ...
    def close(self, reason="Unknown"):
        ...
        self.log(
            "Closing connection: %s, waiting_requests: %s, sites: %s,"
            " buff: %s…"
            % (
                reason,
                len(self.waiting_requests),
                self.sites,
                self.incomplete_buff_recv,
            )
        )
    ...
    def updateOnlineStatus(
        self, outgoing_activity=False, successful_activity=False
    ):
        self.server.updateOnlineStatus(
            self,
