...
class ConnectionServer(object):
    def __init__(self, ip=None, port=None, request_handler=None):
        ...
        self.last_outgoing_internet_activity_time = 0
        self.last_successful_internet_activity_time = 0
        ...
        if msgpack.version[0] == 0 and msgpack.version[1] < 4:
            self.log.error(
                "Error: Unsupported msgpack version: %s (<0.4.0), please run"
                " `sudo apt-get install python-pip; sudo pip install msgpack"
                " --upgrade`"
                % str(msgpack.version)
            )
            sys.exit(0)
    ...
    def start(self, check_connections=True):
        ...
        if not self.port:
            ...
            return False
        self.log.debug(
            "Binding to: %s:%s, (msgpack: %s), supported crypt: %s"
            % (
                self.ip,
                self.port,
        ...
        try:
            ...
            self.log.info(
                "StreamServer create error: %s" % Debug.formatException(err)
            )
    ...
    def onStop(self, ui_websocket=None):
        ...
        for name, pool in list(self.managed_pools.items()):
            ...
            if size:
                ...
                pool.kill()
        self.sendShutdownProgress(
            ui_websocket, "File Server stopped. Now to exit.", 100
        )
    ...
    def getConnection(
        ...
        has_per_site_onion = (
            (
                ip.endswith(".onion")
                or self.port_opened.get(ip_type, None) is False
            )
            and self.tor_manager.start_onions
            and site
        )
        ...
        if key in self.ips:
            ...
            for connection in self.connections:
                if connection.ip == ip:
                    if (
                        peer_id
                        and connection.handshake.get("peer_id") != peer_id
                    ):
                        continue
                    if (
                        ip.endswith(".onion")
                        and self.tor_manager.start_onions
                        and ip.replace(".onion", "") != connection.target_onion
                    ):
                        continue
                    if not connection.connected and create:
                        succ = connection.event_connected.get()
            ...
            try:
                if has_per_site_onion:
                    connection = Connection(
                        ...
                        is_tracker_connection=is_tracker_connection,
                    )
                else:
                    connection = Connection(
                        self,
                ...
                connection.close(
                    "%s Connect error: %s" % (ip, Debug.formatException(err))
                )
                raise err
    ...
    def removeConnection(self, connection):
        ...
        if connection.target_onion:
            if (
                self.ips.get(connection.ip + connection.target_onion)
                == connection
            ):
                del self.ips[connection.ip + connection.target_onion]
        if (
            connection.cert_pin
            and self.ips.get(connection.ip + "#" + connection.cert_pin)
            == connection
        ):
        ...
        while self.running:
            ...
            for connection in self.connections[:]:
                ...
                if connection.unpacker and idle > 30:
                    ...
                    connection.close(
                        "[Cleanup] Tracker connection, idle: %.3fs" % idle
                    )
                if idle > 60 * 60:
                    connection.close(
                        "[Cleanup] After wakeup, idle: %.3fs" % idle
                    )
                elif (
                    idle > 20 * 60
                    and connection.last_send_time < time.time() - 10
                ):
                    if not connection.ping():
                        connection.close("[Cleanup] Ping timeout")
                elif (
                    idle > 10 * timeout_multipler
                    and connection.incomplete_buff_recv > 0
                ):
                    ...
                    idle > 10 * timeout_multipler
                    and connection.waiting_requests
                    and time.time() - connection.last_send_time
                    > 10 * timeout_multipler
                ):
                    connection.close(
                        "[Cleanup] Command %s timeout: %.3fs"
                        % (
                            connection.last_cmd_sent,
                            time.time() - connection.last_send_time,
                        )
                    )
                elif idle < 60 and connection.bad_actions > 40:
                    connection.close(
                        "[Cleanup] Too many bad actions: %s"
                        % connection.bad_actions
                    )
            ...
            if time.time() - s > 0.01:
                self.log.debug(
                    "Connection cleanup in %.3fs" % (time.time() - s)
                )
            self.sleep(15)
    ...
    def checkMaxConnections(self):
        ...
        for connection in self.connections:
            ...
            if num_closed > config.global_connected_limit * 0.1:
                break
        self.log.debug(
            "Closed %s connections of %s after reached limit %s in %.3fs"
            % (
                num_closed,
                num_connected_before,
                config.global_connected_limit,
                time.time() - s,
            )
        )
        return num_closed
    ...
    def updateOnlineStatus(
        self, connection, outgoing_activity=False, successful_activity=False
    ):
        now = time.time()
        ...
        if (
            ...
            < now - self.internet_outage_threshold
        ) and (
            self.last_successful_internet_activity_time
    ...
    def getTimecorrection(self):
        corrections = sorted(
            [
                connection.handshake.get("time")
                - connection.handshake_time
                + connection.last_ping_delay
                for connection in self.connections
                if connection.handshake.get("time")
                and connection.last_ping_delay
            ]
        )
        ...
        median = (
            corrections[mid - 1] + corrections[mid] + corrections[mid + 1]
        ) / 3
        return median
    ...
    def getIpUnreachability(self, ip):
        ...
        if ip_type == "onion" and not self.tor_manager.enabled:
            return (
                "Can't connect to onion addresses, no Tor controller present"
            )
        if (
            config.tor == "always"
            and helper.isPrivateIp(ip)
            and ip not in config.ip_local
        ):
            return "Can't connect to local IPs in Tor: always mode"
        return False

    def isIpReachable(self, ip):
        return self.getIpUnreachability(ip) is False
