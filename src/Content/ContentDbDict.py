...
class ContentDbDict(dict):
    def __init__(self, site, *args, **kwargs):
        ...
        self.log.debug(
            "ContentDb init: %.3fs, found files: %s, sites: %s"
            % (time.time() - s, len(self), len(self.db.site_ids))
        )
        ...
        try:
            ...
            if self.num_loaded % 100 == 0:
                if config.verbose:
                    ...
                    self.log.debug(
                        "Loaded json: %s (latest: %s)" % (self.num_loaded, key)
                    )
    ...
    def addCachedKey(self, key):
        if (
            key not in self.cached_keys
            and key != "content.json"
            and len(key) > 40
        ):
