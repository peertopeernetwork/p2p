...
class ContentDb(Db):
    ...
    def init(self):
        try:
            ...
            if foreign_key_error:
                raise Exception(
                    "Database foreign key error: %s" % foreign_key_error
                )
        except Exception as err:
            self.log.error(
                "Error loading content.db: %s, rebuilding…"
                % Debug.formatException(err)
            )
            self.close()
            os.unlink(self.db_path)
            Db.__init__(
                self, {"db_name": "ContentDb", "tables": {}}, self.db_path
            )
            self.foreign_keys = True
            self.schema = self.getSchema()
            try:
    ...
    def getSchema(self):
        schema = {"db_name": "ContentDb", "version": 3, "tables": {}}
        if not self.getTableVersion("site"):
            ...
            if version > 0:
                ...
                self.execute(
                    ...
                    {"json_id": 0, "key": "table.site.version", "value": 1},
                )
                self.execute(
                    "INSERT INTO keyvalue ?",
                    {"json_id": 0, "key": "table.content.version", "value": 1},
                )
        schema["tables"]["site"] = {
            "cols": [
            ...
            "indexes": ["CREATE UNIQUE INDEX site_address ON site (address)"],
            "schema_changed": 1,
        }
        schema["tables"]["content"] = {
            ...
            "indexes": [
                (
                    "CREATE UNIQUE INDEX content_key ON content (site_id,"
                    " inner_path)"
                ),
    ...
    def needSite(self, site):
        if site.address not in self.site_ids:
            self.execute(
                "INSERT OR IGNORE INTO site ?", {"address": site.address}
            )
            self.site_ids = {}
        ...
        self.insertOrUpdate(
            "content",
            {
                ...
                "size_files": sum(
                    [
                        val["size"]
                        for key, val in content.get("files", {}).items()
                    ]
                ),
                "size_files_optional": sum(
                    [
                        val["size"]
                        for key, val in content.get(
                            "files_optional", {}
                        ).items()
                    ]
                ),
    ...
    def loadDbDict(self, site):
        res = self.execute(
            (
                "SELECT GROUP_CONCAT(inner_path, '|') AS inner_paths FROM"
                " content WHERE ?"
            ),
            {"site_id": self.site_ids.get(site.address, 0)},
        )
    ...
    def getTotalSize(self, site, ignore=None):
        ...
        res = self.execute(
            (
                "SELECT SUM(size) + SUM(size_files) AS size,"
                " SUM(size_files_optional) AS size_optional FROM content"
                " WHERE ?"
            ),
            params,
        )
    ...
    def listModified(self, site, after=None, before=None):
        ...
        res = self.execute(
            "SELECT inner_path, modified FROM content WHERE ?", params
        )
