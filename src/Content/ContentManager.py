...
class ContentManager(object):
    ...
    def loadContents(self):
        if len(self.contents) == 0:
            self.log.info(
                "ContentDb not initialized, load files from filesystem…"
            )
            self.loadContent(add_bad_files=False, delete_removed_files=False)
        (
            self.site.settings["size"],
            self.site.settings["size_optional"],
        ) = self.getTotalSize()
        if "hashfield" in self.site.settings.get("cache", {}):
            self.hashfield.frombytes(
                base64.b64decode(self.site.settings["cache"]["hashfield"])
            )
            del self.site.settings["cache"]["hashfield"]
        elif (
            self.contents.get("content.json")
            and self.site.settings["size_optional"] > 0
        ):
    ...
    def getFileChanges(self, old_files, new_files):
        deleted = {
            key: val for key, val in old_files.items() if key not in new_files
        }
        deleted_hashes = {
            val.get("sha512"): key
        ...
        added = {
            key: val for key, val in new_files.items() if key not in old_files
        }
        renamed = {}
        ...
        if os.path.isfile(content_path):
            try:
                if (
                    not force
                    and old_content
                    and not self.site.settings.get("own")
                ):
        ...
        try:
            ...
            for relative_path, info in new_content.get("files", {}).items():
                ...
                if old_content and old_content["files"].get(relative_path):
                    old_hash = old_content["files"][relative_path].get(
                        hash_type
                    )
                else:
                    old_hash = None
            ...
            for relative_path, info in new_content.get(
                "files_optional", {}
            ).items():
                file_inner_path = content_inner_dir + relative_path
                new_hash = info["sha512"]
                if old_content and old_content.get("files_optional", {}).get(
                    relative_path
                ):
                    old_hash = old_content["files_optional"][
                        relative_path
                    ].get("sha512")
                    if old_hash != new_hash and self.site.isDownloadable(
                        file_inner_path
                    ):
                        changed.append(file_inner_path)
                    elif (
                        old_hash != new_hash
                        and self.hashfield.hasHash(old_hash)
                        and not self.site.settings.get("own")
                    ):
                        try:
                            old_hash_id = self.hashfield.getHashId(old_hash)
                            self.optionalRemoved(
                                file_inner_path,
                                old_hash_id,
                                old_content["files_optional"][relative_path][
                                    "size"
                                ],
                            )
                            self.optionalDelete(file_inner_path)
                            self.log.debug(
                                "Deleted changed optional file: %s"
                                % file_inner_path
                            )
                        except Exception as err:
                            self.log.warning(
            ...
            if old_content:
                ...
                for relative_path_old, relative_path_new in renamed.items():
                    self.log.debug(
                        "Renaming: %s -> %s"
                        % (relative_path_old, relative_path_new)
                    )
                    if relative_path_new in new_content.get(
                        "files_optional", {}
                    ):
                        self.optionalRenamed(
                            content_inner_dir + relative_path_old,
                            content_inner_dir + relative_path_new,
                        )
                    if self.site.storage.isFile(relative_path_old):
                        try:
                            self.site.storage.rename(
                                relative_path_old, relative_path_new
                            )
                            ...
                            self.log.warning(
                                ...
                                % (relative_path_old, relative_path_new, err)
                            )
                if deleted and not self.site.settings.get("own"):
                    for file_relative_path in deleted:
                        ...
                        try:
                            if old_content.get(
                                "files_optional"
                            ) and old_content["files_optional"].get(
                                file_relative_path
                            ):
                                self.optionalDelete(file_inner_path)
                                old_hash = old_content["files_optional"][
                                ...
                                if self.hashfield.hasHash(old_hash):
                                    old_hash_id = self.hashfield.getHashId(
                                        old_hash
                                    )
                                    self.optionalRemoved(
                                        file_inner_path,
                                        old_hash_id,
                                        old_content["files_optional"][
                                            file_relative_path
                                        ]["size"],
                                    )
                            else:
                            ...
                            self.log.debug(
                                "Deleted file: %s" % file_inner_path
                            )
                        except Exception as err:
                            self.log.debug(
                                "Error deleting file %s: %s"
                    ...
                    tree = {
                        ...
                        for root, dirs, files in os.walk(
                            self.site.storage.getPath(content_inner_dir)
                        )
                    }
                    for root in sorted(tree, key=len, reverse=True):
                        dirs, files = tree[root]
                        if dirs == [] and files == []:
                            root_inner_path = self.site.storage.getInnerPath(
                                root.replace("\\", "/")
                            )
                            self.log.debug(
                                "Empty directory: %s, cleaning up."
                                % root_inner_path
                            )
                            try:
                                self.site.storage.deleteDir(root_inner_path)
                                tree[os.path.dirname(root)][0].remove(
                                    os.path.basename(root)
                                )
                            except Exception as err:
                                self.log.debug(
                                    "Error deleting empty directory %s: %s"
                                    % (root_inner_path, err)
                                )
            if (
                old_content
                and "user_contents" in new_content
                and "archived" in new_content["user_contents"]
            ):
                old_archived = old_content.get("user_contents", {}).get(
                    "archived", {}
                )
                new_archived = new_content.get("user_contents", {}).get(
                    "archived", {}
                )
                self.log.debug(
                    "old archived: %s, new archived: %s"
                    % (len(old_archived), len(new_archived))
                )
                archived_changed = {
                    key: date_archived
                ...
                if archived_changed:
                    ...
                    for (
                        ...
                        if (
                            self.contents.get(archived_inner_path, {}).get(
                                "modified", 0
                            )
                            < date_archived
                        ):
            ...
            if (
                old_content
                and "user_contents" in new_content
                and "archived_before" in new_content["user_contents"]
            ):
                old_archived_before = old_content.get("user_contents", {}).get(
                    "archived_before", 0
                )
                new_archived_before = new_content.get("user_contents", {}).get(
                    "archived_before", 0
                )
                if old_archived_before != new_archived_before:
                    self.log.debug(
                        "Archived before changed: %s -> %s"
                        % (old_archived_before, new_archived_before)
                    )
                    num_removed_contents = 0
                    for archived_inner_path in self.listModified(
                        before=new_archived_before
                    ):
                        if (
                            archived_inner_path.startswith(content_inner_dir)
                            and archived_inner_path != content_inner_path
                        ):
                            self.removeContent(archived_inner_path)
                    ...
                    self.log.debug(
                        "Archived removed contents: %s, removed bad files: %s"
                        % (num_removed_contents, num_removed_bad_files)
                    )
            if load_includes and "includes" in new_content:
                for relative_path, info in list(
                    new_content["includes"].items()
                ):
                    ...
                    if self.site.storage.isFile(include_inner_path):
                        include_changed, include_deleted = self.loadContent(
                            ...
                            delete_removed_files=delete_removed_files,
                        )
                        if include_changed:
            ...
            self.log.warning(
                "%s parse error: %s"
                % (content_inner_path, Debug.formatException(err))
            )
            return [], []
        if add_bad_files:
            for inner_path in changed:
                self.site.bad_files[inner_path] = (
                    self.site.bad_files.get(inner_path, 0) + 1
                )
            for inner_path in deleted:
                if inner_path in self.site.bad_files:
                    del self.site.bad_files[inner_path]
                self.site.worker_manager.removeSolvedFileTasks()
        if new_content.get("modified", 0) > self.site.settings.get(
            "modified", 0
        ):
    ...
    def removeContent(self, inner_path):
        ...
        try:
            ...
            self.log.debug(
                "Error loading %s for removeContent: %s"
        ...
        for file_relative_path in files:
            ...
            try:
                ...
                self.log.debug(
                    "Error deleting file %s: %s" % (file_inner_path, err)
                )
        try:
    ...
    def listModified(self, after=None, before=None):
        return self.contents.db.listModified(
            self.site, after=after, before=before
        )
    ...
    def listContents(self, inner_path="content.json", user_files=False):
        ...
        for relative_path in list(
            self.contents[inner_path].get("includes", {}).keys()
        ):
            include_inner_path = content_inner_dir + relative_path
    ...
    def isArchived(self, inner_path, modified):
        ...
        if file_info:
            ...
            time_directory_archived = file_info.get("archived", {}).get(
                relative_directory, 0
            )
            if (
                modified <= time_archived_before
    ...
    def isModified(self, inner_path):
        ...
        if inner_path.endswith("content.json"):
            try:
                is_valid = self.verifyFile(
                    inner_path,
                    self.site.storage.open(inner_path),
                    ignore_same=False,
                )
                if is_valid:
                    is_modified = False
    ...
    def getFileInfo(self, inner_path, new_file=False):
        ...
        while True:
            ...
            if content and "user_contents" in content:
                ...
                relative_content_path = inner_path[
                    len(content_inner_path_dir) :
                ]
                user_auth_address_match = re.match(
                    r"([A-Za-z0-9]+)/.*", relative_content_path
                )
                if user_auth_address_match:
                    user_auth_address = user_auth_address_match.group(1)
                    back["content_inner_path"] = "%s%s/content.json" % (
                        content_inner_path_dir,
                        user_auth_address,
                    )
                else:
                    back["content_inner_path"] = (
    ...
    def getRules(self, inner_path, content=None):
        ...
        while True:
            ...
            if parent_content and "includes" in parent_content:
                return parent_content["includes"].get(
                    "/".join(inner_path_parts)
                )
            elif parent_content and "user_contents" in parent_content:
                return self.getUserContentRules(
                    parent_content, inner_path, content
                )
            else:
                if dirs:
                    inner_path_parts.insert(0, dirs.pop())
    ...
    def getUserContentRules(self, parent_content, inner_path, content):
        ...
        if "inner_path" in parent_content:
            parent_content_dir = helper.getDirname(
                parent_content["inner_path"]
            )
            user_address = re.match(
                r"([A-Za-z0-9]*?)/", inner_path[len(parent_content_dir) :]
            ).group(1)
        else:
            user_address = re.match(
                r".*/([A-Za-z0-9]*?)/.*?$", inner_path
            ).group(1)
        try:
            if not content:
                content = self.site.storage.loadJson(inner_path)
            user_urn = "%s/%s" % (
                content["cert_auth_type"],
                content["cert_user_id"],
            )
        ...
        if user_address in user_contents["permissions"]:
            rules = copy.copy(
                user_contents["permissions"].get(user_address, {})
            )
        else:
            rules = copy.copy(
                user_contents["permissions"].get(cert_user_id, {})
            )
        if rules is False:
            banned = True
            rules = {}
        ...
        for permission_pattern, permission_rules in list(
            user_contents["permission_rules"].items()
        ):
        ...
        rules["cert_signers_pattern"] = user_contents.get(
            "cert_signers_pattern"
        )
        ...
        for file_relative_path in self.contents[inner_path].get("files", {}):
            ...
            if self.site.storage.isFile(file_inner_path + "-new"):
                ...
                if update_files:
                    self.site.storage.delete(file_inner_path)
                    self.site.storage.rename(
                        file_inner_path + "-new", file_inner_path
                    )
    ...
    def hashFile(self, dir_inner_path, file_relative_path, optional=False):
        ...
        if optional and not self.hashfield.hasHash(sha512sum):
            self.optionalDownloaded(
                file_inner_path,
                self.hashfield.getHashId(sha512sum),
                file_size,
                own=True,
            )
    ...
    def isValidRelativePath(self, relative_path):
        if ".." in relative_path.replace("\\", "/").split("/"):
            ...
            r".*(^|/)(CON|PRN|AUX|NUL|COM[1-9]|LPT[1-9]|CONOUT\$|CONIN\$)(\.|/|$)",
            relative_path,
            re.IGNORECASE,
        ):
            return False
    ...
    def hashFiles(
        ...
        for file_relative_path in self.site.storage.walk(
            dir_inner_path, ignore_pattern
        ):
            ...
            if file_name == "content.json":
                ...
                or file_name.endswith("-new")
            ):
                ignored = True
            elif not self.isValidRelativePath(file_relative_path):
                ignored = True
                self.log.error(
                    "- [ERROR] Invalid filename: %s" % file_relative_path
                )
            elif (
                dir_inner_path == ""
                and db_inner_path
                and file_relative_path.startswith(db_inner_path)
            ):
                ignored = True
            elif optional_pattern and SafeRe.match(
                optional_pattern, file_relative_path
            ):
                optional = True
            if ignored:
                ...
                if optional:
                    ...
                    files_optional_node.update(
                        self.hashFile(
                            dir_inner_path, file_relative_path, optional=True
                        )
                    )
                else:
    ...
    def serializeForSigning(self, content):
        ...
        if (
            config.fix_float_decimals
            and type(modified) is float
            and not str(modified).endswith(".0")
        ):
    ...
    def sign(
        ...
        if not inner_path.endswith("content.json"):
            raise SignError(
                "Invalid file name, you can only sign content.json files"
            )
        if inner_path in self.contents:
            content = self.contents.get(inner_path)
            if (
                content
                and content.get("cert_sign", False) is None
                and self.site.storage.isFile(inner_path)
            ):
                content["cert_sign"] = self.site.storage.loadJson(
                    inner_path
                ).get("cert_sign")
        else:
            content = None
        if not content:
            self.log.info(
                "File %s not exist yet, loading default values…" % inner_path
            )
        ...
        files_node, files_optional_node = self.hashFiles(
            helper.getDirname(inner_path),
            content.get("ignore"),
            content.get("optional"),
        )
        if not remove_missing_optional:
            for file_inner_path, file_details in content.get(
                "files_optional", {}
            ).items():
        ...
        if inner_path == "content.json":
            new_content["p2p_version"] = config.version
        ...
        if privatekey_address not in valid_signers:
            raise SignError(
                "Private key invalid! Valid signers: %s, Private key"
                " address: %s" % (valid_signers, privatekey_address)
            )
        self.log.info(
            "Correct %s in valid signers: %s"
            % (privatekey_address, valid_signers)
        )
        if (
            inner_path == "content.json"
            and privatekey_address == self.site.address
        ):
            signers_data = "%s:%s" % (
                new_content["signs_required"],
                ",".join(valid_signers),
            )
            new_content["signers_sign"] = CryptBitcoin.sign(
                str(signers_data), privatekey
            )
    ...
    def getValidSigners(self, inner_path, content=None):
        ...
        if inner_path == "content.json":
            if (
                "content.json" in self.contents
                and "signers" in self.contents["content.json"]
            ):
    ...
    def verifyCertSign(
        self, user_address, user_auth_type, user_name, issuer_address, sign
    ):
        from Crypt import CryptBitcoin
    ...
    def verifyCert(self, inner_path, content):
        ...
        if not rules.get("cert_signers") and not rules.get(
            "cert_signers_pattern"
        ):
            return True
        ...
        if not cert_address:
            if rules.get("cert_signers_pattern") and SafeRe.match(
                rules["cert_signers_pattern"], domain
            ):
                cert_address = domain
            else:
                raise VerifyError("Invalid cert signer: %s" % domain)
        return self.verifyCertSign(
            rules["user_address"],
            content["cert_auth_type"],
            name,
            cert_address,
    ...
    def verifyContent(self, inner_path, content):
        content_size = len(json.dumps(content, indent=1)) + sum(
            [
                file["size"]
                for file in list(content["files"].values())
                if file["size"] >= 0
            ]
        )
        old_content = self.contents.get(inner_path)
        if old_content:
            old_content_size = len(json.dumps(old_content, indent=1)) + sum(
                [
                    file["size"]
                    for file in list(old_content.get("files", {}).values())
                ]
            )
            old_content_size_optional = sum(
                [
                    file["size"]
                    for file in list(
                        old_content.get("files_optional", {}).values()
                    )
                ]
            )
        else:
        ...
        content_size_optional = sum(
            [
                file["size"]
                for file in list(content.get("files_optional", {}).values())
                if file["size"] >= 0
            ]
        )
        site_size = (
            self.site.settings["size"] - old_content_size + content_size
        )
        site_size_optional = (
            self.site.settings["size_optional"]
            - old_content_size_optional
        ...
        if content.get("address") and content["address"] != self.site.address:
            raise VerifyError(
                "Wrong site address: %s != %s"
                % (content["address"], self.site.address)
            )
        ...
        if inner_path == "content.json":
            ...
            if content_size_file > site_size_limit:
                ...
                raise VerifyError(
                    "Content too large %s B > %s B, aborting task…"
                    % (site_size, site_size_limit)
                )
        for file_relative_path in list(content.get("files", {}).keys()) + list(
            content.get("files_optional", {}).keys()
        ):
            if not self.isValidRelativePath(file_relative_path):
                raise VerifyError(
                    "Invalid relative path: %s" % file_relative_path
                )
        if inner_path == "content.json":
            self.site.settings["size"] = site_size
            ...
            if self.verifyContentInclude(
                inner_path, content, content_size, content_size_optional
            ):
                self.site.settings["size"] = site_size
                self.site.settings["size_optional"] = site_size_optional
                return True
        ...
        if type(file) is dict:
            ...
            try:
                if sys.version_info.major == 3 and sys.version_info.minor < 6:
                    new_content = json.loads(file.read().decode("utf_8"))
                else:
        ...
        if signs:
            ...
            if inner_path == "content.json" and len(valid_signers) > 1:
                ...
                if not CryptBitcoin.verify(
                    signers_data,
                    self.site.address,
                    new_content["signers_sign"],
                ):
                    raise VerifyError("Invalid signers_sign!")
            if inner_path != "content.json" and not self.verifyCert(
                inner_path, new_content
            ):
                raise VerifyError("Invalid cert!")
            valid_signs = 0
            for address in valid_signers:
                if address in signs:
                    valid_signs += CryptBitcoin.verify(
                        sign_content, address, signs[address]
                    )
                if valid_signs >= signs_required:
                    break
            if valid_signs < signs_required:
                raise VerifyError(
                    "Valid signs: %s/%s" % (valid_signs, signs_required)
                )
            else:
