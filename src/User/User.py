...
class User(object):
    def __init__(self, master_address=None, master_seed=None, data=None):
        if data is None:
            data = {}
        if master_seed:
            self.master_seed = master_seed
            self.master_address = CryptBitcoin.privatekeyToAddress(
                self.master_seed
            )
        elif master_address:
    ...
    def save(self):
        ...
        helper.atomicWrite(
            "%s/users.json" % config.data_dir,
            helper.jsonDumps(users).encode("utf_8"),
        )
    ...
    def generateAuthAddress(self, address):
        ...
        auth_privatekey = CryptBitcoin.hdPrivatekey(
            self.master_seed, address_id
        )
        self.sites[address] = {
        ...
        self.saveDelayed()
        self.log.debug(
            "Added new site: %s in %.3fs" % (address, time.time() - s)
        )
    ...
    def getNewSiteData(self):
        ...
        site_privatekey = CryptBitcoin.hdPrivatekey(
            self.master_seed, bip32_index
        )
