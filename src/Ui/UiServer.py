...
class UiWSGIHandler(WebSocketHandler):
    ...
    def run_application(self):
        ...
        try:
            ...
            logging.warning(
                "%s error: %s" % (err_name, Debug.formatException(err))
            )
...
class UiServer:
    def __init__(self):
        ...
        if config.ui_host:
            ...
            self.allowed_hosts = {"p2p", "localhost:%s" % config.ui_port}
    ...
    def handleRequest(self, env, start_response):
        path = bytes(env["PATH_INFO"], "raw-unicode-escape").decode("utf_8")
        ...
        if config.debug:
            ...
            try:
                ...
                logging.debug(
                    "UiRequest error: %s" % Debug.formatException(err)
                )
                return ui_request.error500(
                    "Err: %s" % Debug.formatException(err)
                )
    ...
    def reload(self):
        ...
        UiRequest = SourceFileLoader(
            "UiRequest", "src/Ui/UiRequest.py"
        ).UiRequest
    ...
    def start(self):
        ...
        if config.debug:
            ...
            try:
                ...
                self.log.info(
                    "%s: For debugging please download Werkzeug"
                    " (http://werkzeug.pocoo.org/)" % err
                )
                from Debug import DebugReloader
        ...
        try:
            ...
            self.log.error(
                "Web interface bind error, must be running already,"
                " exiting… %s" % err
            )
            import main
