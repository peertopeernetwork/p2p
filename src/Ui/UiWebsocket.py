...
class UiWebsocket(object):
    ...
    def start(self):
        ws = self.ws
        if (
            self.site.address == config.homepage
            and not self.site.page_requested
        ):
            ...
            if (
                not file_server.port_opened
                or file_server.tor_manager.start_onions is None
            ):
                ...
                try:
                    ...
                    self.log.error(
                        "Uncaught Exception: " + Debug.formatException(err)
                    )
        ...
        while True:
            try:
                ...
                self.log.error(
                    "WebSocket receive error: %s" % Debug.formatException(err)
                )
                break
            ...
            if message:
                try:
                    ...
                    self.log.error(
                        "WebSocket handleRequest error: %s \n %s"
    ...
    def addHomepageNotifications(self):
        if not (self.hasPlugin("Multiuser")) and not (
            self.hasPlugin("UiPassword")
        ):
            ...
            if ("0.0.0.0" == bind_ip or "*" == bind_ip) and (not whitelist):
                self.site.notifications.append(
                    [
                        "error",
                        _(
                            "You are not going to set up a public gateway."
                            " However, <b>your Web UI is<br>"
    ...
    def hasSitePermission(self, address, cmd=None):
        if (
            ...
            and "ADMIN" not in self.site.settings["permissions"]
        ):
            return False
        else:
            return True
    ...
    def hasFilePermission(self, inner_path):
        ...
        return (
            self.site.settings["own"]
            or self.user.getAuthAddress(self.site.address) in valid_signers
        )
    ...
    def send(self, message, cb=None):
        ...
        try:
            ...
            self.log.debug(
                "Websocket send error: %s" % Debug.formatException(err)
            )
            self.state["sending"] = False
    ...
    def asyncWrapper(self, func):
        def asyncErrorWatcher(func, *args, **kwargs):
            try:
                ...
                self.cmd(
                    "error",
                    "Internal error: %s" % Debug.formatException(err, "html"),
                )
        ...
        if cmd == "response":
            ...
            if not func:
                return self.response(
                    req["id"], {"error": "Unknown command: %s" % cmd}
                )

            if not self.hasCmdPermission(cmd):
                return self.response(
                    req["id"],
                    {"error": "You don't have permission to run %s" % cmd},
                )
    ...
    def formatSiteInfo(self, site, create_user=True):
        ...
        ret = {
            "auth_address": self.user.getAuthAddress(
                site.address, create=create_user
            ),
        ...
        if site.settings["own"]:
            ret["privatekey"] = bool(
                self.user.getSiteData(site.address, create=create_user).get(
                    "privatekey"
                )
            )
    ...
    def actionAs(self, to, address, cmd, params=None):
        if params is None:
            params = []
    ...
    def actionResponse(self, to, result):
        if to in self.waiting_cb:
            ...
            self.log.error(
                "Websocket callback not found: %s, %s" % (to, result)
            )
    ...
    def actionAnnouncerStats(self, to):
        ...
        for site in list(self.server.sites.values()):
            for tracker, stats in site.announcer.stats.items():
                ...
                is_latest_data = bool(
                    stats["time_request"]
                    > back[tracker].get("time_request", 0)
                    and stats["status"]
                )
    ...
    def actionSiteSign(
        ...
        is_user_content = file_info and (
            "cert_signers" in file_info or "cert_signers_pattern" in file_info
        )
        if is_user_content and privatekey is None:
            ...
            self.log.debug(
                "Extending content.json with cert %s" % extend["cert_user_id"]
            )

        if not self.hasFilePermission(inner_path):
            self.log.error(
                "SiteSign error: you don't own this site & site owner doesn't"
                " allow you to do so."
            )
            return self.response(
        ...
        if privatekey == "stored":
            privatekey = self.user.getSiteData(self.site.address).get(
                "privatekey"
            )
            if not privatekey:
                self.cmd(
                    ...
                    [
                        "error",
                        _["Content signing failed"]
                        + "<br><small>Private key not found in sites.json"
                        " </small>",
                    ],
                )
                self.response(
                    to, {"error": "Site sign failed: Private key not stored."}
                )
                self.log.error(
                    "Site sign failed: %s: Private key not stored in"
                    " sites.json" % inner_path
                )
                return
        if not privatekey:
        ...
        site.content_manager.loadContent(
            inner_path, add_bad_files=False, force=True
        )
    ...
    def actionSitePublish(
        ...
        if inner_path not in self.site.content_manager.contents:
            return self.response(
                to, {"error": "File %s not found" % inner_path}
            )
        ...
        thread = RateLimit.callAsync(
            event_name, 30, self.doSitePublish, self.site, inner_path
        )
        ...
        if called_instantly:
            self.cmd(
                ...
                [
                    "publish",
                    _["Content published to {0}/{1} peers."].format(0, 5),
                    0,
                ],
            )
            thread.link(
                lambda thread: self.cbSitePublish(
            ...
            self.cmd(
                ...
                [
                    ...
                    _["Content publish queued for {0:.0f} seconds."].format(
                        RateLimit.delayLeft(event_name, 30)
                    ),
                    5000,
                ],
            )
    ...
    def doSitePublish(self, site, inner_path):
        def cbProgress(published, limit):
            ...
            self.cmd(
                ...
                [
                    "publish",
                    _["Content published to {0}/{1} peers."].format(
                        published, limit
                    ),
                    progress,
                ],
        ...
        back = site.publish(
            limit=5, inner_path=inner_path, diffs=diffs, cb_progress=cbProgress
        )
        if back == 0:
            self.cmd(
                "progress", ["publish", _["Content publish failed."], -100]
            )
        else:
    ...
    def cbSitePublish(
        self, to, site, thread, notification=True, callback=True
    ):
        ...
        if published > 0:
            ...
            if len(site.peers) == 0:
                ...
                if (
                    any(main.file_server.port_opened.values())
                    or main.file_server.tor_manager.start_onions
                ):
                    if notification:
                        self.cmd(
                            "notification",
                            [
                                "info",
                                _[
                                    "No peers found, but your content is ready"
                                    " to access."
                                ],
                            ],
                        )
                    ...
                    if notification:
                        self.cmd(
                            ...
                            [
                                "info",
                                _(
                                    """{_[Your network connection is restricted. Please, open <b>{0}</b> port]}<br>
    ...
    def actionFileWrite(
        ...
        content_inner_path = re.sub(
            "^(.*)/.*?$", "\\1/content.json", inner_path
        )
        if (
            self.site.bad_files.get(inner_path)
            or self.site.bad_files.get(content_inner_path)
        ) and not ignore_bad_files:
            ...
            if not found:
                self.cmd(
                    ...
                    [
                        _[
                            "This file still in sync, if you write it now,"
                            " then the previous content may be lost."
                        ],
                        _["Write content anyway"],
                    ],
                    lambda res: self.actionFileWrite(
                        to, inner_path, content_base64, ignore_bad_files=True
                    ),
                )
                return False
        ...
        try:
            ...
            if (
                inner_path.endswith(".json")
                and not inner_path.endswith("content.json")
                and self.site.storage.isFile(inner_path)
                and not self.site.storage.isFile(inner_path + "-old")
            ):
                try:
            ...
            return self.response(
                to, {"error": "Write error: %s" % Debug.formatException(err)}
            )
    ...
    def actionFileDelete(self, to, inner_path):
        if not self.hasFilePermission(inner_path):
            self.log.error(
                "File delete error: you don't own this site & you are not"
        ...
        try:
            if stats:
                ...
                for file_name in self.site.storage.list(inner_path):
                    file_stats = os.stat(
                        self.site.storage.getPath(inner_path + "/" + file_name)
                    )
    ...
    def actionDbQuery(self, to, query, params=None, wait_for=None):
        ...
        try:
            ...
            return self.response(
                to, {"error": Debug.formatExceptionMessage(err)}
            )
    ...
    def actionFileRules(self, to, inner_path, use_my_cert=False, content=None):
        ...
        if not content:
            ...
            if cert and cert[
                "auth_address"
            ] in self.site.content_manager.getValidSigners(inner_path):
                content = {}
                content["cert_auth_type"] = cert["auth_type"]
                content["cert_user_id"] = self.user.getCertUserId(
                    self.site.address
                )
        ...
        if inner_path.endswith("content.json") and rules:
            if content:
                rules["current_size"] = len(json.dumps(content)) + sum(
                    [
                        file["size"]
                        for file in list(content.get("files", {}).values())
                    ]
                )
            else:
    ...
    def actionCertAdd(self, to, domain, auth_type, auth_user_name, cert):
        try:
            res = self.user.addCert(
                self.user.getAuthAddress(self.site.address),
                domain,
                auth_type,
                auth_user_name,
                cert,
            ...
            if res is True:
                self.cmd(
                    ...
                    [
                        "done",
                        _(
                            "{_[New certificate added]:}"
                            " <b>{auth_type}/{auth_user_name}@{domain}</b>."
                        ),
                    ],
                )
                ...
                body = _(
                    "{_[Your current certificate]:}"
                    " <b>{cert_current[auth_type]}/{cert_current[auth_user_name]}@{domain}</b>"
                )
                self.cmd(
                    "confirm",
                    [
                    ...
                    lambda res: self.cbCertAddConfirm(
                        to, domain, auth_type, auth_user_name, cert
                    ),
                )
            else:
            ...
            self.log.error(
                "CertAdd error: Exception - %s (%s)"
                % (err.message, Debug.formatException(err))
            )
    ...
    def actionCertSelect(
        ...
        for domain, cert in list(self.user.certs.items()):
            if auth_address == cert[
                "auth_address"
            ] and domain == site_data.get("cert"):
                active = domain
            title = cert["auth_user_name"] + "@" + domain
            accepted_pattern_match = accepted_pattern and SafeRe.match(
                accepted_pattern, domain
            )
            if (
                domain in accepted_domains
                or accept_any
                or accepted_pattern_match
            ):
        ...
        body = (
            ...
            + _["Select account you want to use in this site:"]
            + "</span>"
        )
        ...
        for domain, account, css_class in accounts:
            if domain == active:
                css_class += " active"
                title = (
                    _("<b>%s</b> <small>({_[currently selected]})</small>")
                    % account
                )
            else:
                title = "<b>%s</b>" % account
            body += (
                "<a href='#Select+account' class='select select-close cert %s'"
                " title='%s'>%s</a>" % (css_class, domain, title)
            )
        ...
        if more_domains:
            body += (
                "<div style='background-color: #F7F7F7; margin-right: -30px'>"
            )
        ...
        script = (
            """
             $(".notification .select.cert").on("click", function() {
                ...
                p2pframe.response(%s, this.title)
                return false
             })
        """
        ...
        self.cmd(
            "notification",
            ["ask", body],
            lambda domain: self.actionCertSet(to, domain),
        )
    ...
    def actionPermissionDetails(self, to, permission):
        if permission == "ADMIN":
            self.response(
                to,
                _["Modify your client's configuration and access all site"]
    ...
    def actionSiteList(self, to, connecting_sites=False):
        ...
        for site in list(self.server.sites.values()):
            if (
                not site.content_manager.contents.get("content.json")
                and not connecting_sites
            ):
                continue
    ...
    def actionSiteUpdate(
        ...
        if site and (
            site.address == self.site.address
            or "ADMIN" in self.site.settings["permissions"]
        ):
    ...
    def cbSiteClone(
        ...
        if target_address:
            ...
            privatekey = self.user.getSiteData(target_site.address).get(
                "privatekey"
            )
            site.clone(
                target_address, privatekey, root_inner_path=root_inner_path
            )
            ...
            (
                ...
                new_address_index,
                new_site_data,
            ) = self.user.getNewSiteData()
            new_site = site.clone(
                new_address,
                new_site_data["privatekey"],
                address_index=new_address_index,
    ...
    def actionSiteClone(
        ...
        if site.bad_files:
            for bad_inner_path in list(site.bad_files.keys()):
                is_user_file = "cert_signers" in site.content_manager.getRules(
                    bad_inner_path
                )
                if not is_user_file and bad_inner_path != "content.json":
                    self.cmd(
                        "notification",
                        ["error", _["Clone error: Site still in sync"]],
                    )
                    return {"error": "Site still in sync"}

        if "ADMIN" in self.getPermissions(to):
            self.cbSiteClone(
                to, address, root_inner_path, target_address, redirect
            )
        else:
            self.cmd(
                "confirm",
                [_["Clone site <b>%s</b>?"] % address, _["Clone"]],
                lambda res: self.cbSiteClone(
                    to, address, root_inner_path, target_address, redirect
                ),
            )
    ...
    @flag.async_run
    def actionSiteListModifiedFiles(
        self, to, content_inner_path="content.json"
    ):
        ...
        if content.get("modified", 0) < self.site.settings["cache"].get(
            "time_modified_files_check", 0
        ):
            min_mtime = self.site.settings["cache"].get(
                "time_modified_files_check"
            )
            modified_files = self.site.settings["cache"].get(
                "modified_files", []
            )

        inner_paths = (
            [content_inner_path]
            + list(content.get("includes", {}).keys())
            + list(content.get("files", {}).keys())
        )
        ...
        for relative_inner_path in inner_paths:
            inner_path = (
                helper.getDirname(content_inner_path) + relative_inner_path
            )
            try:
                is_mtime_newer = (
                    os.path.getmtime(self.site.storage.getPath(inner_path))
                    > min_mtime + 1
                )
                if is_mtime_newer:
                    if inner_path.endswith("content.json"):
                        is_modified = self.site.content_manager.isModified(
                            inner_path
                        )
                    else:
                        previous_size = content["files"][inner_path]["size"]
                        is_same_size = (
                            self.site.storage.getSize(inner_path)
                            == previous_size
                        )
                        ...
                        if is_same_size:
                            if is_text_file:
                                is_modified = (
                                    self.site.content_manager.isModified(
                                        inner_path
                                    )
                                )
                            else:
    ...
    def actionUserGetSettings(self, to):
        settings = self.user.sites.get(self.site.address, {}).get(
            "settings", {}
        )
    ...
    def actionServerUpdate(self, to):
            ...
            for websocket in self.server.websockets:
                websocket.cmd(
                    ...
                    [
                        "info",
                        _[
                            "Updating Peer-to-Peer Network client, will be"
        ...
        self.cmd(
            ...
            [
                _[
                    "Update <b>Peer-to-Peer Network client</b> to latest"
                    " version?"
                ],
                _["Update"],
            ],
    ...
    def actionServerSetPassiveMode(self, to, passive_mode=False):
        ...
        if file_server.isPassiveMode() != passive_mode:
            ...
            if file_server.isPassiveMode():
                self.cmd(
                    "notification", ["info", _["Passive mode enabled"], 5000]
                )
            else:
                self.cmd(
                    "notification", ["info", _["Passive mode disabled"], 5000]
                )
    ...
    def actionServerSetOfflineMode(self, to, offline_mode=False):
        ...
        if file_server.isOfflineMode() != offline_mode:
            ...
            if file_server.isOfflineMode():
                self.cmd(
                    "notification", ["info", _["Offline mode enabled"], 5000]
                )
            else:
                self.cmd(
                    "notification", ["info", _["Offline mode disabled"], 5000]
                )
            self.server.updateWebsocket()
    ...
    def actionServerShutdown(self, to, restart=False):
        ...
        if restart:
            message = [
                _["Restart <b>Peer-to-Peer Network client</b>?"],
                _["Restart"],
            ]
        else:
            message = [
                _["Shut down <b>Peer-to-Peer Network client</b>?"],
                _["Shut down"],
            ]
    ...
    def actionServerShowdirectory(self, to, directory="backup", inner_path=""):
        if self.request.env["REMOTE_ADDR"] != "127.0.0.1":
            return self.response(
                ...
                {
                    "error": (
                        "Only clients from 127.0.0.1 allowed to run this"
                        " command"
                    )
                },
            )
        ...
        if directory == "backup":
            ...
            path = os.path.abspath(
                self.site.storage.getPath(helper.getDirname(inner_path))
            )
    ...
    def actionConfigSet(self, to, key, value):
        ...
        if key not in config.keys_api_change_allowed:
            self.response(
                to, {"error": "Forbidden: You cannot set this config key"}
            )
            return
        ...
        if key == "language":
            ...
            message = (
                _[
                    "You have successfully changed the web interface's"
                    " language!"
                ]
                + "<br>"
            )
            message += _[
                "Due to the browser's caching, the full transformation could"
                " take some minute."
            ]
