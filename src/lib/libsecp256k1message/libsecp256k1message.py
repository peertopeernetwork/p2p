...
from util.Electrum import format as p2p_format
...
def sign_data(secretkey, byte_string):
    ...
    Return serialized signature compatible with Electrum (Peer-to-Peer Network).
    """
    # encode the message
    encoded = p2p_format(byte_string)
...
def recover_address(data, sign):
    ...
    publickey = recover_public_key(coincurve_sig(sign_bytes), p2p_format(data))
...
__all__ = [
    ...
    "compute_secret_address",
    "public_digest",
    "address_public_digest",
    "recover_public_key",
    "decode_secret_key",
    "sign_data",
    "verify_data",
...
if __name__ == "__main__":
    ...
    privatekey = decode_secret_key(
        b"5JsunC55XGVqFQj5kPGK4MWgTL26jKbnPhjnmchSNPo75XXCwtk"
    )
    threads = []
    for i in range(1000):
        data = bytes("hello", "utf_8")
