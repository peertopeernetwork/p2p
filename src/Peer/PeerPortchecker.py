...
class PeerPortchecker(object):
    ...
    def requestUrl(self, url, post_data=None):
        if type(post_data) is dict:
            post_data = urllib.parse.urlencode(post_data).encode("utf_8")
        req = urllib.request.Request(url, post_data)
        req.add_header("Referer", url)
        req.add_header(
            "User-Agent",
            (
                "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.11 (KHTML,"
    ...
    def portOpen(self, port):
        ...
        try:
            UpnpPunch.ask_to_open_port(
                port, "Peer-to-Peer Network", retries=3, protos=["TCP"]
            )
            self.upnp_port_opened = True
        except Exception as err:
            self.log.warning(
                "UpnpPunch run error: %s" % Debug.formatException(err)
            )
            return False
        return True
        ...
        for func_name in checker_functions:
            ...
            try:
                ...
                if res:
                    ...
                    if (
                        res["opened"]
                        and not self.file_server.had_external_incoming
                    ):
                        res["opened"] = False
                        self.log.warning(
                            "Port %s:%s looks opened, but no incoming"
                            " connection" % (res["ip"], port)
                        )
                    break
            except Exception as err:
                self.log.warning(
    ...
    def checkIpfingerprints(self, port):
        ...
        ip = re.match(
            r'.*name="remoteHost".*?value="(.*?)"', data, re.DOTALL
        ).group(1)
        post_data = {
            "remoteHost": ip,
            "start_port": port,
        ...
        message = (
            self.requestUrl(
                "https://www.ipfingerprints.com/scripts/getPortsInfo.php",
                post_data,
            )
            .read()
            .decode("utf_8")
    ...
    def checkMyaddr(self, port):
        ...
        data = self.requestUrl(url).read().decode("utf_8")
        ip = re.match(
        ...
        data = self.requestUrl(url, post_data).read().decode("utf_8")
        message = re.match(
    ...
    def checkPortchecker(self, port):  # Not working: Forbidden
        data = self.requestUrl("https://portchecker.co").read().decode("utf_8")
        csrf = re.match(
            r'.*name="_csrf" value="(.*?)"', data, re.DOTALL
        ).group(1)
        data = (
            self.requestUrl(
                "https://portchecker.co", {"port": port, "_csrf": csrf}
            )
            .read()
            .decode("utf_8")
        )
    ...
    def checkSubnetonline(self, port):  # Not working: Invalid response
        ...
        token = re.match(
            r'.*name="token".*?value="(.*?)"', data, re.DOTALL
        ).group(1)
        post_data = {
            "host": ip,
            "port": port,
