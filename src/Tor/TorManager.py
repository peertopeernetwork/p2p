...
class TorManager(object):
    ...
    def start(self):
        ...
        try:
            ...
            if sys.platform.startswith("win") and os.path.isfile(self.tor_exe):
                self.log.info(
                    "Starting self-bundled Tor, due to Tor proxy port %s check"
                    " error: %s" % (config.tor_proxy, err)
                )
                self.port = 65534
                self.proxy_port = 65533
                if config.tor == "always":
                    socks.setdefaultproxy(
                        socks.PROXY_TYPE_SOCKS5, "127.0.0.1", self.proxy_port
                    )
                self.enabled = True
                if not self.connect():
                    self.startTor()
                ...
                self.log.info(
                    "Disabling Tor, because we encountered issues while trying"
                    " to access Tor proxy at host:port %s: %s"
    ...
    def startTor(self):
        if sys.platform.startswith("win"):
            try:
                ...
                cmd = (
                    r"%s -f torrc --defaults-torrc torrc-defaults"
                    r" --ignore-missing-torrc" % self.tor_exe
                )
                if config.tor_use_bridges:
                    cmd += " --UseBridges 1"
                self.tor_process = subprocess.Popen(
                    cmd, cwd=tor_dir, close_fds=True, startupinfo=startupinfo
                )
    ...
    def isSubprocessRunning(self):
        return (
            self.tor_process
            and self.tor_process.pid
            and self.tor_process.poll() is None
        )
    ...
    def connectController(self):
        ...
        self.log.debug(
            "Connecting to Tor Controller %s:%s" % (self.ip, self.port)
        )
        self.connecting = True
        try:
            with self.lock:
                ...
                if float(version.replace(".", "0", 2)) < 407.10:
                    raise Exception(
                        "Tor version >=0.4.7.10 required, found: %s" % version
                    )
            ...
            self.log.warning(
                "Tor controller connect error: %s"
                % Debug.formatException(str(err))
            )
            self.enabled = False
        return self.conn
    ...
    def addOnion(self):
        if len(self.privatekeys) >= config.tor_hs_limit:
            return random.choice(
                [
                    key
                    for key in list(self.privatekeys.keys())
                    if key != self.site_onions.get("global")
                ]
            )
        ...
        if result:
            ...
            SiteManager.peer_blacklist.append(
                (onion_address + ".onion", self.fileserver_port)
            )
    ...
    def makeOnionAndKey(self):
        res = self.request(
            "ADD_ONION NEW:ED25519-V3 port=%s" % self.fileserver_port
        )
        match = re.search(
            "ServiceID=([A-Za-z0-9]+).*PrivateKey=ED25519-V3:(.*?)[\r\n]",
            res,
            re.DOTALL,
        )
        if match:
        ...
        for retry in range(2):
            try:
                ...
                while not back.endswith("250 OK\r\n"):
                    back += conn.recv(1024 * 64).decode("utf_8")
                break
            except Exception as err:
    ...
    def getOnion(self, site_address):
        ...
        if not onion:
            with self.lock:
                ...
                self.log.debug(
                    "Created new hidden service for %s: %s"
                    % (site_address, onion)
                )
        return onion
