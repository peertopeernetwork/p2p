...
class FileRequest(object):
    ...
    def route(self, cmd, req_id, params):
        ...
        if cmd == "update":
            ...
            RateLimit.callAsync(
                event,
                max(self.connection.bad_actions, 15),
                self.actionUpdate,
                params,
            )
        else:
                ...
                if self.connection.cpu_time > 0.5:
                    ...
                    if self.connection.cpu_time > 5:
                        self.connection.close(
                            "Cpu time: %.3fs" % self.connection.cpu_time
                        )
                s = time.time()
            if func:
                func(params)
            ...
            if cmd not in ["getFile", "streamFile"]:
                taken = time.time() - s
                taken_sent = (
                    self.connection.last_sent_time
                    - self.connection.last_send_time
                )
    ...
    def actionUpdate(self, params):
        ...
        current_content_modified = site.content_manager.contents.get(
            inner_path, {}
        ).get("modified", 0)
        body = params["body"]
        ...
        if (
            "modified" in params
            and params["modified"] <= current_content_modified
        ):
            should_validate_content = False
            valid = None
        elif not body:
            site.log.debug(
                "Missing body from update for file %s, downloading …"
                % inner_path
            )
            peer = site.addPeer(
                self.connection.ip,
            ...
            try:
                ...
                site.log.debug(
                    "Can't download updated file %s: %s" % (inner_path, err)
                )
                self.response(
                    {
                        "error": (
                            "File invalid update: Can't download updated file"
                        )
                    }
                )
                self.connection.badAction(5)
        ...
        if should_validate_content:
            try:
                ...
                site.log.debug(
                    "Update for %s is invalid JSON: %s" % (inner_path, err)
                )
            ...
            if self.server.files_parsing.get(file_uri):
                ...
                try:
                    valid = site.content_manager.verifyFile(
                        inner_path, content
                    )
                except Exception as err:
                    site.log.debug(
                        "Update for %s is invalid: %s" % (inner_path, err)
                    )
                    error = err
                    valid = False
        if valid is True:
            ...
            if inner_path.endswith("content.json"):
                ...
                site.onComplete.once(
                    lambda: site.publish(inner_path=inner_path, diffs=diffs),
                    "publish_%s" % inner_path,
                )

                def downloader():
                    site.downloadContent(
                        inner_path, peer=peer, diffs=params.get("diffs", {})
                    )
            ...
            if peer:
                ...
                if inner_path in site.content_manager.contents:
                    peer.last_content_json_update = (
                        site.content_manager.contents[inner_path]["modified"]
                    )
                if config.verbose:
                    site.log.debug(
                        "Same version, adding new peer for locked files: %s,"
                        " tasks: %s"
            ...
            self.response(
                {"error": "File %s invalid: %s" % (inner_path, error)}
            )
            self.connection.badAction(5)
    ...
    def handleGetFile(self, params, streaming=False):
        ...
        try:
            ...
            with file_obj as file:
                ...
                if file_size > read_bytes:
                    if not self.isReadable(
                        site, params["inner_path"], file, params["location"]
                    ):
                        raise RequestError(
                    ...
                    if (
                        ...
                        and params["file_size"] != file_size
                    ):
                ...
                if streaming:
                    back = {
                        ...
                        "stream_bytes": min(
                            read_bytes, file_size - params["location"]
                        ),
                    }
                    self.response(back)
                    ...
                    back = {
                        ...
                        "size": file_size,
                        "location": min(
                            file.tell() + file.read_bytes, file_size
                        ),
                    }
                    self.response(back, streaming=True)
                bytes_sent = min(read_bytes, file_size - params["location"])
                site.settings["bytes_sent"] = (
                    site.settings.get("bytes_sent", 0) + bytes_sent
                )
            if config.debug_socket:
                self.log.debug(
                    "File %s at position %s sent %s bytes"
                    % (file_path, params["location"], bytes_sent)
                )
            connected_peer = site.addPeer(
                self.connection.ip, self.connection.port, source="request"
            )
            if connected_peer:
            ...
            if config.verbose:
                self.log.debug(
                    "GetFile read error: %s" % Debug.formatException(err)
                )
            self.response({"error": "File read error"})
            return False
        except Exception as err:
            self.log.error(
                "GetFile exception: %s" % Debug.formatException(err)
            )
    ...
    def actionPex(self, params):
        ...
        if connected_peer:
            added += 1
            connected_peer.connect(self.connection)
        for packed_address in itertools.chain(
            params.get("peers", []), params.get("peers_ipv6", [])
        ):
        ...
        packed_peers = helper.packPeers(
            site.getConnectablePeers(
                params["need"], ignore=got_peer_keys, allow_private=False
            )
        )
        if added:
            ...
            if config.verbose:
                self.log.debug(
                    ...
                    % (
                        added,
                        site,
                        {key: len(val) for key, val in packed_peers.items()},
                    )
                )
        back = {
    ...
    def actionFindHashIds(self, params):
        ...
        if config.verbose:
            self.log.debug(
                ...
                % (
                    {key: len(val) for key, val in back.items()},
                    len(params["hash_ids"]),
                    time.time() - s,
                )
            )
    ...
    def actionCheckport(self, params):
        ...
        with closing(helper.createSocket(self.connection.ip)) as sock:
            ...
            if sock.connect_ex(sock_address) == 0:
                self.response(
                    {"status": "open", "ip_external": self.connection.ip}
                )
            else:
                self.response(
                    {"status": "closed", "ip_external": self.connection.ip}
                )
