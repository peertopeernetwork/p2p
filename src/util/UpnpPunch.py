...
def perform_m_search(local_ip):
    ...
    if local_ip == "127.0.0.1":
        ...
        sock.settimeout(5)
    try:
        return sock.recv(2048).decode("utf_8")
    except socket.error:
...
def _retrieve_igd_profile(url):
    ...
    try:
        return (
            urllib.request.urlopen(url.geturl(), timeout=5)
            .read()
            .decode("utf-8")
        )
    except socket.error:
...
def _parse_igd_profile(profile_xml):
    ...
    for service in service_types:
        if (
            _get_first_child_data(service).find("WANIPConnection") > 0
            or _get_first_child_data(service).find("WANPPPConnection") > 0
        ):
            try:
...
def _get_local_ips():
    ...
    local_ips = sorted(
        local_ips, key=lambda a: a.startswith("192"), reverse=True
    )
    return local_ips


def _create_open_message(
    local_ip,
    port,
...
def _parse_for_errors(soap_response):
    ...
    if soap_response.status >= 400:
        ...
        try:
            ...
            err_code = _get_first_child_data(
                err_dom.getElementsByTagName("errorCode")[0]
            )
...
def _send_soap_request(
    location, upnp_schema, control_path, soap_fn, soap_message
):
    """
    ...
    headers = {
        "SOAPAction": (
            '"urn:schemas-upnp-org:service:{schema}:1#{fn_name}"'.format(
                schema=upnp_schema, fn_name=soap_fn
            )
        ),
        "Content-Type": "text/xml",
    }
    logger.debug(
        "Sending UPnP request to {0}:{1}…".format(
...
def _send_requests(messages, location, upnp_schema, control_path):
    responses = [
        _send_soap_request(
            location, upnp_schema, control_path, message_tup[1], message_tup[0]
        )
...
def _communicate_with_igd(
    ...
    def job(local_ip):
        for retry in range(retries):
            try:
                ...
                logger.debug(
                    'Upnp request using "{0}" failed: {1}'.format(local_ip, e)
                )
                gevent.sleep(1)
        return False
    ...
    if not success:
        raise UpnpError(
            "Failed to communicate with igd using port {0} on local machine"
            " after {1} tries.".format(port, retries)
        )
    return success


def ask_to_open_port(
    port=55568, desc="UpnpPunch", retries=3, protos=("TCP", "UDP")
):
    logger.debug("Trying to open port %d." % port)
    return _communicate_with_igd(
        port=port,
        desc=desc,
        retries=retries,
        fn=_create_open_message,
...
if __name__ == "__main__":
    ...
    print(
        "Success:",
        ask_to_open_port(55568, "Peer-to-Peer Network", protos=["TCP"]),
    )
    print("Done in", time.time() - s)
    print("Closing port…")
    print(
        "Success:",
        ask_to_close_port(55568, "Peer-to-Peer Network", protos=["TCP"]),
    )
    print("Done in", time.time() - s)
