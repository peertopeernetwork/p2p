...
def isSafePattern(pattern):
    ...
    unsafe_pattern_match = re.search(r"[^.][*{+]", pattern)
    if unsafe_pattern_match:
        raise UnsafePatternError(
            "Potentially unsafe part of the pattern: %s in %s"
            % (unsafe_pattern_match.group(0), pattern)
        )
...
def compilePattern(pattern):
    ...
    if isSafePattern(pattern):
        ...
        log.debug(
            "Cache size: %d + %d"
            % (len(cached_patterns), len(old_cached_patterns))
        )
