...
def getOpensslPath():
    ...
    if sys.platform.startswith("win"):
        lib_paths = [
            ...
            os.path.join(
                os.path.dirname(sys.executable), "DLLs/libcrypto-1_1-x64.dll"
            ),
            os.path.join(
                os.path.dirname(sys.executable), "DLLs/libcrypto-1_1.dll"
            ),
        ]
        ...
        lib_paths = [
            "../runtime/lib/libcrypto.so.1.1",
            "../../Frameworks/libcrypto.1.1.dylib",
    ...
    if "LD_LIBRARY_PATH" in os.environ:
        ...
        for path in lib_dir_paths:
            try:
                return [
                    lib for lib in os.listdir(path) if "libcrypto.so" in lib
                ][0]
            except Exception as err:
                logging.debug(
                    "OpenSSL lib not found in: %s (%s)" % (path, err)
                )
    lib_path = (
