...
if "handle_error" in dir(gevent.hub.Hub):
    ...
    logging.debug(
        "gevent.hub.Hub.handle_error not found using old gevent hooks"
    )
...
def handleGreenletError(context, type, value, tb):
    if (
        context.__class__ is tuple
        and context[0].__class__.__name__ == "ThreadPool"
    ):
        return None
...
try:
    signal.signal(
        signal.SIGTERM, lambda signum, stack_frame: shutdown("SIGTERM")
    )
except Exception as err:
