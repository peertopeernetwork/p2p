...
if config.debug and config.action == "main":
    try:
        ...
        logging.debug(
            "Watchdog fs listener detected, source code autoreload enabled"
        )
        enabled = True
...
class DebugReloader:
    ...
    def onChanged(self, evt):
        ...
        if (
            ext not in ["py", "json"]
            or "Test" in path
            or time.time() - self.last_changed < 1.0
        ):
            return False
        ...
        if os.path.isfile(path):
            ...
            time_modified = 0
        self.log.debug(
            "File changed: %s reloading source code (modified %.3fs ago)"
            % (evt, time.time() - time_modified)
        )
