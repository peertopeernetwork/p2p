...
def findfiles(path, find_ext):
    ...
    for root, dirs, files in sorted(
        os.walk(path, topdown=False), key=functools.cmp_to_key(sorter)
    ):
...
def merge(merged_path):
    ...
    logging.debug(
        "Merging %s (changed: %s, old parts: %s)"
        % (merged_path, changed, len(old_parts))
    )
    parts = []
    s_total = time.time()
    for file_path in findfiles(merge_dir, find_ext):
        ...
        parts.append(
            b"\n/* ---- %s ---- */\n\n" % file_relative_path.encode("utf_8")
        )
