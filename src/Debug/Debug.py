...
python_lib_dirs = [
    ...
    if re.sub(r".*[\\/]", "", path) in ("site-packages", "dist-packages")
]
python_lib_dirs.append(os.path.dirname(os.__file__).replace("\\", "/"))
        ...
        if path.startswith("src/gevent/"):
            ...
            for base in python_lib_dirs:
                ...
                for base in (
                    ...
                    file_title = path
                    for needle in ("/p2p/", "/core/"):
                        if needle in file_title.lower():
                            file_title = (
                                "?/"
                                + file_title[
                                    file_title.lower().rindex(needle)
                                    + len(needle) :
                                ]
                            )
...
def formatException(err=None, format="text"):
    ...
    if type(err) == Notify:
        return err
    elif type(err) == tuple and err and err[0] is not None:
    ...
    if not err:
        if hasattr(err, "message"):
            ...
            err = exc_obj
    tb = formatTraceback(
        [[frame[0], frame[1]] for frame in traceback.extract_tb(exc_tb)]
    )
    if format == "html":
        return "%s: %s<br><small class='multiline'>%s</small>" % (
            repr(err),
            err,
            " > ".join(tb),
        )
...
def formatStack(limit=None):
    ...
    tb = formatTraceback(
        [[frame[1], frame[2]] for frame in inspect.stack()[1:]], limit=limit
    )
    return " > ".join(tb)
...
def testBlock():
    ...
    while 1:
        ...
        if time.time() - last_time > 1.1:
            logging.debug(
                "Gevent block detected: %.3fs" % (time.time() - last_time - 1)
            )
            num_block += 1
        last_time = time.time()
