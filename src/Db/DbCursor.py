...
class DbCursor:
            ...
            if query_type in ("SELECT", "DELETE", "UPDATE"):
                ...
                for key, value in params.items():
                    if type(value) is list:
                        ...
                        if len(value) > 100:
                            query_values = ",".join(
                                map(helper.sqlquote, value)
                            )
                        else:
                            query_values = ",".join(["?"] * len(value))
                            values += value
                        query_wheres.append(
                            "%s %s (%s)" % (field, operator, query_values)
                        )
                    else:
                        if key.startswith("not__"):
                            query_wheres.append(
    ...
    def execute(self, query, params=None):
        ...
        if self.db.lock.locked():
            self.db.log.debug(
                "Locked for %.3fs" % (time.time() - self.db.lock.time_lock)
            )
        try:
            s = time.time()
        ...
        if self.logging or taken_query > 1:
            if params:
                self.db.log.debug(
                    ...
                    + " "
                    + str(params)
                    + " (Done in %.4f)" % (time.time() - s)
                )
            else:
                self.db.log.debug(
                    "Query: " + query + " (Done in %.4f)" % (time.time() - s)
                )
    ...
    def executemany(self, query, params):
        ...
        if self.logging or taken_query > 0.1:
            self.db.log.debug(
                "Execute many: %s (Done in %.4f)" % (query, taken_query)
            )
        self.db.need_commit = True
        return cursor

    def insertOrUpdate(self, table, query_sets, query_wheres, oninsert=None):
        if oninsert is None:
            oninsert = {}
        ...
        res = self.execute(
            ...
            % (table, ", ".join(sql_sets), " AND ".join(sql_wheres)),
            params,
        )
        if res.rowcount == 0:
    ...
    def createTable(self, table, cols):
        ...
        self.execute(
            "CREATE TABLE %s (%s)" % (table, ",".join(col_definitions))
        )
        return True
    ...
    def createIndexes(self, table, indexes):
        for index in indexes:
            if not index.strip().upper().startswith("CREATE"):
                self.db.log.error(
                    "Index command should start with CREATE: %s" % index
                )
                continue
            self.execute(index)
    ...
    def needTable(self, table, cols, indexes=None, version=1):
        ...
        if int(current_version) < int(version):
            self.db.log.debug(
                "Table %s outdated…version: %s need: %s, rebuilding…"
    ...
    def getJsonRow(self, file_path):
        ...
        if self.db.schema["version"] == 1:
            res = self.execute(
                "SELECT * FROM json WHERE ? LIMIT 1", {"path": file_path}
            )
            row = res.fetchone()
            if not row:
            ...
            res = self.execute(
                "SELECT * FROM json WHERE ? LIMIT 1",
            ...
            site_address, directory = re.match(
                "^([^/]*)/(.*)$", directory
            ).groups()
            res = self.execute(
                "SELECT * FROM json WHERE ? LIMIT 1",
            ...
            raise Exception(
                "Dbschema version %s not supported"
                % self.db.schema.get("version")
            )
        return row
