...
class Db(object):
    ...
    def connect(self):
        ...
        try:
            ...
            self.log.debug(
                "Connect by thread: %s" % threading.current_thread().ident
            )
    ...
    def commit(self, reason="Unknown"):
        ...
        try:
            ...
            self.conn.commit()
            self.log.debug(
                "Committed in %.3fs (reason: %s)" % (time.time() - s, reason)
            )
            return True
        except Exception as err:
            if "SQL statements in progress" in str(err):
                self.log.warning(
                    "Commit delayed: %s (reason: %s)"
                    % (Debug.formatException(err), reason)
                )
            else:
                self.log.error(
                    "Commit error: %s (reason: %s)"
                    % (Debug.formatException(err), reason)
                )
            return False
        finally:
    ...
    def processDelayed(self):
        ...
        if len(self.delayed_queue) > 10:
            self.log.debug(
                "Processed %s delayed queue in %.3fs"
                % (len(self.delayed_queue), time.time() - s)
            )
        self.delayed_queue = []
        self.delayed_queue_thread = None
    ...
    def close(self, reason="Unknown"):
        ...
        self.cur = None
        self.log.debug(
            "%s closed (reason: %s) in %.3fs, opened: %s"
            % (self.db_path, reason, time.time() - s, len(opened_dbs))
        )
        self.connect_lock.release()
        return True
    ...
    def checkTables(self):
        ...
        if "json" not in self.schema.get("tables", {}):
            if self.schema["version"] == 1:
                ...
                changed = cur.needTable(
                    ...
                    [
                        "CREATE UNIQUE INDEX path ON json(directory, site,"
                        " file_name)"
                    ],
                    version=self.schema["version"],
                )
        ...
        for table_name, table_settings in self.schema.get(
            "tables", {}
        ).items():
            try:
                ...
                self.log.error(
                    ...
                    % (table_name, Debug.formatException(err))
                )
                raise DbTableError(err, table_name)
        self.log.debug(
            "Db check done in %.3fs, changed tables: %s"
            % (time.time() - s, changed_tables)
        )
        if changed_tables:
            self.db_keyvalues = {}
    ...
    def updateJson(self, file_path, file=None, cur=None):
        ...
        try:
            ...
            if file is False:
                ...
                if sys.version_info.major == 3 and sys.version_info.minor < 6:
                    data = json.loads(file.read().decode("utf_8"))
                else:
                    data = json.load(file)
        except Exception as err:
        ...
        if not data or [
            ...
            if "to_keyvalue" in dbmap or "to_table" in dbmap
        ]:
            json_row = cur.getJsonRow(relative_path)
        for dbmap in matched_maps:
            if dbmap.get("to_keyvalue"):
                res = cur.execute(
                    "SELECT * FROM keyvalue WHERE json_id = ?",
                    (json_row["json_id"],),
                )
                current_keyvalue = {}
                current_keyvalue_id = {}
                ...
                for key in dbmap["to_keyvalue"]:
                    if key not in current_keyvalue:
                        ...
                        cur.execute(
                            (
                                "UPDATE keyvalue SET value = ? WHERE"
                                " keyvalue_id = ?"
                            ),
                            (data.get(key), current_keyvalue_id[key]),
                        )
            if dbmap.get("to_json_table"):
                directory, file_name = re.match(
                    "^(.*?)/*([^/]*)$", relative_path
                ).groups()
                data_json_row = dict(
                    cur.getJsonRow(
                        directory + "/" + dbmap.get("file_name", file_name)
                    )
                )
                changed = False
                for key in dbmap["to_json_table"]:
                    if data.get(key) != data_json_row.get(key):
                        changed = True
                if changed:
                    data_json_row.update(
                        {
                            key: val
                            for key, val in data.items()
                            if key in dbmap["to_json_table"]
                        }
                    )
            ...
            for table_settings in dbmap.get("to_table", []):
                ...
                if not import_cols:
                    import_cols = set(
                        [
                            item[0]
                            for item in self.schema["tables"][table_name][
                                "cols"
                            ]
                        ]
                    )
                cur.execute(
                    "DELETE FROM %s WHERE json_id = ?" % table_name,
                    (json_row["json_id"],),
                )
                if node not in data:
                    continue
                if key_col:
                    ...
                    for row in data[node]:
                        ...
                        cur.execute(
                            "INSERT OR REPLACE INTO %s ?" % table_name, row
                        )
        if not data:
            self.log.debug("Cleanup json row for %s" % file_path)
            cur.execute(
                "DELETE FROM json WHERE json_id = %s" % json_row["json_id"]
            )
        return True
...
if __name__ == "__main__":
    ...
    dbjson = Db(json.load(open("p2p.schema.json")), "data/users/p2p.db")
