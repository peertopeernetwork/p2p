...
class Config(object):
    def __init__(self, argv):
        ...
        self.config_file = self.start_dir + "/p2p.conf"
    ...
    def createParser(self):
        ...
        self.subparsers = self.parser.add_subparsers(
            title="Action to perform", dest="action"
        )
    ...
    def getStartDir(self):
        ...
        if "--start_dir" in self.argv:
            ...
            if (
                this_file.startswith("/Application")
                or this_file.startswith("/private")
                or this_file.startswith(os.path.expanduser("~/Library"))
            ):
                start_dir = os.path.expanduser(
                    "~/Library/Application Support/p2p"
                )
            else:
                start_dir = re.sub(
                    "/[^/]+/Contents/Resources/core/src/Config.py",
                    "",
                    this_file,
                )
        elif this_file.endswith("/core/src/Config.py"):
            start_dir = this_file.replace("/core/src/Config.py", "")
        elif this_file.endswith("usr/share/p2p/src/Config.py"):
            start_dir = os.path.expanduser("~/Peer-to-Peer-Network")
        else:
            start_dir = "."
        return start_dir
    ...
    def createArguments(self):
        ...
        try:
            ...
            if language not in ["pt-br", "zh-hans", "zh-hant"]:
        ...
        config_file = self.start_dir + "/p2p.conf"
        data_dir = self.start_dir + "/data"
        log_dir = self.start_dir + "/log"
        ip_local = ["127.0.0.1", "::1"]
        action = self.subparsers.add_parser(
            "main", help="Start UiServer and FileServer (default)"
        )
        action = self.subparsers.add_parser(
            "siteCreate", help="Create a new site"
        )
        action.register("type", "bool", self.strToBool)
        action.add_argument(
            "--use_master_seed",
            help=(
                "Allow created site's private key to be recovered using the"
                " master seed in users.json (default: True)"
            ),
            type="bool",
            choices=[True, False],
            default=False,
        ...
        action = self.subparsers.add_parser(
            "siteNeedFile", help="Get a file from site"
        )
        action.add_argument("address", help="Site address")
        action.add_argument("inner_path", help="File inner path")
        action = self.subparsers.add_parser(
            "siteDownload", help="Download a new site"
        )
        action.add_argument("address", help="Site address")
        action = self.subparsers.add_parser(
            "siteSign",
            help="Update and sign content.json: address [privatekey]",
        )
        action.add_argument("address", help="Site to sign")
        action.add_argument(
            "privatekey",
            help="Private key (default: ask on execute)",
            nargs="?",
        )
        action.add_argument(
            "--inner_path",
            help="File you want to sign (default: content.json)",
            default="content.json",
            metavar="inner_path",
        )
        action.add_argument(
            "--remove_missing_optional",
            help="Remove optional files that is not present in the directory",
            action="store_true",
        )
        action.add_argument(
            "--publish",
            help="Publish site after the signing",
            action="store_true",
        )
        action = self.subparsers.add_parser(
            "sitePublish", help="Publish site to other peers: address"
        )
        action.add_argument("address", help="Site to publish")
        action.add_argument(
            "peer_ip",
            help="Peer ip to publish (default: random peers ip from tracker)",
            default=None,
            nargs="?",
        )
        action.add_argument(
            "peer_port",
            help=(
                "Peer port to publish (default: random peer port from tracker)"
            ),
            default=55566,
            nargs="?",
        )
        action.add_argument(
            "--inner_path",
            help="Content.json you want to publish (default: content.json)",
            default="content.json",
            metavar="inner_path",
        )
        action = self.subparsers.add_parser(
            "siteVerify", help="Verify site files using sha512: address"
        )
        action.add_argument("address", help="Site to verify")
        action = self.subparsers.add_parser(
            "siteCmd", help="Execute a P2PFrame API command on a site"
        )
        action.add_argument("address", help="Site address")
        action.add_argument("cmd", help="API command name")
        action.add_argument(
            "parameters", help="Parameters of the command", nargs="?"
        )
        action = self.subparsers.add_parser(
            "dbRebuild", help="Rebuild site database cache"
        )
        action.add_argument("address", help="Site to rebuild")
        action = self.subparsers.add_parser(
            "dbQuery", help="Query site sql cache"
        )
        action.add_argument("address", help="Site to query")
        action.add_argument("query", help="Sql query")
        action = self.subparsers.add_parser(
            "peerPing", help="Send Ping command to peer"
        )
        action.add_argument("peer_ip", help="Peer ip")
        action.add_argument("peer_port", help="Peer port", nargs="?")
        action = self.subparsers.add_parser(
            "peerGetFile", help="Request and print a file content from peer"
        )
        action.add_argument("peer_ip", help="Peer ip")
        ...
        action.add_argument(
            "--benchmark",
            help="Request file 10x then displays the total time",
            action="store_true",
        )
        action = self.subparsers.add_parser(
            "peerCmd", help="Request and print a file content from peer"
        )
        action.add_argument("peer_ip", help="Peer ip")
        action.add_argument("peer_port", help="Peer port")
        action.add_argument("cmd", help="Command to execute")
        action.add_argument(
            "parameters", help="Parameters to command", nargs="?"
        )
        action = self.subparsers.add_parser(
            "cryptSign", help="Sign message using Bitcoin private key"
        )
        action.add_argument("message", help="Message to sign")
        action.add_argument("privatekey", help="Private key")
        action = self.subparsers.add_parser(
            "cryptVerify", help="Verify message using Bitcoin public address"
        )
        ...
        action = self.subparsers.add_parser(
            "cryptGetPrivatekey", help="Generate a privatekey from master seed"
        )
        action.add_argument("master_seed", help="Source master seed")
        action.add_argument(
            "site_address_index", help="Site address index", type=int
        )
        action = self.subparsers.add_parser(
            "getConfig", help="Return json-encoded info"
        )
        ...
        self.test_parser = self.subparsers.add_parser(
            "test", help="Run a test"
        )
        self.test_parser.add_argument("test_name", help="Test name", nargs="?")
        self.parser.add_argument(
            "--verbose", help="More detailed logging", action="store_true"
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "--silent",
            help="Only log errors to terminal output",
            action="store_true",
        )
        self.parser.add_argument(
            "--debug_socket",
        ...
        self.parser.add_argument(
            "--start_dir",
            help="Path of working dir for variable content (data, log, .conf)",
            default=self.start_dir,
            metavar="path",
        )
        self.parser.add_argument(
            "--config_file",
        ...
        self.parser.add_argument(
            ...
            default="default",
            choices=["default", "DEBUG", "INFO", "ERROR", "off"],
        )
        self.parser.add_argument(
            "--log_dir",
        ...
        self.parser.add_argument(
            ...
            default="daily",
            choices=["hourly", "daily", "weekly", "off"],
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            ...
            default="1PqVbR12CzLasYAUkDi9LAbZHNo2W7cVhz",
            metavar="address",
        )
        self.parser.add_argument(
            "--updatesite",
            help="Source code update site",
            default="1DGnwuE6Qc2SKz7CoS3kxm7J7rSXCgJnF7",
            metavar="address",
        )
        self.parser.add_argument(
            "--dist_type",
        ...
        self.parser.add_argument(
            "--file_size_limit",
            help="Maximum per file size limit in MB",
            default=21,
            type=int,
            metavar="limit",
        )
        self.parser.add_argument(
            "--connected_limit",
            help="Max number of connected peers per site. Soft limit.",
            default=21,
            type=int,
            metavar="connected_limit",
        )
        self.parser.add_argument(
            "--global_connected_limit",
            help="Max number of connections. Soft limit.",
            default=512,
            type=int,
            metavar="global_connected_limit",
        )
        self.parser.add_argument(
            "--workers",
            help="Download workers per site",
            default=5,
            type=int,
            metavar="workers",
        )
        self.parser.add_argument(
            ...
            help=(
                "Site announce interval for the most active sites, in minutes."
            ),
            default=4,
            type=int,
            metavar="site_announce_interval_min",
        )
        self.parser.add_argument(
            "--site_announce_interval_max",
            help="Site announce interval for inactive sites, in minutes.",
            default=30,
            type=int,
            metavar="site_announce_interval_max",
        )
        self.parser.add_argument(
            "--site_peer_check_interval_min",
            help=(
                "Connectable peers check interval for the most active sites,"
                " in minutes."
            ),
            default=5,
            type=int,
            metavar="site_peer_check_interval_min",
        )
        self.parser.add_argument(
            "--site_peer_check_interval_max",
            help=(
                "Connectable peers check interval for inactive sites, in"
                " minutes."
            ),
            default=20,
            type=int,
            metavar="site_peer_check_interval_max",
        )
        self.parser.add_argument(
            "--site_update_check_interval_min",
            help=(
                "Site update check interval for the most active sites, in"
                " minutes."
            ),
            default=5,
            type=int,
            metavar="site_update_check_interval_min",
        )
        self.parser.add_argument(
            "--site_update_check_interval_max",
            help="Site update check interval for inactive sites, in minutes.",
            default=45,
            type=int,
            metavar="site_update_check_interval_max",
        )
        self.parser.add_argument(
            "--site_connectable_peer_count_max",
            help=(
                "Search for as many connectable peers for the most active"
                " sites"
            ),
            default=21,
            type=int,
            metavar="site_connectable_peer_count_max",
        )
        self.parser.add_argument(
            "--site_connectable_peer_count_min",
            help="Search for as many connectable peers for inactive sites",
            default=2,
            type=int,
            metavar="site_connectable_peer_count_min",
        )
        self.parser.add_argument(
            "--send_back_lru_size",
            help="Size of the send back LRU cache",
            default=5000,
            type=int,
            metavar="send_back_lru_size",
        )
        self.parser.add_argument(
            "--send_back_limit",
            help=(
                "Send no more than so many files at once back to peer, when we"
                " discovered that the peer held older file versions"
            ),
            default=3,
            type=int,
            metavar="send_back_limit",
        )
        self.parser.add_argument(
            "--expose_no_ownership",
            help=(
                "By default, Peer-to-Peer Network tries checking updates for"
                " own sites more frequently. This can be used by a third party"
                " for revealing the network addresses of a site owner. If this"
                " option is enabled, Peer-to-Peer Network performs the checks"
                " in the same way for any sites."
            ),
            type="bool",
            choices=[True, False],
            default=False,
        ...
        self.parser.add_argument(
            "--simultaneous_connection_throttle_threshold",
            help=(
                "Throttle opening new connections when the number of outgoing"
                " connections in not fully established state exceeds the"
                " threshold."
            ),
            default=15,
            type=int,
            metavar="simultaneous_connection_throttle_threshold",
        )
        self.parser.add_argument(
            "--fileserver_ip",
        ...
        self.parser.add_argument(
            "--fileserver_port",
            help="FileServer bind port (0: randomize)",
            default=0,
            type=int,
            metavar="port",
        )
        self.parser.add_argument(
            "--fileserver_port_range",
        ...
        self.parser.add_argument(
            "--ip_external",
            help="Set reported external ip (tested on start if None)",
            metavar="ip",
            nargs="*",
        )
        self.parser.add_argument(
            "--offline",
            help="Disable network communication",
            action="store_true",
        )
        self.parser.add_argument(
            "--disable_udp",
            help="Disable UDP connections",
            action="store_true",
        )
        self.parser.add_argument(
            "--proxy", help="Socks proxy address", metavar="ip:port"
        )
        self.parser.add_argument(
            "--bind",
            help="Bind outgoing sockets to this address",
            metavar="ip",
        )
        self.parser.add_argument(
            "--trackers",
            help="Bootstrapping torrent trackers",
            default=trackers,
        ...
        self.parser.add_argument(
            "--trackers_file",
            help="Load torrent trackers dynamically from a file",
            metavar="path",
            nargs="*",
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "--use_libsecp256k1",
            help="Use Libsecp256k1 library for speedup",
            type="bool",
            choices=[True, False],
            default=True,
        )
        self.parser.add_argument(
            "--use_openssl",
            help="Use OpenSSL library for speedup",
            type="bool",
            choices=[True, False],
            default=True,
        )
        self.parser.add_argument(
            "--openssl_lib_file",
            help="Path for OpenSSL library file (default: detect)",
            default=argparse.SUPPRESS,
            metavar="path",
        )
        self.parser.add_argument(
            "--openssl_bin_file",
            help="Path for OpenSSL binary file (default: detect)",
            default=argparse.SUPPRESS,
            metavar="path",
        )
        self.parser.add_argument(
            "--disable_db",
            help="Disable database updating",
            action="store_true",
        )
        self.parser.add_argument(
            "--disable_encryption",
            help="Disable connection encryption",
            action="store_true",
        )
        self.parser.add_argument(
            "--force_encryption",
            help="Enforce encryption to all peer connections",
            action="store_true",
        )
        self.parser.add_argument(
            "--disable_sslcompression",
            help="Disable SSL compression to save memory",
            type="bool",
            choices=[True, False],
            default=True,
        )
        self.parser.add_argument(
            "--keep_ssl_cert",
            help="Disable new SSL cert generation on startup",
            action="store_true",
        )
        self.parser.add_argument(
            "--max_files_opened",
            help=(
                "Change maximum opened files allowed by OS to this value on"
                " startup"
            ),
            default=2048,
            type=int,
        ...
        self.parser.add_argument(
            "--use_tempfiles",
            help="Use temporary files when downloading (experimental)",
            type="bool",
            choices=[True, False],
            default=False,
        )
        self.parser.add_argument(
            "--stream_downloads",
            help="Stream download directly to files (experimental)",
            type="bool",
            choices=[True, False],
            default=False,
        )
        self.parser.add_argument(
            "--msgpack_purepython",
            help="Use less memory, but a bit more CPU power",
            type="bool",
            choices=[True, False],
            default=False,
        )
        self.parser.add_argument(
            ...
            type="bool",
            choices=[True, False],
            default=fix_float_decimals,
        )
        self.parser.add_argument(
            "--db_mode", choices=["speed", "security"], default="speed"
        )
        self.parser.add_argument(
            "--threads_fs_read",
            help="Number of threads for file read operations",
            default=1,
            type=int,
        )
        self.parser.add_argument(
            "--threads_fs_write",
            help="Number of threads for file write operations",
            default=1,
            type=int,
        )
        self.parser.add_argument(
            "--threads_crypt",
            help="Number of threads for cryptographic operations",
            default=2,
            type=int,
        )
        self.parser.add_argument(
            "--threads_db",
            help="Number of threads for database operations",
            default=1,
            type=int,
        )
        self.parser.add_argument(
            "--download_optional", choices=["manual", "auto"], default="manual"
        )
        self.parser.add_argument(
            "--tor",
            help=(
                "enable: Use only for Tor peers, always: Use Tor for every"
                " connection"
            ),
        ...
        self.parser.add_argument(
            "--tor_controller",
            help="Tor controller address",
            metavar="ip:port",
            default="127.0.0.1:9051",
        )
        self.parser.add_argument(
            "--tor_proxy",
            help="Tor proxy address",
            metavar="ip:port",
        ...
        self.parser.add_argument(
            "--tor_use_bridges",
            help="Use obfuscated bridge relays to avoid Tor block",
            action="store_true",
        )
        self.parser.add_argument(
            "--tor_hs_limit",
            help="Maximum number of hidden services in Tor always mode",
            metavar="limit",
            type=int,
            default=10,
        )
        ...
        self.parser.add_argument(
            "--version",
            action="version",
            version="Peer-to-Peer Network %s r%s" % (self.version, self.rev),
        )
        self.parser.add_argument(
            "--end",
            help="Stop multi value argument parsing",
            action="store_true",
        )
        return self.parser
    ...
    def loadTrackersFile(self):
        ...
        for trackers_file in self.trackers_file:
            try:
                if trackers_file.startswith("/"):
                    trackers_file_path = trackers_file
                elif trackers_file.startswith("{data_dir}"):
                    trackers_file_path = trackers_file.replace(
    ...
    def moveUnknownToEnd(self, argv, default_action):
        valid_actions = sum(
            [action.option_strings for action in self.parser._actions], []
        )
        valid_parameters = []
        plugin_parameters = []
        plugin = False
    ...
    def parse(self, silent=False, parse_config=True):
        ...
        if silent:
            ...
            current_parser._print_message = lambda *args, **kwargs: silencer(
                current_parser, "_print_message"
            )
            current_parser.exit = lambda *args, **kwargs: silencer(
                current_parser, "exit"
            )
        ...
        if not silent:
            if (
                self.fileserver_ip != "*"
                and self.fileserver_ip not in self.ip_local
            ):
                self.ip_local.append(self.fileserver_ip)
        if silent:
            if current_parser.exited and self.action == "main":
                self.action = None
    ...
    def saveValue(self, key, value):
        ...
        if key_line_i and len(lines) > key_line_i + 1:
            while True:
                is_value_line = lines[key_line_i + 1].startswith(" ") or lines[
                    key_line_i + 1
                ].startswith("\t")
    ...
    def initFileLogger(self):
        ...
        if self.log_rotate == "off":
            ...
            file_logger = logging.handlers.TimedRotatingFileHandler(
                log_file_path,
                when=when_names[self.log_rotate],
                interval=1,
                backupCount=self.log_rotate_backup_count,
                encoding="utf_8",
            )
    ...
    def initLogging(self, console_logging=None, file_logging=None):
        ...
        if not os.path.isdir(self.log_dir):
            ...
            try:
                os.chmod(
                    self.log_dir, stat.S_IRUSR | stat.S_IWUSR | stat.S_IXUSR
                )
            except Exception as err:
                print(
                    "Can't change permission of %s: %s" % (self.log_dir, err)
                )
        logging.WARNING = 15
