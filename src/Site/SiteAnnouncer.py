...
class SiteAnnouncer(object):
    def __init__(self, site):
        ...
        self.log = logging.getLogger(
            "Site:%s SiteAnnouncer" % self.site.address_short
        )
        self.stats = {}
    ...
    def getSupportedTrackers(self):
        ...
        trackers = [
            tracker for tracker in trackers if self.getAddressParts(tracker)
        ]
        if "ipv6" not in self.connection_server.supported_ip_types:
            trackers = [
                tracker
                for tracker in trackers
                if self.connection_server.getIpType(
                    self.getAddressParts(tracker)["ip"]
                )
                != "ipv6"
            ]
        return trackers
    ...
    def getAnnouncingTrackers(self, mode, force):
        ...
        if trackers and (mode == "update" or mode == "more"):
            ...
            while 1:
                ...
                if not self.shouldTrackerBeTemporarilyIgnored(
                    tracker, mode, force
                ):
    ...
    def getOpenedServiceTypes(self):
        ...
        if config.trackers_proxy == "disable" and config.tor != "always":
            for ip_type, opened in list(
                self.connection_server.port_opened.items()
            ):
                if opened:
                    back.append(ip_type)
    ...
    def announce(self, force=False, mode="start", pex=True):
        ...
        if time.time() - self.time_last_announce < 30 and not force:
            return
        self.log.debug(
            "announce: force=%s, mode=%s, pex=%s" % (force, mode, pex)
        )
        self.fileserver_port = config.fileserver_port
    ...
    def getAddressParts(self, tracker):
        if "://" not in tracker or not re.match(
            "^[A-Za-z0-9:/.#-]+$", tracker
        ):
            return None
    ...
    def announceTracker(self, tracker, mode="start", num_want=10):
        ...
        if config.verbose:
            self.log.debug(
                "Tracker announcing to %s (mode: %s)" % (tracker, mode)
            )
        if mode == "update":
            num_want = 10
        else:
        ...
        try:
            if handler:
                peers = handler(
                    address_parts["address"], mode=mode, num_want=num_want
                )
            else:
                raise AnnounceError(
                    "Unknown protocol: %s" % address_parts["protocol"]
                )
        except Exception as err:
            self.log.warning(
                "Tracker %s announce failed: %s in mode %s"
                % (tracker, Debug.formatException(err), mode)
            )
            error = err
        if error:
    ...
    def announceToTrackers(self, trackers, force=False, mode="start"):
        ...
        num_announced = 0
        for tracker in trackers:
            thread = self.site.greenlet_manager.spawn(
                self.announceTracker, tracker, mode=mode
            )
        ...
        if len(errors) < len(threads):
            ...
            if len(threads) > 1:
                self.log.error(
                    "Announce to %s trackers in %.3fs, failed"
                    % (len(threads), time.time() - s)
                )
            if len(threads) > 1 and mode != "start":
                self.log.debug("Tracker failed, skipping to next one…")
                self.site.greenlet_manager.spawnLater(
                    5.0, self.announce, force=force, mode=mode, pex=False
                )
        self.updateWebsocket(trackers="announced")

    @util.Noparallel(blocking=False)
    def announcePex(
        self, query_num=2, need_num=10, establish_connections=True
    ):
        peers = []
        try:
            ...
            for peer in peers:
                ...
                time.sleep(0.1)
            self.log.debug(
                "Pex result: from %s peers got %s new peers."
                % (done, total_added)
            )
        finally:
            if len(peers) > 0:
