...
thread_pool_fs_read = ThreadPool.ThreadPool(
    config.threads_fs_read, name="FS read"
)
thread_pool_fs_write = ThreadPool.ThreadPool(
    config.threads_fs_write, name="FS write"
)
...
class VerifyFiles_Notificator(object):
    ...
    def inc(self):
        ...
        if (
            self.websocket_update_time + self.websocket_update_interval
            < time.time()
        ):
            self.send()
...
class SiteStorage(object):
        ...
        if self.has_db:
            ...
            if not os.path.isfile(db_path) or os.path.getsize(db_path) == 0:
                try:
                    ...
                    pass
            if self.db:
                self.db.close("Getting new db for SiteStorage")
    ...
    def getDbFiles(self):
        ...
        for (
            content_inner_path,
            content,
        ) in self.site.content_manager.contents.items():
            ...
            for file_relative_path in list(
                ...
                if not file_relative_path.endswith(
                    ".json"
                ) and not file_relative_path.endswith("json.gz"):
                    continue
        ...
        try:
            if num_total > 100:
                self.site.messageWebsocket(
                    _[
                        "Database rebuilding…<br>Imported {0} of {1} files"
                        " (error: {2})…"
                    ].format("0000", num_total, num_error),
                    "rebuild",
                    0,
                )
            for file_inner_path, file_path in db_files:
                try:
                    if self.updateDbFile(
                        file_inner_path, file=open(file_path, "rb"), cur=cur
                    ):
                        num_imported += 1
                except Exception as err:
                    self.log.error(
                ...
                if num_imported and num_imported % 100 == 0:
                    self.site.messageWebsocket(
                        _[
                            "Database rebuilding…<br>Imported {0} of {1} files"
                            " (error: {2})…"
                        ].format(num_imported, num_total, num_error),
                        "rebuild",
            ...
            self.log.info(
                "Rebuild: Imported %s data file in %.3fs"
                % (num_imported, time.time() - s)
            )
    ...
    def query(self, query, params=None):
        ...
        try:
            ...
            if err.__class__.__name__ == "DatabaseError":
                self.log.error(
                    "Database error: %s, query: %s, try to rebuilding it…"
                    % (err, query)
                )
                try:
    ...
    def rename(self, inner_path_before, inner_path_after):
        for retry in range(3):
            ...
            try:
                os.rename(
                    self.getPath(inner_path_before),
                    self.getPath(inner_path_after),
                )
                break
            except Exception as err:
                rename_err = err
                self.log.error(
                    "%s rename error: %s (retry #%s)"
                    % (inner_path_before, err, retry)
                )
    ...
    def walk(self, dir_inner_path, ignore=None):
        ...
        for root, dirs, files in os.walk(directory):
            root = root.replace("\\", "/")
            root_relative_path = re.sub(
                "^%s" % re.escape(directory), "", root
            ).lstrip("/")
    ...
    def onUpdated(self, inner_path, file=None):
        should_load_to_db = inner_path.endswith(
            ".json"
        ) or inner_path.endswith(".json.gz")
        if inner_path == "dbschema.json":
            ...
            if config.verbose:
                self.log.debug(
                    "Loading json file to db: %s (file: %s)"
                    % (inner_path, file)
                )
            try:
    ...
    def loadJson(self, inner_path):
        try:
            with self.open(inner_path, "r", encoding="utf_8") as file:
    ...
    def writeJson(self, inner_path, data):
        self.write(inner_path, helper.jsonDumps(data).encode("utf_8"))
            ...
            try:
                ...
                if "changed size during iteration" in str(err):
                    ...
                    if tries >= max_tries:
                        self.log.info(
                            (
                                "contents.json file list changed during"
                                " iteration. %s tries done. Giving up."
                            ),
                            tries,
                        )
                        stop = True
                    self.log.info(
                        (
                            "contents.json file list changed during iteration."
                            " Trying again… (%s)"
                        ),
                        tries,
                    )
        ...
        for content_inner_path, content in contents.items():
            ...
            for file_relative_path in list(content.get("files", {}).keys()):
                ...
                if quick_check:
                    ...
                    expected_size = content["files"][file_relative_path][
                        "size"
                    ]
                    ok = file_size == expected_size
                    if not ok:
                        err = "Invalid size: %s - actual, %s - expected" % (
                            file_size,
                            expected_size,
                        )
                else:
            ...
            for file_relative_path in list(
                ...
                file_inner_path = file_inner_path.strip("/")
                file_path = self.getPath(file_inner_path)
                hash_id = self.site.content_manager.hashfield.getHashId(
                    file_node["sha512"]
                )
                if not os.path.isfile(file_path):
                    if self.site.content_manager.isDownloaded(
                        file_inner_path, hash_id
                    ):
                        back["num_optional_removed"] += 1
                        self.log.debug(
                            "[OPTIONAL MISSING] %s" % file_inner_path
                        )
                        self.site.content_manager.optionalRemoved(
                            file_inner_path, hash_id, file_node["size"]
                        )
                    if add_optional and self.site.isDownloadable(
                        file_inner_path
                    ):
                        self.log.debug(
                            "[OPTIONAL ADDING] %s" % file_inner_path
                        )
                        bad_files.append(file_inner_path)
                    continue
                if quick_check:
                    ...
                    try:
                        ...
                        ok = False
                if ok:
                    if not self.site.content_manager.isDownloaded(
                        file_inner_path, hash_id
                    ):
    ...
    def updateBadFiles(self, quick_check=True):
        ...
        res = self.verifyFiles(
            quick_check,
            add_optional=True,
            add_changed=not self.site.settings.get("own"),
        )
        ...
        self.log.debug(
            "Checked files in %.2fs… Found bad files: %s, Quick:%s"
            % (time.time() - s, len(bad_files), quick_check)
        )
    ...
    def deleteFiles(self):
        site_title = self.site.content_manager.contents.get(
            "content.json", {}
        ).get("title", self.site.address)
        message_id = "delete-%s" % self.site.address
        self.log.debug(
            "Deleting files from content.json (title: %s)…" % site_title
        )
        files = []
        ...
        for i, content_inner_path in enumerate(content_inner_paths):
            ...
            if i % 100 == 0:
                ...
                self.site.messageWebsocket(
                    _(
                        "Deleting site <b>{site_title}</b>…<br>Collected"
                        " {num_files} files"
                    ),
                    message_id,
                    (i / len(content_inner_paths)) * 25,
                )
        if self.isFile("dbschema.json"):
        ...
        for i, inner_path in enumerate(files):
            ...
            if i % 100 == 0:
                self.site.messageWebsocket(
                    _(
                        "Deleting site <b>{site_title}</b>…<br>Deleting file"
                        " {i}/{num_files}"
                    ),
                    message_id,
                    25 + (i / num_files) * 50,
                    ...
                    try:
                        ...
                        if i % 100 == 0:
                            self.site.messageWebsocket(
                                _(
                                    "Deleting site"
                                    " <b>{site_title}</b>…<br>Deleting empty"
                                    " directories {i}"
                                ),
                                message_id,
                                85,
        ...
        if os.path.isdir(self.directory):
            ...
            self.site.messageWebsocket(
                _(
                    "Deleting site <b>{site_title}</b>…<br>Site deleted, but"
                    " some unknown files left in the directory"
                ),
                message_id,
                100,
            )
            ...
            self.log.debug(
                "Site %s data directory deleted: %s…"
                % (site_title, self.directory)
            )
            self.site.messageWebsocket(
                _(
                    "Deleting site <b>{site_title}</b>…<br>All files deleted"
                    " successfully"
                ),
                message_id,
                100,
