...
class BackgroundPublisher:
    ...
    def reinit(self, published=None, limit=5, diffs=None):
        if diffs is None:
            diffs = {}
        if published is None:
            published = []
    ...
    def processStage(self, stage):
        ...
        if not stage["thread"]:
            ...
            stage["thread"] = self.threads.spawn(
                self.site.publisher,
                self.inner_path,
                peers,
                self.published,
        ...
        stage["interval"].done()
        self.site.log.info(
            "Background publisher: Stage #%s: %s published to %s/%s peers",
            stage["nr"],
            self.inner_path,
            len(self.published),
            stage["limit"],
    ...
    def finalize(self):
        self.threads.kill()
        self.site.log.info(
            "Background publisher: Published %s to %s peers",
            self.inner_path,
            len(self.published),
        )
...
class Site(object):
    def __init__(self, address, allow_create=True, settings=None):
        ...
        self.greenlet_manager = GreenletManager.GreenletManager(
            self.connection_server.site_pool
        )
        ...
        self.peer_connector = SiteHelpers.PeerConnector(self)
        self.persistent_peer_req = None
        if not self.settings.get("wrapper_key"):
            self.settings["wrapper_key"] = CryptHash.random()
            self.log.debug(
                "New wrapper key: %s" % self.settings["wrapper_key"]
            )
    ...
    def loadSettings(self, settings=None):
        if not settings:
            settings = json.load(open("%s/sites.json" % config.data_dir)).get(
                self.address
            )
        if settings:
            ...
            for inner_path in self.bad_files:
                self.bad_files[inner_path] = min(
                    self.bad_files[inner_path], 20
                )
        else:
            self.settings = {
        ...
        if (
            self.address in (config.homepage, config.updatesite)
            and "ADMIN" not in self.settings["permissions"]
        ):
    ...
    def isStopping(self):
        return self.connection_server.stopping or self.settings.get(
            "deleting", False
        )
    ...
    def checkSendBackLRU(self, peer, inner_path, remote_modified):
        return SiteManager.site_manager.checkSendBackLRU(
            self, peer, inner_path, remote_modified
        )

    def addToSendBackLRU(self, peer, inner_path, modified):
        return SiteManager.site_manager.addToSendBackLRU(
            self, peer, inner_path, modified
        )

    def getSettingsCache(self):
        back = {}
        back["bad_files"] = self.bad_files
        back["hashfield"] = base64.b64encode(
            self.content_manager.hashfield.tobytes()
        ).decode("ascii")
        return back
    ...
    def isFileVerificationExpired(
        self, check_files_interval, verify_files_interval
    ):
        now = time.time()
        ...
        if check_files_interval is None:
            ...
            check_files_expiration = (
                check_files_timestamp + check_files_interval
            )
        if verify_files_interval is None:
            verify_files_expiration = now + 1
        else:
            verify_files_expiration = (
                verify_files_timestamp + verify_files_interval
            )
    ...
    def downloadContent(
        ...
        if config.verbose:
            self.log.debug(
                "DownloadContent %s: Started. (download_files: %s,"
                " check_modifications: %s, diffs: %s)…"
                % (
                    inner_path,
                    download_files,
                    check_modifications,
        ...
        if not inner_path.endswith("content.json"):
            return False
        found = self.needFile(
            inner_path, update=self.bad_files.get(inner_path)
        )
        content_inner_dir = helper.getDirname(inner_path)
        if not found:
            self.log.debug(
                "DownloadContent %s: Download failed, check_modifications: %s"
        ...
        if config.verbose:
            self.log.debug(
                "DownloadContent %s: loadContent done in %.3fs"
                % (inner_path, time.time() - sub_s)
            )
        ...
        if inner_path == "content.json":
            ...
            content_size = len(
                json.dumps(self.content_manager.contents[inner_path], indent=1)
            ) + sum(
                [
                    file["size"]
                    for file in list(
                        self.content_manager.contents[inner_path]
                        .get("files", {})
                        .values()
                    )
                    if file["size"] >= 0
            ...
            if site_size_limit < content_size:
                self.log.debug(
                    "DownloadContent Size limit reached (site too big please"
                    " increase limit): %.2f MB > %.2f MB"
                    % (
        ...
        if download_files:
                ...
                if diff_actions and self.bad_files.get(file_inner_path):
                    try:
                        ...
                        if diff_success:
                            ...
                            self.log.debug(
                                "DownloadContent Patched successfully: %s"
                                " (diff: %.3fs, verify: %.3fs, write: %.3fs,"
                                " on_done: %.3fs)"
                                % (
                                    file_inner_path,
                                    time_diff,
                        ...
                        self.log.debug(
                            "DownloadContent Failed to patch %s: %s"
                            % (file_inner_path, err)
                        )
                        diff_success = False
                if not diff_success:
            ...
            for file_relative_path in list(
                ...
                file_inner_path = content_inner_dir + file_relative_path
                if file_inner_path not in changed and not self.bad_files.get(
                    file_inner_path
                ):
                    continue
        ...
        if config.verbose:
            self.log.debug(
                "DownloadContent %s: Downloading %s includes…"
                % (inner_path, len(include_threads))
            )
        gevent.joinall(include_threads)
        if config.verbose:
            self.log.debug(
                "DownloadContent %s: Includes download ended" % inner_path
            )
        if check_modifications:
            self.checkModifications(0)
        if config.verbose:
            self.log.debug(
                "DownloadContent %s: Downloading %s files, changed: %s…"
                % (inner_path, len(file_threads), len(changed))
            )
        gevent.joinall(file_threads)
        if config.verbose:
            self.log.debug(
                "DownloadContent %s: ended in %.3fs (tasks left: %s)"
    ...
    def getReachableBadFiles(self):
        if not self.bad_files:
            return False
        return [
            bad_file for bad_file, retry in self.bad_files.items() if retry < 3
        ]
        ...
        for bad_file in list(self.bad_files.keys()):
            ...
            if bad_file.endswith("content.json"):
                if file_info is False and bad_file != "content.json":
                    ...
                    self.log.debug(
                        "No info for file: %s, removing from bad_files"
                        % bad_file
                    )
            else:
                if file_info is False or not file_info.get("size"):
                    del self.bad_files[bad_file]
                    self.log.debug(
                        "No info or size for file: %s, removing from bad_files"
                        % bad_file
                    )

    @util.Noparallel(blocking=False)
    def download(
        self, check_size=False, blind_includes=False, retry_bad_files=True
    ):
        ...
        s = time.time()
        self.log.debug(
            "Start downloading, bad_files: %s, check_size: %s, blind_includes:"
            " %s, isAddedRecently: %s"
            % (
                self.bad_files,
                check_size,
                blind_includes,
                self.isAddedRecently(),
            )
        )
        ...
        valid = self.downloadContent(
            "content.json", check_modifications=blind_includes
        )
        if retry_bad_files:
    ...
    def pooledDownloadContent(
        self, inner_paths, pool_size=100, only_if_bad=False
    ):
        self.log.debug(
            "New downloadContent pool: len: %s, only if bad: %s"
            % (len(inner_paths), only_if_bad)
        )
        ...
        for inner_path in inner_paths:
            ...
            if self.settings["size"] > site_size_limit * 0.95:
                self.log.warning(
                    "Site size limit almost reached, aborting downloadContent"
                    " pool"
                )
        ...
        pool.join()
        self.log.debug(
            "Ended downloadContent pool len: %s, skipped: %s"
            % (len(inner_paths), num_skipped)
        )

    def pooledDownloadFile(
        self, inner_paths, pool_size=100, only_if_bad=False
    ):
        self.log.debug(
            "New downloadFile pool: len: %s, only if bad: %s"
            % (len(inner_paths), only_if_bad)
        )
        self.worker_manager.started_task_num += len(inner_paths)
        pool = gevent.pool.Pool(pool_size)
        ...
        for inner_path in inner_paths:
            ...
            self.worker_manager.started_task_num -= 1
        self.log.debug(
            "Ended downloadFile pool len: %s, skipped: %s"
            % (len(inner_paths), num_skipped)
        )
    ...
    def updater(self, peers_try, queried, need_queries, since):
        threads = []
        while 1:
            if not peers_try or len(queried) >= need_queries:
                break
            ...
            if config.verbose:
                self.log.debug(
                    "CheckModifications: Trying to get updates from: %s"
                    " Left: %s" % (peer, peers_try)
                )
            res = None
            ...
            inner_paths = itertools.chain(
                peer_modified_files.keys(), my_modified_files.keys()
            )
            seen_inner_paths = {}
            for inner_path in inner_paths:
                ...
                has_older = diff < 0
                if (
                    inner_path not in self.bad_files
                    and not self.content_manager.isArchived(
                        inner_path, peer_modified
                    )
                ):
                    if has_newer:
                    ...
                    if has_older:
                        if self.checkSendBackLRU(
                            peer, inner_path, peer_modified
                        ):
                            send_back_skipped += 1
                        else:
            ...
            if modified_contents:
                self.log.info(
                    "CheckModifications: %s new modified files from %s"
                    % (len(modified_contents), peer)
                )
                modified_contents.sort(
                    key=lambda inner_path: 0 - peer_modified_files[inner_path]
                )
                for inner_path in modified_contents:
                    self.log.info(
                        "CheckModifications:     %s: %s > %s"
                        % (
                            inner_path,
                            peer_modified_files.get(inner_path, 0),
                            my_modified_files.get(inner_path, 0),
                        )
                    )
                t = self.spawn(
                    self.pooledDownloadContent,
                    modified_contents,
                    only_if_bad=True,
                )
                threads.append(t)
            if send_back:
                self.log.info(
                    "CheckModifications: %s has older versions of %s files"
                    % (peer, len(send_back))
                )
                if len(send_back) > send_back_limit:
                    self.log.info(
                        "CheckModifications: choosing %s random files to"
                        " publish back" % send_back_limit
                    )
                ...
                for inner_path in send_back:
                    ...
                    self.spawn(
                        ...
                        [],
                        1,
                        save_to_send_back_lru=True,
                    )
            if send_back_skipped:
                self.log.info(
                    "CheckModifications: %s has older versions of %s files,"
                    " skipped according to send back LRU"
                    % (peer, send_back_skipped)
                )
        self.log.debug(
            "CheckModifications: Waiting for %s pooledDownloadContent"
            % len(threads)
        )
        gevent.joinall(threads)

    def getForcedP2PSince(self):
        now = time.time()
    ...
    def checkModifications(self, since=None):
        ...
        if since == 0:
            peer_limit *= 4
            need_queries *= 4
        peers_try = self.getConnectedPeers() + self.getConnectablePeers(
            peer_limit
        )
        self.log.debug(
            "CheckModifications: Trying to get listModifications from %s"
            " peers, %s connected, since: %s"
        ...
        for i in range(updater_limit):
            updaters.append(
                self.spawn(
                    self.updater, peers_try, queried, need_queries, since
                )
            )
        for r in range(10):
            ...
            if len(queried) >= need_queries or len(peers_try) == 0:
                break
            self.log.debug(
                "CheckModifications: Waiting… (%s) successfully queried: %s,"
                " left: %s" % (r + 1, len(queried), len(peers_try))
            )
        self.log.debug(
            "CheckModifications: Queried listModifications from %s peers in"
            " %.3fs since %s" % (len(queried), time.time() - s, since)
        )
        time.sleep(0.1)
        return queried
    ...
    def invalidateUpdateTime(self, invalid_interval):
        ...
        if a is None:
            a = b
        if a <= self.last_online_update <= b:
            self.last_online_update = 0
    ...
    @util.Noparallel()
    def update(
        self, announce=False, check_files=False, verify_files=False, since=None
    ):
        ...
        changed, deleted = self.content_manager.loadContent(
            "content.json", load_includes=False
        )
        if self.bad_files:
    ...
    def redownloadContents(self):
        ...
        self.log.debug(
            "Waiting %s content.json to finish…" % len(content_threads)
        )
    ...
    def publisher(
        ...
        while 1:
            ...
            if peer.connection and peer.connection.last_ping_delay:
                timeout = (
                    5 + int(file_size / 1024) + peer.connection.last_ping_delay
                )
            else:
            ...
            for retry in range(max_retries):
                try:
                    ...
                    self.log.error(
                        "Publish error: %s" % Debug.formatException(err)
                    )
    ...
    def addBackgroundPublisher(
        self, published=[], limit=5, inner_path="content.json", diffs={}
    ):
        with self.background_publishers_lock:
            if self.background_publishers.get(inner_path, None):
                ...
                background_publisher.reinit(
                    published=published, limit=limit, diffs=diffs
                )
            else:
    ...
    def processBackgroundPublishers(self):
        with self.background_publishers_lock:
            for inner_path, background_publisher in list(
                self.background_publishers.items()
            ):
    ...
    def getPeersForForegroundPublishing(self, limit):
        reqs = [
            self.peer_connector.newReq(limit, limit / 2, 10),
            self.peer_connector.newReq(1, 1, 60),
        ]
        ...
        peers = sorted(
            peers,
            key=lambda peer: peer.connection.handshake.get("rev", 0)
            < config.rev - 100,
        )
    ...
    def publish(
        ...
        self.log.info(
            "Publishing %s to %s/%s peers (connected: %s) diffs: %s (%.2fk)…"
            % (
                inner_path,
                limit,
                len(self.peers),
                len(self.getConnectedPeers()),
                list(diffs.keys()),
                float(len(str(diffs))) / 1024,
            )
        )
        if not peers:
            self.addBackgroundPublisher(
                published=published,
                limit=limit,
                inner_path=inner_path,
                diffs=diffs,
        ...
        self.log.info(
            "Published %s to %s peers, publishing to more peers in the"
            " background" % (inner_path, len(published))
        )
        self.addBackgroundPublisher(
            published=published,
            limit=limit,
    ...
    def clone(
        ...
        self.log.debug(
            "Cloning to %s, ignore dirs: %s, root: %s"
            % (address, default_dirs, root_inner_path)
        )
        if not new_site.storage.isFile("content.json") and not overwrite:
            ...
            if self.storage.isFile(root_inner_path + "/content.json-default"):
                content_json = self.storage.loadJson(
                    root_inner_path + "/content.json-default"
                )
            else:
        ...
        for content_inner_path, content in list(
            self.content_manager.contents.items()
        ):
            file_relative_paths = list(content.get("files", {}).keys())
            file_relative_paths.sort()
            file_relative_paths.sort(
                key=lambda key: key.replace("-default", "").endswith(
                    "content.json"
                )
            )
            for file_relative_path in file_relative_paths:
                file_inner_path = (
                    helper.getDirname(content_inner_path) + file_relative_path
                )
                file_inner_path = file_inner_path.strip("/")
                if not file_inner_path.startswith(root_inner_path):
                    self.log.debug(
                        "[SKIP] %s (not in clone root)" % file_inner_path
                    )
                    continue
                if file_inner_path.split("/")[0] in default_dirs:
                    self.log.debug(
                        "[SKIP] %s (has default alternative)" % file_inner_path
                    )
                    continue
                file_path = self.storage.getPath(file_inner_path)
                if root_inner_path:
                    file_inner_path_dest = re.sub(
                        "^%s/" % re.escape(root_inner_path),
                        "",
                        file_inner_path,
                    )
                    file_path_dest = new_site.storage.getPath(
                        file_inner_path_dest
                    )
                else:
                    file_inner_path_dest = file_inner_path
                    file_path_dest = new_site.storage.getPath(file_inner_path)
                self.log.debug(
                    "[COPY] %s to %s…" % (file_inner_path, file_path_dest)
                )
                dest_dir = os.path.dirname(file_path_dest)
                if not os.path.isdir(dest_dir):
                    os.makedirs(dest_dir)
                if (
                    file_inner_path_dest.replace("-default", "")
                    == "content.json"
                ):
                    continue
                ...
                if "-default" in file_inner_path_dest:
                    ...
                    if (
                        new_site.storage.isFile(
                            file_inner_path_dest.replace("-default", "")
                        )
                        and not overwrite
                    ):
                        self.log.debug(
                            "[SKIP] Default file: %s (already exist)"
                            % file_inner_path
                        )
                        continue
                    self.log.debug(
                        "[COPY] Default file: %s to %s…"
                        % (file_inner_path, file_path_dest)
                    )
                    dest_dir = os.path.dirname(file_path_dest)
                    if not os.path.isdir(dest_dir):
                        os.makedirs(dest_dir)
    ...
    def isFileDownloadAllowed(self, inner_path, file_info):
        ...
        if file_info.get("size", 0) > config.file_size_limit * 1024 * 1024:
            self.log.debug(
                ...
                % (
                    inner_path,
                    file_info.get("size", 0) / 1024 / 1024,
    ...
    def needFileInfo(self, inner_path):
        ...
        if not file_info:
            self.log.debug(
                "No info for %s, waiting for all content.json" % inner_path
            )
            success = self.downloadContent(
                "content.json", download_files=False
            )
            if not success:
                return False
            file_info = self.content_manager.getFileInfo(inner_path)
    ...
    def needFile(
        ...
        if self.worker_manager.tasks.findTask(inner_path):
            task = self.worker_manager.addTask(
                inner_path, peer, priority=priority
            )
            if blocking:
                return task["evt"].get()
            else:
            ...
            if not self.content_manager.contents.get("content.json"):
                self.log.debug(
                    "Need content.json first (inner_path: %s, priority: %s)"
                    % (inner_path, priority)
                )
                if priority > 0:
            ...
            if not inner_path.endswith("content.json"):
                ...
                if not file_info:
                    return False
                if (
                    "cert_signers" in file_info
                    and not file_info["content_inner_path"]
                    in self.content_manager.contents
                ):
                    self.log.debug(
                        "Missing content.json for requested user file: %s"
                        % inner_path
                    )
                    if (
                        self.bad_files.get(file_info["content_inner_path"], 0)
                        > 5
                    ):
                        self.log.debug(
                            "File %s not reachable: retry %s"
                            % (
                                inner_path,
                                self.bad_files.get(
                                    file_info["content_inner_path"], 0
                                ),
                            )
                        )
                        return False
            ...
            task = self.worker_manager.addTask(
                inner_path, peer, priority=priority, file_info=file_info
            )
            if blocking:
                return task["evt"].get()
            else:
    ...
    def getActiveConnectionCountLimit(self):
        ...
        limit = (
            self.getPreferableActiveConnectionCount() + count_above_preferable
        )
            ...
            if i > 0 and nr_connected_peers != waiting_req.nr_connected_peers:
                ...
                if update_websocket:
                    self.updateWebsocket(
                        connecting_to_peers=nr_connected_peers
                    )
            i += 1
        if update_websocket:
    ...
    def getConnectablePeers(self, need_num=5, ignore=None, allow_private=True):
        if ignore is None:
            ignore = []
        ...
        for peer in peers:
            ...
            if len(connectable_peers) >= need_num:
                break
        if len(connectable_peers) < need_num:
            connectable_peers += reachable_peers[
                0 : need_num - len(connectable_peers)
            ]
    ...
    def getRecentPeers(self, need_num):
        ...
        if len(found) < need_num and len(found) < len(self.peers):
            ...
            peers = sorted(
                list(peers), key=lambda peer: peer.reputation, reverse=True
            )
            while len(found) < need_num and len(peers) > 0:
                found.add(peers.pop())
        return sorted(
            list(found), key=lambda peer: peer.reputation, reverse=True
        )[0:need_num]
    ...
    def getConnectedPeers(self, only_fully_connected=False):
        ...
        for connection in self.connection_server.connections:
            if len(back) >= len(self.peers):
                break
            if (
                not connection.connected
                and time.time() - connection.start_time > 20
            ):
                continue
            if not connection.connected and only_fully_connected:
                continue
            peer = self.peers.get("%s:%s" % (connection.ip, connection.port))
            if peer:
                if (
                    connection.ip.endswith(".onion")
                    and connection.target_onion
                    and tor_manager.start_onions
                ):
                    valid_target_onions = (
                        tor_manager.getOnion(self.address),
                        tor_manager.getOnion("global"),
                    )
    ...
    def removeDeadPeers(self):
        ...
        if removed:
            self.log.debug(
                "Cleanup peers result: Removed %s, left: %s"
                % (removed, len(self.peers))
            )

    def cleanupPeers(self):
        self.removeDeadPeers()
        limit = max(
            self.getActiveConnectionCountLimit(), self.waitingForConnections()
        )
        ...
        if need_to_close > 0:
            closed = 0
            for peer in sorted(
                connected_peers, key=lambda peer: min(peer.connection.sites, 5)
            ):
                ...
                if closed >= need_to_close:
                    break
            self.log.debug(
                "Connected: %s, Need to close: %s, Closed: %s"
    ...
    def runPeriodicMaintenance(self, startup=False, force=False):
        if not self.isServing():
            return False
        self.log.debug(
            "runPeriodicMaintenance: startup=%s, force=%s" % (startup, force)
        )
        result = False
    ...
    def periodicMaintenanceHandler_general(self, startup=False, force=False):
        ...
        if not self.peers:
            return False
        self.log.debug(
            "periodicMaintenanceHandler_general: startup=%s, force=%s"
            % (startup, force)
        )
    ...
    def periodicMaintenanceHandler_peer_check(
        self, startup=False, force=False
    ):
        if not self.isServing():
            return False
        if not self.peers:
            return False
        self.log.debug(
            "periodicMaintenanceHandler_peer_check: startup=%s, force=%s"
            % (startup, force)
        )
        if not startup:
            self.cleanupPeers()
    ...
    def periodicMaintenanceHandler_announce(self, startup=False, force=False):
        if not self.isServing():
            return False
        self.log.debug(
            "periodicMaintenanceHandler_announce: startup=%s, force=%s"
            % (startup, force)
        )
    ...
    def sendMyHashfield(self, limit=5):
        ...
        if sent:
            ...
            self.log.debug(
                "Sent my hashfield (changed %.3fs ago) to %s peers"
                % (time.time() - my_hashfield_changed, sent)
            )
        return sent
    ...
    def updateHashfield(self, limit=5):
        if not self.isServing():
            return False
        if (
            not self.content_manager.hashfield
            and not self.content_manager.has_optional_files
        ):
            return False
        s = time.time()
        queried = 0
        ...
        if queried:
            self.log.debug(
                "Queried hashfield from %s peers in %.3fs"
                % (queried, time.time() - s)
            )
        return queried
    ...
    def delete(self):
        ...
        self.saveSettings()
        num_greenlets = self.greenlet_manager.stopGreenlets(
            "Site %s deleted" % self.address
        )
    ...
    def fileFailed(self, inner_path):
        ...
        if (
            inner_path in self.bad_files
            and self.connection_server.has_internet
        ):
