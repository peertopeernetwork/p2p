...
class Worker(object):
        ...
        for sleep_i in range(1, timeout * 10):
            ...
            if task["done"] or task["workers_num"] == 0:
                if config.verbose:
                    self.manager.log.debug(
                        "%s: %s, picked task free after %ss sleep. (done: %s)"
                        % (
                            self.key,
                            task["inner_path"],
                            0.1 * sleep_i,
            ...
            if sleep_i % 10 == 0:
                ...
                if not workers or not workers[0].peer.connection:
                    break
                worker_idle = (
                    time.time() - workers[0].peer.connection.last_recv_time
                )
                if worker_idle > 1:
                    if config.verbose:
                        self.manager.log.debug(
                            "%s: %s, worker %s seems idle, picked up task"
                            " after %ss sleep. (done: %s)"
                            % (
                                self.key,
                                task["inner_path"],
                                workers[0].key,
    ...
    def pickTask(self):
        ...
        if not task:
            ...
            task = self.manager.getTask(self.peer)
            if not task:
                stats = "downloaded files: %s, failed: %s" % (
                    self.num_downloaded,
                    self.num_failed,
                )
                self.manager.log.debug(
                    "%s: No task found, stopping (%s)" % (self.key, stats)
                )
                return False
        ...
        if task["workers_num"] > 0:
            ...
            if config.verbose:
                self.manager.log.debug(
                    "%s: Someone already working on %s (pri: %s), sleeping %s"
                    " sec…"
    ...
    def downloadTask(self, task):
        try:
            buff = self.peer.getFile(
                task["site"].address, task["inner_path"], task["size"]
            )
    ...
    def writeTask(self, task, buff):
        ...
        try:
            ...
            if type(err) == Debug.Notify:
                self.manager.log.debug(
                    "%s: Write aborted: %s (%s: %s)"
                    % (self.key, task["inner_path"], type(err), err)
                )
            else:
                self.manager.log.error(
                    "%s: Error writing: %s (%s: %s)"
                    % (self.key, task["inner_path"], type(err), err)
                )
    ...
    def onTaskVerifyFail(self, task, error_message):
        ...
        if (
            self.peer.hash_failed >= max(len(self.manager.tasks), 3)
            or self.peer.connection_error > 10
        ):
            raise WorkerStop(
    ...
    def handleTask(self, task):
        ...
        try:
            ...
            if self.running is False:
                self.manager.log.debug(
                    "%s: No longer needed, returning: %s"
                    % (self.key, task["inner_path"])
                )
                raise WorkerStop("Running got disabled")
            write_lock = self.getTaskLock(task)
            write_lock.acquire()
            if (
                task["site"].content_manager.verifyFile(
                    task["inner_path"], buff
                )
                is None
            ):
                is_same = True
        ...
        if is_valid and not is_same:
            if (
                self.manager.started_task_num < 50
                or task["priority"] > 10
                or config.verbose
            ):
                self.manager.log.debug(
                    "%s: Verify correct: %s" % (self.key, task["inner_path"])
                )
            try:
        ...
        if not task["done"]:
            if write_err:
                ...
                self.manager.log.error(
                    "%s: Error writing %s: %s"
                    % (self.key, task["inner_path"], write_err)
                )
            elif is_valid:
    ...
    def downloader(self):
        ...
        while self.running:
            ...
            try:
                ...
                self.manager.log.debug(
                    "%s: Worker stopped: %s" % (self.key, err)
                )
    ...
    def skip(self, reason="Unknown"):
        self.manager.log.debug(
            "%s: Force skipping (reason: %s)" % (self.key, reason)
        )
        if self.thread:
            self.thread.kill(
                exception=Debug.createNotifyType(
                    "Worker skipping (reason: %s)" % reason
                )
            )
        self.start()
    ...
    def stop(self, reason="Unknown"):
        self.manager.log.debug(
            "%s: Force stopping (reason: %s)" % (self.key, reason)
        )
        self.running = False
        if self.thread:
            self.thread.kill(
                exception=Debug.createNotifyType(
                    "Worker stopped (reason: %s)" % reason
                )
            )
        del self.thread
        self.manager.removeWorker(self)
