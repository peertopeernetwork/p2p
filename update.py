...
def update():
    ...
    updatesite_bad_files = (
        sites_json.get(config.updatesite, {})
        .get("cache", {})
        .get("bad_files", {})
    )
    print(
        "Update site path: %s, bad_files: %s, source path: %s, runtime path:"
        " %s, dist type: %s"
        % (
            updatesite_path,
            len(updatesite_bad_files),
            source_path,
            runtime_path,
            config.dist_type,
        )
    )
    updatesite_content_json = json.load(
        open(updatesite_path + "/content.json")
    )
    inner_paths = list(updatesite_content_json.get("files", {}).keys())
    inner_paths += list(
        updatesite_content_json.get("files_optional", {}).keys()
    )
    inner_paths = [
        inner_path
        for inner_path in inner_paths
        if re.match("^(core|bundle)", inner_path)
    ]
    plugins_enabled = []
    plugins_disabled = []
    if os.path.isdir("%s/plugins" % source_path):
        ...
        print(
            "Plugins enabled:", plugins_enabled, "disabled:", plugins_disabled
        )
    update_paths = {}
    for inner_path in inner_paths:
        if ".." in inner_path:
        ...
        if inner_path.startswith("core"):
            ...
            dest_path = (
                runtime_path + "/" + re.sub("^bundle[^/]+/", "", inner_path)
            )
        else:
            continue
        if not dest_path:
            continue
        match = re.match(
            re.escape(source_path) + "/plugins/([^/]+)", dest_path
        )
        if match:
            plugin_name = match.group(1).replace("disabled-", "")
            if plugin_name in plugins_enabled:
                dest_path = dest_path.replace(
                    "plugins/disabled-" + plugin_name, "plugins/" + plugin_name
                )
            elif plugin_name in plugins_disabled:
                dest_path = dest_path.replace(
                    "plugins/" + plugin_name, "plugins/disabled-" + plugin_name
                )
            print("P", end=" ")
    ...
    for path_from, path_to in update_paths.items():
        ...
        try:
            ...
            try:
                print(
                    "Error writing: %s. Renaming old file as workaround…" % err
                )
                ...
                num_error += 1
    print(
        "* Updated files: %s, renamed: %s, error: %s"
        % (num_ok, num_rename, num_error)
    )
